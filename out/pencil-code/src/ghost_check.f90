...
integer :: px, py, pz, lx, ux, ly, uy, lz, uz, alloc_err, mwx, mwy, mwz
logical :: ok
!
...
if (lroot) then
mwx=mx*nprocx; mwy=my*nprocy; mwz=mz*nprocz
endif
if (lfirst_proc_xy) then
allocate (buffer(mwx,mwy,mz,mfarray), stat=alloc_err)
if (alloc_err > 0) call stop_it ('check_ghosts_consistency: could not allocate buffer memory.')
endif
if (lroot) then
allocate (global(mwx,mwy,mwz,mfarray), stat=alloc_err)
