...
namelist /eos_init_pars/  xHe, mu, cp, cs0, rho0, gamma, error_cp,  sigmaSBt, lanelastic_lin, lcs_as_aux, lcs_as_comaux, fac_cs,isothmid, lstratz, gztype, gz_coeff, lpres_grad,  lcs_tdep, cs20_tdep_rate
!
...
namelist /eos_run_pars/  xHe, mu, cp, cs0, rho0, gamma, error_cp,  pres_corr, sigmaSBt,  lanelastic_lin, lcs_as_aux, lcs_as_comaux,  lcs_tdep, cs20_tdep_rate
!
...
if (lcs_as_aux .or. lcs_as_comaux)  call register_report_aux('cs',ics,communicated=lcs_as_comaux)
!
...
if (lanelastic)  call put_shared_variable('lanelastic_lin',lanelastic_lin)
!
...
if (lfargo_advection.and.(pretend_lnTT.or.ltemperature))  call fatal_error("initialize_eos","fargo advection not "// "implemented for the temperature equation")
!
...
if (ieosvar_count>=2)  call fatal_error('select_eos_variable',  '2 thermodynamic quantities have already been defined '//  'while attempting to add a 3rd')
!
...
call fatal_error('select_eos_variable',  'This thermodynamic variable combination is not implemented')
endselect
!
...
if (lpencil_in(i_cs2).or.  lpencil_in(i_TT).or.  lpencil_in(i_lnTT).or.  lpencil_in(i_TT1)) then
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='calc_pencils_eos')
!
...
if (lpenc_loc(i_del6ss)) call fatal_error('calc_pencils_eos',  'del6ss not available for ilnrho_lnTT')
!
...
if (lpenc_loc(i_lnTT).or.lpenc_loc(i_ss).or.lpenc_loc(i_ee))  p%lnTT=log(f(l1:l2,m,n,ieosvar2))
...
if (lpenc_loc(i_lnTT)) call fatal_error('calc_pencils_eos',  'temperature not needed for localisothermal')
if (lpenc_loc(i_glnTT))  p%glnTT=f(l1:l2,m,n,iglobal_glnTT:iglobal_glnTT+2)
if (lpenc_loc(i_hlnTT)) call fatal_error('calc_pencils_eos',  'no gradients yet for localisothermal')
if (lpenc_loc(i_del2lnTT)) call fatal_error('calc_pencils_eos',  'no gradients yet for localisothermal')
if (lpenc_loc(i_ss)) call fatal_error('calc_pencils_eos',  'entropy not needed for localisothermal')
if (lpenc_loc(i_del2ss)) call fatal_error('calc_pencils_eos',  'no gradients yet for localisothermal')
if (lpenc_loc(i_gss)) call fatal_error('calc_pencils_eos',  'entropy gradient not needed for localisothermal')
if (lpenc_loc(i_hss)) call fatal_error('calc_pencils_eos',  'no gradients yet for localisothermal')
...
if (lpenc_loc(i_TT)) p%TT=exp(p%lnTT)
if (lpenc_loc(i_TT1)) p%TT1=exp(-p%lnTT)
if (lpenc_loc(i_del6ss)) call fatal_error('calc_pencils_eos',  'del6ss not available for ilnrho_cs2')
!
...
if (lpenc_loc(i_hlnTT)) call fatal_error('calc_pencils_eos',  'hlnTT not yet implemented for ilnrho_eth or irho_eth')
!
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,  caller='pressure_gradient_farray')
!
...
if (gamma_m1==0.) call fatal_error('temperature_gradient',  'gamma=1 not allowed with entropy turned on!')
!
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='eosperturb')
!
...
case (ilnrho_ss,irho_ss)
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='eoscalc_farray')
!
...
if (gamma_m1==0.and..not.lanelastic) call fatal_error  ('eoscalc_point','gamma=1 not allowed w/entropy')
!
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='isothermal_entropy')
...
average_pressure = average_pressure+((average_density/ init_average_density)**gamma-1.0)*pp0*pres_corr
...
cs2_xy = f(:,:,n1,iss)
if (lreference_state)  cs2_xy(l1:l2,:) = cs2_xy(l1:l2,:) + spread(reference_state(:,iref_s),2,my)
!
...
do i=1,nghost
call getdlnrho_z(f(:,:,:,ilnrho),n1,i,rho_xy)
...
do i=1,nghost
call getdlnrho_z(f(:,:,:,ilnrho),n2,i,rho_xy)
...
character (len=3) :: topbot
real, dimension (:,:,:,:) :: f
real, dimension (size(f,1),size(f,2)) :: dsdz_xy,cs2_xy,lnrho_xy,rho_xy,  TT_xy,dlnrhodz_xy,chi_xy
integer :: i
!
...
dlnrhodz_xy= coeffs_1_z(1,1)*(f(:,:,n1+1,ilnrho)-f(:,:,n1-1,ilnrho))  +coeffs_1_z(2,1)*(f(:,:,n1+2,ilnrho)-f(:,:,n1-2,ilnrho))  +coeffs_1_z(3,1)*(f(:,:,n1+3,ilnrho)-f(:,:,n1-3,ilnrho))
!

if (lheatc_kramers) then
dsdz_xy= cv*( (sigmaSBt/hcond0_kramers)*TT_xy**(3-6.5*nkramers)*rho_xy**(2.*nkramers)  +gamma_m1*dlnrhodz_xy)
else
dsdz_xy=(sigmaSBt*TT_xy**3+hcondzbot*gamma_m1*dlnrhodz_xy)/  (chit_prof1*chi_t*rho_xy+hcondzbot/cv)
endif
!
...
cs2_xy=gamma_m1*(lnrho_xy-lnrho0)+cv1*f(:,:,n2,iss)
if (lreference_state)  cs2_xy(l1:l2,:)=cs2_xy(l1:l2,:) + cv1*spread(reference_state(:,iref_s),2,my)
cs2_xy=cs20*exp(cs2_xy)
call getrho(f(:,:,n2,ilnrho),rho_xy)
endif
...
TT_xy=cs2_xy/(gamma_m1*cp)
dlnrhodz_xy= coeffs_1_z(1,2)*(f(:,:,n2+1,ilnrho)-f(:,:,n2-1,ilnrho))  +coeffs_1_z(2,2)*(f(:,:,n2+2,ilnrho)-f(:,:,n2-2,ilnrho))  +coeffs_1_z(3,2)*(f(:,:,n2+3,ilnrho)-f(:,:,n2-3,ilnrho))
!
...
if (lheatc_kramers) then
dsdz_xy=-cv*(sigmaSBt*TT_xy**3+hcond0_kramers*TT_xy**(6.5*nkramers)*rho_xy**(-2.*nkramers)*  gamma_m1*dlnrhodz_xy)/(hcond0_kramers*TT_xy**(6.5*nkramers)*rho_xy**(-2.*nkramers)  + chit_prof2*chi_t*rho_xy/gamma)
elseif (hcondztop==impossible) then
dsdz_xy=-(sigmaSBt*TT_xy**3+chi_xy*rho_xy*cp*gamma_m1*dlnrhodz_xy)/  (chit_prof2*chi_t*rho_xy+chi_xy*rho_xy*cp/cv)
else
!
...
dsdz_xy=-(sigmaSBt*TT_xy**3+hcondztop*gamma_m1*dlnrhodz_xy)/  (chit_prof2*chi_t*rho_xy+hcondztop/cv)
endif
!
...
if (pretend_lnTT) then
do i=1,nghost
f(l1-i,:,:,iss)=f(l1+i,:,:,iss) +  dx2_bound(-i)*sigmaSBt*exp(f(l1,:,:,iss))**3/hcondxbot
enddo
else
!
...
dlnrhodx_yz= coeffs_1_x(1,1)*(f(l1+1,:,:,ilnrho)-f(l1-1,:,:,ilnrho))  +coeffs_1_x(2,1)*(f(l1+2,:,:,ilnrho)-f(l1-2,:,:,ilnrho))  +coeffs_1_x(3,1)*(f(l1+3,:,:,ilnrho)-f(l1-3,:,:,ilnrho))
!
...
if (lheatc_kramers) then
dsdx_yz=-cv*( (sigmaSBt/hcond0_kramers)*TT_yz**(3-6.5*nkramers)*rho_yz**(2.*nkramers)  +gamma_m1*dlnrhodx_yz)
else
dsdx_yz=-(sigmaSBt*TT_yz**3+hcondxbot*gamma_m1*dlnrhodx_yz)/  (chit_prof1*chi_t*rho_yz+hcondxbot/cv)
endif
!
...
if (pretend_lnTT) then
do i=1,nghost
f(l2+i,:,:,iss)=f(l2-i,:,:,iss) +  dx2_bound(i)*sigmaSBt*exp(f(l2,:,:,iss))**3/hcondxtop
enddo
else
!
...
if (ldensity_nolog) then
cs2_yz=f(l2,:,:,iss)
if (lreference_state)  cs2_yz = cs2_yz+reference_state(TOP,iref_s)
cs2_yz=cs20*exp(gamma_m1*(log(rho_yz)-lnrho0)+cv1*cs2_yz)
else
...
dlnrhodx_yz= coeffs_1_x(1,2)*(f(l2+1,:,:,ilnrho)-f(l2-1,:,:,ilnrho))  +coeffs_1_x(2,2)*(f(l2+2,:,:,ilnrho)-f(l2-2,:,:,ilnrho))  +coeffs_1_x(3,2)*(f(l2+3,:,:,ilnrho)-f(l2-3,:,:,ilnrho))
!
...
if (lheatc_kramers) hcond_total = hcond_total +  hcond0_kramers*TT_yz**(6.5*nkramers)*rho_yz**(-2.*nkramers)
!

dsdx_yz = -(sigmaSBt*TT_yz**3+hcond_total*gamma_m1*dlnrhodx_yz) /  (chit_prof2*chi_t*rho_yz+hcond_total/cv)
!
...
if (ldensity_nolog) then
TT_yz=f(l1,:,:,iss)
if (lreference_state)  TT_yz = TT_yz+reference_state(BOT,iref_s)
TT_yz=cs20*exp(gamma_m1*(log(rho_yz)-lnrho0)+cv1*TT_yz)
else
TT_yz=cs20*exp(gamma_m1*(f(l1,:,:,ilnrho)-lnrho0)+cv1*f(l1,:,:,iss))
endif
TT_yz=TT_yz/(cp*gamma_m1)
!
...
f(l1-i,:,:,iss)=f(l1+i,:,:,iss) +  Kxbot*gamma_m1/(Kxbot*cv1+chit_prof1*chi_t*rho_yz)*  dlnrhodx_yz + dx2_bound(-i)*dsdx_yz
enddo
endif
!
...
lf=l+nghost+1
do n=n1,n2
call getlnrho(f(lf,:,n,ilnrho),BOT,tmp2)
...
dlnrmxdx= coeffs_1_x(1,1)*(lnrmx(1)-lnrmx(-1))  +coeffs_1_x(2,1)*(lnrmx(2)-lnrmx(-2))  +coeffs_1_x(3,1)*(lnrmx(3)-lnrmx(-3))
!
...
dsdx_yz=(-cp*gamma_m1*Fbot/cs2mx)/  (chit_prof1*chi_t*exp(lnrmx(0)) + hcondxbot*gamma) -  gamma_m1/gamma*dlnrmxdx
!
...
call getlnrho(f(:,:,n1,ilnrho),rho_xy)
TT_xy=f(:,:,n1,iss)
if (lreference_state)  TT_xy(l1:l2,:) = TT_xy(l1:l2,:) + spread(reference_state(:,iref_s),2,my)
TT_xy=cs20*exp(gamma_m1*(rho_xy-lnrho0)+cv1*TT_xy)
TT_xy=TT_xy/(cp*gamma_m1)
!
...
do i=1,nghost
call getdlnrho_z(f(:,:,:,ilnrho),n1,i,TT_xy)
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_ss_temp_old')
!
...
case ('bot')
if ((bcz12(ilnrho,1) /= 'a2') .and. (bcz12(ilnrho,1) /= 'a3'))  call fatal_error('bc_ss_temp_old','Inconsistent boundary conditions 3.')
...
if (lreference_state)  tmp_xy(l1:l2,:) = tmp_xy(l1:l2,:) - spread(reference_state(:,iref_s),2,my)
f(:,:,n1,iss) = tmp_xy
!
...
case ('top')
if ((bcz12(ilnrho,2) /= 'a2') .and. (bcz12(ilnrho,2) /= 'a3'))  call fatal_error('bc_ss_temp_old','Inconsistent boundary conditions 3.')
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_ss_temp_x')
!
...
if (lreference_state)  f(l1,:,:,iss) = f(l1,:,:,iss) - reference_state(BOT,iref_s)
!
...
f(l1-i,:,:,iss) = - f(l1+i,:,:,iss) + tmp - 2*reference_state(i,iref_s)  - (cp-cv)*(log( (f(l1+i,:,:,irho)+reference_state(i,iref_rho))  *(f(l1-i,:,:,irho)+reference_state(i,iref_rho)) ) - 2*lnrho0)
else
f(l1-i,:,:,iss) = - f(l1+i,:,:,iss) + tmp  - (cp-cv)*(log(f(l1+i,:,:,irho)*f(l1-i,:,:,irho)) - 2*lnrho0)
endif
enddo
else
do i=1,nghost
f(l1-i,:,:,iss) = - f(l1+i,:,:,iss) + tmp  - (cp-cv)*(f(l1+i,:,:,ilnrho)+f(l1-i,:,:,ilnrho)-2*lnrho0)
enddo
endif
!
...
if (lreference_state)  f(l2,:,:,iss) = f(l2,:,:,iss) - reference_state(TOP,iref_s)
!
...
f(l2+i,:,:,iss) =-f(l2-i,:,:,iss) + tmp - 2.*reference_state(nx-i,iref_s)  - (cp-cv)*(log((f(l2-i,:,:,irho)+reference_state(TOP-i,iref_rho))  *(f(l2+i,:,:,irho)+reference_state(TOP-i,iref_rho)))-2*lnrho0)
else
f(l2+i,:,:,iss) = -f(l2-i,:,:,iss) + tmp  -(cp-cv)*(log(f(l2-i,:,:,irho)*f(l2+i,:,:,irho))-2*lnrho0)
endif
enddo
else
do i=1,nghost
f(l2+i,:,:,iss) = -f(l2-i,:,:,iss) + tmp  - (cp-cv)*(f(l2-i,:,:,ilnrho)+f(l2+i,:,:,ilnrho)-2*lnrho0)
enddo
endif
elseif (lentropy .and. pretend_lnTT) then
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_ss_temp_y')
!
...
if (lreference_state)  f(l1:l2,m1,:,iss) = f(l1:l2,m1,:,iss) - spread(reference_state(:,iref_s),2,mz)
!

do i=1,nghost
if (ldensity_nolog) then
if (lreference_state) then
f(l1:l2,m1-i,:,iss) =- f(l1:l2,m1+i,:,iss) + tmp  - (cp-cv)*(log((f(l1:l2,m1+i,:,irho)+spread(reference_state(:,iref_rho),2,mz))*  (f(l1:l2,m1-i,:,irho)+spread(reference_state(:,iref_rho),2,mz)))-2*lnrho0)
else
f(:,m1-i,:,iss) =- f(:,m1+i,:,iss) + tmp  - (cp-cv)*(log(f(:,m1+i,:,irho)*f(:,m1-i,:,irho))-2*lnrho0)
endif
else
f(:,m1-i,:,iss) =- f(:,m1+i,:,iss) + tmp  - (cp-cv)*(f(:,m1+i,:,ilnrho)+f(:,m1-i,:,ilnrho)-2*lnrho0)
endif
enddo
!
...
if (lreference_state)  f(l1:l2,m2,:,iss) = f(l1:l2,m2,:,iss) - spread(reference_state(:,iref_s),2,mz)
!

do i=1,nghost
if (ldensity_nolog) then
if (lreference_state) then
f(l1:l2,m2+i,:,iss) =- f(l1:l2,m2-i,:,iss) + tmp  - (cp-cv)*(log((f(l1:l2,m2-i,:,irho)+spread(reference_state(:,iref_rho),2,mz))*  (f(l1:l2,m2+i,:,irho)+spread(reference_state(:,iref_rho),2,mz)))-2*lnrho0)
else
f(:,m2+i,:,iss) = -f(:,m2-i,:,iss) + tmp  - (cp-cv)*(log(f(:,m2-i,:,irho)*f(:,m2+i,:,irho))-2*lnrho0)
endif
else
f(:,m2+i,:,iss) = -f(:,m2-i,:,iss) + tmp  - (cp-cv)*(f(:,m2-i,:,ilnrho)+f(:,m2+i,:,ilnrho)-2*lnrho0)
endif
enddo
!
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_ss_temp_z')
!
...
if (cs2bot<=0.) print*,  'bc_ss_temp_z: cannot have cs2bot = ', cs2bot, ' <= 0'
if (lentropy .and. .not. pretend_lnTT) then
!
...
if (lreference_state)  f(l1:l2,:,n1,iss) = f(l1:l2,:,n1,iss) - spread(reference_state(:,iref_s),2,my)
!
...
do i=1,nghost
f(:,:,n1-i,iss) = -f(:,:,n1+i,iss) + tmp  - 2*(cp-cv)*(lnrho_xy-lnrho0)
enddo
else
do i=1,nghost
f(:,:,n1-i,iss) = -f(:,:,n1+i,iss) + tmp  - (cp-cv)*(f(:,:,n1+i,ilnrho)+f(:,:,n1-i,ilnrho)-2*lnrho0)
enddo
endif
endif
!
...
if (cs2top<=0.) print*,  'bc_ss_temp_z: cannot have cs2top = ', cs2top, ' <= 0'
!DM+PC next two lines need to be looked into.

!AB: This was implemented in revision: 17029 dhruba.mitra, but it works!

if (lread_oldsnap)  cs2top=cs20*exp(gamma*f(l2,m2,n2,iss)/cp+gamma_m1*(f(l2,m2,n2,ilnrho)-lnrho0))
!
...
if (lreference_state)  f(l1:l2,:,n2,iss) = f(l1:l2,:,n2,iss) - spread(reference_state(:,iref_s),2,my)
!
...
if (ldensity_nolog) then
do i=1,nghost
f(:,:,n2+i,iss) = -f(:,:,n2-i,iss) + tmp  - 2*(cp-cv)*(lnrho_xy-lnrho0)
enddo
else
do i=1,nghost
f(:,:,n2+i,iss) = -f(:,:,n2-i,iss) + tmp  - (cp-cv)*(f(:,:,n2-i,ilnrho)+f(:,:,n2+i,ilnrho)-2*lnrho0)
enddo
endif
endif
elseif (lentropy .and. pretend_lnTT) then
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_lnrho_temp_z')
!
...
tmp=-gravz/cs2bot
do i=1,nghost
f(:,:,n1-i,ilnrho)=  f(:,:,n1+i,ilnrho)   + cp1*(f(:,:,n1+i,iss)-f(:,:,n1-i,iss))+dz2_bound(-i)*tmp
enddo
!
...
tmp=gravz/cs2top
do i=1,nghost
f(:,:,n2+i,ilnrho)=  f(:,:,n2-i,ilnrho)  + cp1*(f(:,:,n2-i,iss)-f(:,:,n2+i,iss))+dz2_bound(i)*tmp
enddo
!
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_lnrho_pressure_z')
!
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_ss_temp2_z')
!
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_ss_temp3_z')
!
...
if (lreference_state) then
call getdlnrho_x(f(:,:,:,ilnrho),l1,i,rho_yz,dlnrho)
f(l1-i,:,:,iss) =  f(l1+i,:,:,iss) + dx2_bound(-i)*reference_state(BOT,iref_gs)  + (cp-cv)*dlnrho
else
...
if (ldensity_nolog) then
if (lreference_state) then
call getdlnrho_x(f(:,:,:,ilnrho),l2,i,rho_yz,dlnrho)
f(l2+i,:,:,iss) =  f(l2-i,:,:,iss) - dx2_bound(i)*reference_state(TOP,iref_gs)  - (cp-cv)*dlnrho
else
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_ss_stemp_y')
!
...
do i=1,nghost
call getdlnrho_y(f(:,:,:,ilnrho),m1,i,dlnrho)
...
do i=1,nghost
call getdlnrho_y(f(:,:,:,ilnrho),m2,i,dlnrho)
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_ss_stemp_z')
!
...
do i=1,nghost
call getdlnrho_z(f(:,:,:,ilnrho),n1,i,dlnrho)
...
do i=1,nghost
call getdlnrho_z(f(:,:,:,ilnrho),n2,i,dlnrho)
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_ss_a2stemp_x')
!
...
case ('bot')
if (cs2bot<=0.) print*,  'bc_ss_a2stemp_x: cannot have cs2bot<=0'
do i=1,nghost
...
case ('top')
if (cs2top<=0.) print*,  'bc_ss_a2stemp_x: cannot have cs2top<=0'
do i=1,nghost
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_ss_a2stemp_y')
!
...
case ('bot')
if (cs2bot<=0.) print*,  'bc_ss_a2stemp_y: cannot have cs2bot<=0'
do i=1,nghost
...
case ('top')
if (cs2top<=0.) print*,  'bc_ss_a2stemp_y: cannot have cs2top<=0'
do i=1,nghost
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_ss_a2stemp_z')
!
...
case ('bot')
if (cs2bot<=0.) print*,  'bc_ss_a2stemp_z: cannot have cs2bot<=0'
do i=1,nghost
...
case ('top')
if (cs2top<=0.) print*,  'bc_ss_a2stemp_z: cannot have cs2top<=0'
do i=1,nghost
...
do i=1,nghost
f(:,:,n1-i,iss)=cv*(-gamma_m1*f(:,:,n1-i,ilnrho)-log(cs20) +log(cs2_2d))
enddo
!
...
do i=1,nghost
f(:,:,n2+i,iss)=cv*(-gamma_m1*f(:,:,n2+i,ilnrho)-log(cs20) +log(cs2_2d))
enddo
case default
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_lnrho_cfb_r_iso')
!
...
if (lreference_state)  f(l1-i,:,:,irho)=  f(l1-i,:,:,irho)  + dx2_bound(-i)*reference_state(BOT,iref_grho)
else
...
if (lreference_state)  f(l2+i,:,:,irho)=  f(l2+i,:,:,irho)  - dx2_bound(i)*reference_state(TOP,iref_grho)
else
...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,caller='bc_lnrho_hds_z_iso')
!
...
call fatal_error("bc_lnrho_hds_z_iso",  "This boundary condition for density is "//  "currently only correct for bcz1(iss)='hs'")
endif
if (bcz12(ilnrho,1)/='nil') then
call fatal_error("bc_lnrho_hds_z_iso",  "To avoid a double computation, this boundary condition "//  "should be used only with bcz1(ilnrho)='nil' for density.")
endif
!
...
if (ldensity_nolog) dlnrhodz=dlnrhodz*rho
!
...
call fatal_error("bc_lnrho_hds_z_iso",  "To avoid a double computation, this boundary condition "//  "should be used only with bcz1(irho_b)='nil' for density.")
endif
...
call fatal_error("bc_lnrho_hds_z_iso",  "This boundary condition is at bottom only implemented for ldensity=T or lanelastic=T")
endif
!
...
call fatal_error("bc_lnrho_hds_z_iso",  "This boundary condition for density is "//  "currently only correct for bcz1(iTT)='s'")
endif
call eoscalc(ilnrho_TT,f(l1,m1,n1,ilnrho),f(l1,m1,n1,iTT),  cs2=cs2_point)
else
if (bcz12(ilnTT,1)/='s') then
call fatal_error("bc_lnrho_hds_z_iso",  "This boundary condition for density is "//  "currently only correct for bcz1(ilnTT)='s'")
endif
...
call fatal_error("bc_lnrho_hds_z_iso",  "This boundary condition for density is "// "currently only correct for bcz2(iss)='hs'")
endif
if (bcz12(ilnrho,2)/='nil') then
call fatal_error("bc_lnrho_hds_z_iso",  "To avoid a double computation, this boundary condition "//  "should be used only with bcz2(ilnrho)='nil' for density.")
endif
!
...
call fatal_error("bc_lnrho_hds_z_iso",  "This boundary condition is at top only implemented for ldensity=T")
endif
!
...
call fatal_error("bc_lnrho_hydrostatic_z",  "This boundary condition for density is "// "currently only correct for bcz2(iTT)='s'")
endif
call eoscalc(ilnrho_TT,f(l2,m2,n2,ilnrho),f(l2,m2,n2,iTT),  cs2=cs2_point)
else
if (bcz12(ilnTT,2)/='s') then
call fatal_error("bc_lnrho_hydrostatic_z",  "This boundary condition for density is "// "currently only correct for bcz2(ilnTT)='s'")
endif
