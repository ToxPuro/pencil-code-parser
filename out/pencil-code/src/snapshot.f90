...
if (lroot) then
if (lread_oldsnap_notestfield) print*,'read old snapshot file (but without testfield),iaatest,iaztestpq,mvar,msnap=',  iaatest,iaztestpq,mvar,msnap
if (lread_oldsnap_notestflow) print*,'read old snapshot file (but without testflow),iuutest,iuztestpq,mvar,msnap=',  iuutest,iuztestpq,mvar,msnap
endif
...
if (stat>0) call fatal_error('powersnap',  'Could not allocate memory for b_vec')
!
...
if (jb_spec)  call powerhel(f,'j.b',lfirstcall_powerhel)
if (ja_spec)  call powerhel(f,'j.a',lfirstcall_powerhel)
