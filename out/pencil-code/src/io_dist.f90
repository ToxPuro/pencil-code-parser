...
if ((datadir_snap == '') .or. (index(datadir_snap,'proc0')>0))  datadir_snap = datadir
!
...
if (write_persist_id (label, id)) return
!print*, 'write_persist_real_0D, label, id, value=', label, id, value
...
use Mpicomm, only: start_serialize, end_serialize, mpibcast, mpibcast_real, mpiallreduce_or,  stop_it, mpiallreduce_min, mpiallreduce_max, MPI_COMM_PENCIL,  mpisend_char, mpirecv_char, mpiallreduce_and
...
character(len=*), intent(in) :: file
real, intent(out):: pos
type(scattered_array), pointer :: data
...
allocate (gx(ceiling(real(nxgrid)/downsampl(1))+2*nghost),  gy(ceiling(real(nygrid)/downsampl(2))+2*nghost),  gz(ceiling(real(nzgrid)/downsampl(3))+2*nghost), stat=alloc_err)
else
...
call input_grid(xdb,ydb,zdb,dxdb,dydb,dzdb,Lxdb,Lydb,Lzdb,  dx_1db,dy_1db,dz_1db,dx_tildedb,dy_tildedb,dz_tildedb)
...
call input_grid(xsg,ysg,zsg,dxsg,dysg,dzsg,Lxsg,Lysg,Lzsg,  dx_1sg,dy_1sg,dz_1sg,dx_tildesg,dy_tildesg,dz_tildesg)
...
if (.not.lotherprec)  call input_grid(x,y,z,dx,dy,dz,Lx,Ly,Lz,dx_1,dy_1,dz_1,dx_tilde,dy_tilde,dz_tilde)
close(lun_input)
!

if (lotherprec) call wgrid(file,lwrite=.true.)
!
