...
if (lwrite_slice_r) then
if (present(r))  call interp_rslice(slices%r,rslice=r(:,:,:,:,:,slices%index))
endif
...
if (lwrite_slice_yz)  slices%yz =f(ix_loc,m1:m2  ,n1:n2  ,ind1)
if (lwrite_slice_xz)  slices%xz =f(l1:l2 ,iy_loc ,n1:n2  ,ind1)
if (lwrite_slice_xy)  slices%xy =f(l1:l2 ,m1:m2  ,iz_loc ,ind1)
if (lwrite_slice_xy2) slices%xy2=f(l1:l2 ,m1:m2  ,iz2_loc,ind1)
if (lwrite_slice_xy3) slices%xy3=f(l1:l2 ,m1:m2  ,iz3_loc,ind1)
if (lwrite_slice_xy4) slices%xy4=f(l1:l2 ,m1:m2  ,iz4_loc,ind1)
if (lwrite_slice_xz2) slices%xz2=f(l1:l2 ,iy2_loc,n1:n2  ,ind1)
...
if (lwrite_slice_xz)  slices%xz =f(l1:l2,iy_loc ,n1:n2  ,ind-1+slices%index)
if (lwrite_slice_xy)  slices%xy =f(l1:l2,m1:m2  ,iz_loc ,ind-1+slices%index)
if (lwrite_slice_xy2)  slices%xy2=f(l1:l2,m1:m2  ,iz2_loc,ind-1+slices%index)
if (lwrite_slice_xy3)  slices%xy3=f(l1:l2,m1:m2  ,iz3_loc,ind-1+slices%index)
if (lwrite_slice_xy4)  slices%xy4=f(l1:l2,m1:m2  ,iz4_loc,ind-1+slices%index)
if (lwrite_slice_xz2)  slices%xz2=f(l1:l2,iy2_loc,n1:n2  ,ind-1+slices%index)
...
if (lwrite_slice_r  )  where(rslice_adjec_corn_inds(:,:,1)/=0) slices%r=func(slices%r)
...
dth=pi/nth_rslice; dphi=2*pi/nph_rslice; dth0=.5*dth; dph0=0.
do iph=1,nph_rslice
...
if (z(n1-1)<=zs.and.zs<=z(n2).and.poms>dx**2+dy**2) then
...
if (lwrite_slice_r) then
if (allocated(rslice_adjec_corn_inds))  deallocate(rslice_adjec_corn_inds,rslice_interp_weights)
...
rslice_interp_weights(ith,iph,:,:,:)=  reshape((/ (/dxs*dys,(dx-dxs)*dys,dxs*(dy-dys),(dx-dxs)*(dy-dys)/)*dzs,  (/dxs*dys,(dx-dxs)*dys,dxs*(dy-dys),(dx-dxs)*(dy-dys)/)*(dz-dzs) /),  shape(rslice_interp_weights(ith,iph,:,:,:)))/(dx*dy*dz)
...
mdif=m-mind
if (mdif==-1.or.mdif==0) then
nind=rslice_adjec_corn_inds(ith,iph,3)
ndif=n-nind
if (ndif==-1.or.ndif==0) then
lind=lind-nghost
if (lind==1) then
rslice(ith,iph,-1,mdif,ndif)=2.*pencil(lind)-pencil(lind+1)
rslice(ith,iph, 0,mdif,ndif)=pencil(lind)
else
rslice(ith,iph,:,mdif,ndif)=pencil(lind-1:lind)
endif
nind=nind-nghost; mind=mind-nghost
if (mind==1)  rslice(ith,iph,:,-1,ndif)=rslice(ith,iph,:,mdif,ndif)
if (nind==1)  rslice(ith,iph,:,mdif,-1)=rslice(ith,iph,:,mdif,ndif)
if (nind==1.and.mind==1)  rslice(ith,iph,:,-1,-1)=rslice(ith,iph,:,mdif,ndif)
endif
endif
endif
enddo
