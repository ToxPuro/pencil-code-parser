...
integer :: jdone=0
type(scattered_array), pointer :: slc_dat_xy, slc_dat_xy2, slc_dat_xz, slc_dat_xz2,  slc_dat_yz, slc_dat_yz2
integer :: nt_slices=0
...
if (lcoarse)  call coarsegrid_interp(f,updated_var_ranges(1,i),updated_var_ranges(2,i))
...
logical :: lread_slice_yz,lread_slice_yz2,lread_slice_xz,lread_slice_xz2,  lread_slice_xy,lread_slice_xy2
integer ::  mx_in, my_in, mz_in, mvar_in, maux_in, mglobal_in,  nghost_in, nprocx_in, nprocy_in, nprocz_in, nprocz_in_
...
if (lroot) then
if (.not.directory_exists(trim(bc_slc_dir)//'/data'))  call fatal_error('initialize_boundconds', 'working directory '//trim(bc_slc_dir)//' for slices not found')
call input_dim(bc_slc_dir, mx_in, my_in, mz_in, mvar_in, maux_in, mglobal_in,  prec_in, nghost_in, nprocx_in, nprocy_in, nprocz_in)
call input_dim(bc_slc_dir, mx_in, my_in, mz_in, mvar_in, maux_in, mglobal_in,  prec_in, nghost_in, nprocx_in, nprocy_in, nprocz_in_, local=.true.)
if (mx/=mx_in.or.my/=my_in)  call fatal_error('initialize_boundconds',  'data in working directory '//trim(bc_slc_dir)//'have incompatible x or y dimensions')
endif
call mpibarrier
...
j=1
if (any(bcx(1:nvar)=='p'.or. bcx(1:nvar)=='she').and..not.lperi(j).or. any(bcx(1:nvar)/='p'.and.bcx(1:nvar)/='she').and.lperi(j))  call warning_lperi(lwarning,bcx(1:nvar),lperi,j)
!
...
j=2
if (any(bcy(1:nvar)=='p').and..not.lperi(j).or. any(bcy(1:nvar)/='p').and.lperi(j))  call warning_lperi(lwarning,bcy(1:nvar),lperi,j)
!
...
j=3
if (any(bcz(1:nvar)=='p').and..not.lperi(j).or. any(bcz(1:nvar)/='p').and.lperi(j))  call warning_lperi(lwarning,bcz(1:nvar),lperi,j)
endif
!
...
print*,'[bad BCs in start.in only affects post-processing'  //' of start data, not the run]'
else
...
print*,'bc=',bc
print*,"any(bc=='p'.or. bc=='she'), .not.lperi(j) = ",  any(bc=='p'.or. bc=='she'), .not.lperi(j)
print*, "any(bcx/='p'.and.bcx/='she'), lperi(j) = ",  any(bc=='p'.or. bc=='she'), .not.lperi(j)
endif
!
...
if (abs(pos-pos_slc)>dz)  call fatal_error_local('get_slice_data', 'slices in '//trim(file)//  ' at wrong position')
...
if (nt_slices>1) f(l1:l2,m1:m2,1,j)=f(l1:l2,m1:m2,n1,j)
else
if (nt_slices>1) then
...
if (nt_slices>1) f(l1:l2,m1:m2,mz,j)=f(l1:l2,m1:m2,n2,j)
else
...
if (bcx12(j,1) /= bcx12(j,2))  call fatal_error_local('boundconds_x', 'generalize me to have sheared periodic boundary on only one end.')
...
case ('sfr')
if (j==iux) call fatal_error('boundconds_x',  'stress-free BC at r boundary not allowed for uu_r')
...
if (.not.bc%done) then
write(unit=errormsg,fmt='(A,A4,A,I3)')  "No such boundary condition bcx1/2 = ",  bcx12(j,k), " for j=", j
...
case ('sfr')
if (j==iux.or.j==iuy) call fatal_error('boundconds_y',  'stress-free BC at theta boundary only allowed for uu_phi')
...
if (.not.bc%done) then
write(unit=errormsg,fmt='(A,A4,A,I3)') "No such boundary condition bcy1/2 = ",  bcy12(j,k), " for j=", j
...
case ('hs')
if (.not. lgrav) call fatal_error('boundconds_z',  'hs boundary condition requires gravity')
if ((.not. ltemperature .or. ltemperature_nolog) .and. (gravz_profile /= 'const'))  call fatal_error('boundconds_z', 'hs boundary condition requires a constant gravity profile')
if (.not. lequidist(3)) call fatal_error('boundconds_z',  'hs boundary condition requires symmetric grid distances on the z boundary')
...
if ((ilnrho == 0) .or. (ilnTT == 0))  call fatal_error ('boundconds_z', "'hse' requires lnrho and lnTT")
if (j == ilnTT) then
...
if (.not.bc%done) then
write(unit=errormsg,fmt='(A,A4,A,I3)') "No such boundary condition bcz1/2 = ",  bcz12(j,k), " for j=", j
...
if (.not.lyinyang)  call fatal_error_local('bc_yy_y','BC not legal as no Yin-Yang grid run.')
if (j<=jdone) return
if (is_vec) then
!
...
jdone=j+2
if (topbot=='bot') then
call transform_cart_spher(f,1,nghost,1,mz,j)
else
...
if (.not.lpole(2)) call fatal_error_local('bc_pper_y', "for 'p' lpole=F,T,F , lperi=F,F,T in start.in")
if (nprocz>1 .and. modulo(nprocz,2)==1)  call fatal_error_local('bc_pper_y', "for 'pp' nprocz must be multiple of 2")
!
...
if (.not.lyinyang)  call fatal_error_local('bc_yy_z','BC not legal as no Yin-Yang grid run.')
if (j<=jdone) return
if (is_vec) then
!
...
if (topbot=='bot') then
call transform_cart_spher(f,iya,iye,1,nghost,j)
else
...
f(l1-1,:,:,j)=(f(l1,:,:,j)*(-2*tmp1-2/x(l1)**2)  +f(l1+1,:,:,j)*(tmp1+tmp2))  /(-1*tmp1+tmp2)
f(l1-2,:,:,j)=(f(l1-1,:,:,j)*16*(tmp1-tmp2)  +f(l1,:,:,j)*(-30*tmp1-24/x(l1)**2)  +f(l1+1,:,:,j)*16*(tmp1+tmp2)  +f(l1+2,:,:,j)*(-1*tmp1-2*tmp2))  /(tmp1-2*tmp2)
f(l1-3,:,:,j)=(f(l1-2,:,:,j)*27*(-1*tmp1+2*tmp2)  +f(l1-1,:,:,j)*270*(tmp1-tmp2)  +f(l1,:,:,j)*(-490*tmp1-360/x(l1)**2)  +f(l1+1,:,:,j)*270*(tmp1+tmp2)  +f(l1+2,:,:,j)*27*(-1*tmp1-2*tmp2)  +f(l1+3,:,:,j)*(2*tmp1+6*tmp2))  /(-2*tmp1+6*tmp2)
!
...
f(l2+1,:,:,j)=(f(l2,:,:,j)*(-2*tmp1-2/x(l2)**2)  +f(l2-1,:,:,j)*(tmp1-tmp2))  /(-1*tmp1-tmp2)
f(l2+2,:,:,j)=(f(l2+1,:,:,j)*16*(tmp1+tmp2)  +f(l2,:,:,j)*(-30*tmp1-24/x(l2)**2)  +f(l2-1,:,:,j)*16*(tmp1-tmp2)  +f(l2-2,:,:,j)*(-1*tmp1+2*tmp2))  /(tmp1+2*tmp2)
f(l2+3,:,:,j)=(f(l2+2,:,:,j)*27*(-1*tmp1-2*tmp2)  +f(l2+1,:,:,j)*270*(tmp1+tmp2)  +f(l2,:,:,j)*(-490*tmp1-360/x(l2)**2)  +f(l2-1,:,:,j)*270*(tmp1-tmp2)  +f(l2-2,:,:,j)*27*(-1*tmp1+2*tmp2)  +f(l2-3,:,:,j)*(2*tmp1-6*tmp2))  /(-2*tmp1-6*tmp2)
case default
...
if (.not.lspherical_coords)  call fatal_error('bc_spr_x','only implemented for spherical coordinates')
!
...
case ('bot')
f(l1,m1:m2,n1:n2,j)=(360.*f(l1+i1,m1:m2,n1:n2,j)  -450.*f(l1+i2,m1:m2,n1:n2,j)  +400.*f(l1+i3,m1:m2,n1:n2,j)  -225.*f(l1+i4,m1:m2,n1:n2,j)  +72.*f(l1+i5,m1:m2,n1:n2,j)  -10.*f(l1+i6,m1:m2,n1:n2,j))/147.
...
case ('top')
f(l2,m1:m2,n1:n2,j)=(360.*f(l2-i1,m1:m2,n1:n2,j)  -450.*f(l2-i2,m1:m2,n1:n2,j)  +400.*f(l2-i3,m1:m2,n1:n2,j)  -225.*f(l2-i4,m1:m2,n1:n2,j)  +72.*f(l2-i5,m1:m2,n1:n2,j)  -10.*f(l2-i6,m1:m2,n1:n2,j))/147.
...
f(l1,:,:,j)=(360.*x(l1+1)*f(l1+1,:,:,j)-450.*x(l1+2)*f(l1+2,:,:,j)  +400.*x(l1+3)*f(l1+3,:,:,j)-225.*x(l1_4)*f(l1_4,:,:,j)  +72.*x(l1_5)*f(l1_5,:,:,j)- 10.*x(l1_6)*f(l1_6,:,:,j)  )/(147.*x(l1))
do i=1,nghost
...
f(l2,:,:,j)=(360.*x(l2-1)*f(l2-1,:,:,j)-450.*x(l2-2)*f(l2-2,:,:,j)  +400.*x(l2-3)*f(l2-3,:,:,j)-225.*x(l2_4)*f(l2_4,:,:,j)  +72.*x(l2_5)*f(l2_5,:,:,j)- 10.*x(l2_6)*f(l2_6,:,:,j)  )/(147.*x(l2))
do i=1,nghost
...
if (.not.(j==irho.or.j==ilnrho))  call fatal_error("bc_stratified_y","This boundary condition is specific for density")
...
case ('bot')
f(:,m1,:,j)=(360.*f(:,m1+i1,:,j)  -450.*f(:,m1+i2,:,j)  +400.*f(:,m1+i3,:,j)  -225.*f(:,m1+i4,:,j)  +72.*f(:,m1+i5,:,j)  -10.*f(:,m1+i6,:,j))/147.
...
case ('top')
f(:,m2,:,j)=(360.*f(:,m2-i1,:,j)  -450.*f(:,m2-i2,:,j)  +400.*f(:,m2-i3,:,j)  -225.*f(:,m2-i4,:,j)  +72.*f(:,m2-i5,:,j)  -10.*f(:,m2-i6,:,j))/147.
...
f(:,m1-2,:,j)=(f(:,m1-1,:,j)*8*(2-tmp)  +f(:,m1+1,:,j)*8*(2+tmp)  +f(:,m1+2,:,j)*(-1-tmp))/(1-tmp)
f(:,m1-3,:,j)=(f(:,m1-2,:,j)*13.5*(1-tmp)  +f(:,m1-1,:,j)*135*(-1+0.5*tmp)  +f(:,m1+1,:,j)*135*(-1-0.5*tmp)  +f(:,m1+2,:,j)*13.5*(1+tmp)  +f(:,m1+3,:,j)*(-1-1.5*tmp))/(1-1.5*tmp)
!
...
f(:,m2+2,:,j)=(f(:,m2+1,:,j)*8*(2-tmp)  +f(:,m2+1,:,j)*8*(2+tmp)  +f(:,m2+2,:,j)*(-1-tmp))/(1-tmp)
f(:,m2+3,:,j)=(f(:,m2+2,:,j)*13.5*(1-tmp)  +f(:,m2+1,:,j)*135*(-1+0.5*tmp)  +f(:,m2-1,:,j)*135*(-1-0.5*tmp)  +f(:,m2-2,:,j)*13.5*(1+tmp)  +f(:,m2-3,:,j)*(-1-1.5*tmp))/(1-1.5*tmp)
!
...
if (any(f(4:131,4:131,n1-i,j)/=f(4:131,4:131,n1+i,j)))  print'(a,i2,1x,e20.12)','boundcond ghost:i=', i, maxval(abs(f(4:131,4:131,n1-i,j)-f(4:131,4:131,n1+i,j)))
if (any(f(4:131,4:131,n1-i,j)/=f(4:131,4:131,n1,j)))  print'(a,i2,1x,e20.12)','boundcond bound-:i=', i, maxval(abs(f(4:131,4:131,n1-i,j)-f(4:131,4:131,n1,j)))
if (any(f(4:131,4:131,n1+i,j)/=f(4:131,4:131,n1,j)))  print'(a,i2,1x,e20.12)','boundcond bound+:i=', i, maxval(abs(f(4:131,4:131,n1+i,j)-f(4:131,4:131,n1,j)))
endif
enddo
else
...
case ('bot')
f(:,:,n1,j)=(360.*f(:,:,n1+i1,j)  -450.*f(:,:,n1+i2,j)  +400.*f(:,:,n1+i3,j)  -225.*f(:,:,n1+i4,j)  +72.*f(:,:,n1+i5,j)  -10.*f(:,:,n1+i6,j))/147.
...
case ('top')
f(:,:,n2,j)=(360.*f(:,:,n2-i1,j)  -450.*f(:,:,n2-i2,j)  +400.*f(:,:,n2-i3,j)  -225.*f(:,:,n2-i4,j)  +72.*f(:,:,n2-i5,j)  -10.*f(:,:,n2-i6,j))/147.
...
if (stat>0) call fatal_error('bc_file_x',  'Could not allocate memory for bc_file_x_array')
!
...
do i=1,nghost
f(l1-i,:,:,j)=(1-frac)*bc_file_x_array(lbc1,:,:,j)  +frac*bc_file_x_array(lbc2,:,:,j)
enddo
case ('top')
!
...
lbc2=iszx+mod(+lbc0+1,iszx)
do i=1,nghost
f(l2+i,:,:,j)=(1-frac)*bc_file_x_array(lbc1,:,:,j)  +frac*bc_file_x_array(lbc2,:,:,j)
enddo
case default
call warning('bc_fix_x',topbot//" should be 'top' or 'bot'")
!
...
case ('bot')
do k=1,nghost
f(l1-k,:,:,j)= f(l1,:,:,j)*2.*(x(l1)/x(l1-k)) -f(l1+k,:,:,j)*(x(l1+k)/x(l1-k))
enddo
!

case ('top')
do k=1,nghost
f(l2+k,:,:,j)= f(l2,:,:,j)*2.*(x(l2)/x(l2+k)) -f(l2-k,:,:,j)*(x(l2-k)/x(l2+k))
enddo
!
...
f(l1-k,iy,:,j) = (f(l1+k,iy,:,j)+Omega*x(l1+k)*sth)*fac  -Omega*(x(l1+k)-dx2_bound(-k))*sth
endif
enddo
enddo
else
...
f(l2+k,iy,:,j) = (f(l2-k,iy,:,j)+Omega*x(l2-k)*sth)*fac  -Omega*(x(l2-k)+dx2_bound(k))*sth
endif
enddo
enddo
else
...
yhat_min=ymid-rad/2.
yhat_max=ymid+rad/2
hatprofy= step(y(m1:m2),yhat_min,width_hat)  -step(y(m1:m2),yhat_max,width_hat)
!
...
zhat_min=zmid-zlim/2.
zhat_max=zmid+zlim/2
hatprofz= step(z(n1:n2),zhat_min,width_hat)  -step(z(n1:n2),zhat_max,width_hat)
endif
!
...
if (Omega==0) then
do ix=1,size(f,1)
f(ix,m1-k,:,j)= f(ix,m1+k,:,j)*  (exp(LH1(ix)*cos2thmpk/(4.*nu))*sin1th(m1+k))  *(exp(-LH1(ix)*cos2thm_k/(4.*nu))*sinth(m1-k))
enddo
else
do ix=1,size(f,1)
...
somega=x(ix)*Omega*sinth(m1-k)*(  exp(cos2thmpk*LH1(ix)/(4.*nu)) /exp((cos2thm_k)*LH1(ix)/(4.*nu)) -1.)
f(ix,m1-k,:,j)= f(ix,m1+k,:,j)*  (exp(LH1(ix)*cos2thmpk/(4.*nu))*sin1th(m1+k))  *(exp(-LH1(ix)*cos2thm_k/(4.*nu))*sinth(m1-k))  +somega
enddo
endif
enddo
endif
...
if (Omega==0)then
do ix=1,size(f,1)
f(ix,m2+k,:,j)= f(ix,m2-k,:,j)*  (exp(LH1(ix)*cos2thm_k/(4.*nu))*sin1th(m2-k))  *(exp(-LH1(ix)*cos2thmpk/(4.*nu))*sinth(m2+k))
enddo
else
do ix=1,size(f,1)
...
somega=x(ix)*Omega*sinth(m2+k)*(  exp(cos2thm_k*LH1(ix)/(4.*nu))     / exp(cos2thmpk*LH1(ix)/(4.*nu))-1.)
f(ix,m2+k,:,j)= f(ix,m2-k,:,j)*  (exp(LH1(ix)*cos2thm_k/(4.*nu))*sin1th(m2-k))  *(exp(-LH1(ix)*cos2thmpk/(4.*nu))*sinth(m2+k))  +somega
enddo
endif
enddo
endif
...
duz_dz= fac*(+45.0*(f(l1+1:l2+1,m1:m2,iref,iux)-f(l1-1:l2-1,m1:m2,iref,iux))  - 9.0*(f(l1+2:l2+2,m1:m2,iref,iux)-f(l1-2:l2-2,m1:m2,iref,iux))  +     (f(l1+3:l2+3,m1:m2,iref,iux)-f(l1-3:l2-3,m1:m2,iref,iux)))
else
...
duz_dz=duz_dz + fac*(+45.0*(f(l1:l2,m1+1:m2+1,iref,iuy)-f(l1:l2,m1-1:m2-1,iref,iuy))  - 9.0*(f(l1:l2,m1+2:m2+2,iref,iuy)-f(l1:l2,m1-2:m2-2,iref,iuy))  +     (f(l1:l2,m1+3:m2+3,iref,iuy)-f(l1:l2,m1-3:m2-3,iref,iuy)))
else
...
if (.not.((j==irho).or.(j==ilnrho).or.(j==iss)))  call fatal_error("bc_van3rd_y", "boundary coded only for density and entropy")
!
...
do i=1,nghost
k=l1-i
f(k,:,:,j)=7*f(k+1,:,:,j)  -21*f(k+2,:,:,j)  +35*f(k+3,:,:,j)  -35*f(k+4,:,:,j)  +21*f(k+5,:,:,j)  -7*f(k+6,:,:,j)  +f(k+7,:,:,j)
enddo
!
...
case ('bot')
do i=1,nghost
fder=(-3*f(:,:,n1-i+1,j)+4*f(:,:,n1-i+2,j) -f(:,:,n1-i+3,j))/(2*dz)
f(:,:,n1-i,j)=f(:,:,n1-i+2,j)-2*dz*fder
enddo
case ('top')
do i=1,nghost
fder=(3*f(:,:,n2+i-1,j)-4*f(:,:,n2+i-2,j) +f(:,:,n2+i-3,j))/(2*dz)
...
case ('bot')
do i=1,nghost
fder=(-3*f(l1-i+1,:,:,j)+4*f(l1-i+2,:,:,j) -f(l1-i+3,:,:,j))/(2*dx)
f(l1-i,:,:,j)=f(l1-i+2,:,:,j)-2*dx*fder
enddo
case ('top')
do i=1,nghost
fder=(3*f(l2+i-1,:,:,j)-4*f(l2+i-2,:,:,j) +f(l2+i-3,:,:,j))/(2*dx)
...
case default
if (lroot) print*, "No such value for force_lower_bound: <",  trim(force_lower_bound),">"
call stop_it("")
endselect
...
case default
if (lroot) print*, "No such value for force_upper_bound: <",  trim(force_upper_bound),">"
call stop_it("")
endselect
...
if (lroot .and. .not. file_exists(vel_times_dat))  call fatal_error_local('uu_driver','Could not find file "'//trim(vel_times_dat)//'"')
if (lroot .and. .not. file_exists(vel_field_dat))  call fatal_error_local('uu_driver', 'Could not find file "'//trim(vel_field_dat)//'"')
!
...
if (ierr>0) call fatal_error('uu_driver',  'Could not allocate memory for array please check', .true.)
...
bbx= fac*(+ 45.0*(f(l1:l2,m1+1:m2+1,iref,iaz)-f(l1:l2,m1-1:m2-1,iref,iaz))  -  9.0*(f(l1:l2,m1+2:m2+2,iref,iaz)-f(l1:l2,m1-2:m2-2,iref,iaz))  +      (f(l1:l2,m1+3:m2+3,iref,iaz)-f(l1:l2,m1-3:m2-3,iref,iaz)))
else
...
bbx= bbx -fac*(+ 45.0*(f(l1:l2,m1:m2,iref+1,iay)-f(l1:l2,m1:m2,iref-1,iay))  -  9.0*(f(l1:l2,m1:m2,iref+2,iay)-f(l1:l2,m1:m2,iref-2,iay))  +      (f(l1:l2,m1:m2,iref+3,iay)-f(l1:l2,m1:m2,iref-2,iay)))
else
...
if (nzgrid/=1) then
fac=(1./60)*spread(spread(dz_1(iref),1,nx),2,ny)
bby= fac*(+ 45.0*(f(l1:l2,m1:m2,iref+1,iax)-f(l1:l2,m1:m2,iref-1,iax))  -  9.0*(f(l1:l2,m1:m2,iref+2,iax)-f(l1:l2,m1:m2,iref-2,iax))  +      (f(l1:l2,m1:m2,iref+3,iax)-f(l1:l2,m1:m2,iref-3,iax)))
else
if (ip<=5) print*, 'uu_driver: Degenerate case in z-direction'
bby=0.
endif
...
bby=bby-fac*(+45.0*(f(l1+1:l2+1,m1:m2,iref,iaz)-f(l1-1:l2-1,m1:m2,iref,iaz))  -  9.0*(f(l1+2:l2+2,m1:m2,iref,iaz)-f(l1-2:l2-2,m1:m2,iref,iaz))  +      (f(l1+3:l2+3,m1:m2,iref,iaz)-f(l1-3:l2-3,m1:m2,iref,iaz)))
else
...
if (nxgrid/=1) then
fac=(1./60)*spread(dx_1(l1:l2),2,ny)
bbz= fac*(+ 45.0*(f(l1+1:l2+1,m1:m2,iref,iay)-f(l1-1:l2-1,m1:m2,iref,iay))  -  9.0*(f(l1+2:l2+2,m1:m2,iref,iay)-f(l1-2:l2-2,m1:m2,iref,iay))  +      (f(l1+3:l2+3,m1:m2,iref,iay)-f(l1-3:l2-3,m1:m2,iref,iay)))
else
if (ip<=5) print*, 'uu_driver: Degenerate case in x-direction'
bbz=0.
endif
if (nygrid/=1) then
fac=(1./60)*spread(dy_1(m1:m2),1,nx)
bbz=bbz-fac*(+45.0*(f(l1:l2,m1+1:m2+1,iref,iax)-f(l1:l2,m1-1:m2-1,iref,iax))  -  9.0*(f(l1:l2,m1+2:m2+2,iref,iax)-f(l1:l2,m1-2:m2-2,iref,iax))  +      (f(l1:l2,m1+3:m2+3,iref,iax)-f(l1:l2,m1-3:m2-3,iref,iax)))
else
if (ip<=5) print*, 'uu_driver: Degenerate case in y-direction'
endif
!-----------------------------------------------------------------------
...
pp=gamma*(f(l1:l2,m1:m2,iref,iss)+  f(l1:l2,m1:m2,iref,ilnrho))-gamma_m1*lnrho0
...
use Mpicomm, only : mpisend_real, mpirecv_real,  mpisend_logical, mpirecv_logical
!

real, dimension (:,:,:,:) :: f
real, save :: t_l=0., t_r=0., delta_t=0.
integer :: ierr, lend, frame, stat, rec_l, rec_r
integer :: rec_vxl, rec_vxr, rec_vyl, rec_vyr
...
if ((.not. lequidist(1)) .or. (.not. lequidist(2)))  call fatal_error ('bc_force_aa_time', 'not yet implemented for non-equidistant grids', lfirst_proc_xy)
if (mod (nygrid, nprocxy) /= 0)  call fatal_error ('bc_force_aa_time', 'nygrid needs to be an integer multiple of nprocx*nprocy', lfirst_proc_xy)
if (mod (nxgrid, nprocxy) /= 0)  call fatal_error ('bc_force_aa_time', 'nxgrid needs to be an integer multiple of nprocx*nprocy', lfirst_proc_xy)
!
...
open (10,file=mag_times_dat,form='unformatted',status='unknown',  recl=lend,access='direct')
!
...
open (10, file=mag_vel_field_dat, form='unformatted', status='unknown',  recl=lend*nxgrid*nygrid, access='direct')
!
...
open (10,file=mag_field_dat,form='unformatted',status='unknown',  recl=lend*bnx*bny,access='direct')
...
if (stat>0) call fatal_error('bc_ss_flux_x',  'Could not allocate memory for tmp_yz')
!
...
if (stat>0) call fatal_error('bc_ss_flux_x',  'Could not allocate memory for work_yz')
endif
!
...
if (lreference_state)  tmp_yz = tmp_yz + reference_state(nx,iref_gs)
!
...
f(ll-i,:,:,iss)=f(ll+i,:,:,iss)+fac*  ( (f(ll+i,:,:,irho)-f(ll-i,:,:,irho))*coef + dx2_bound(-i)*inh )
else
f(ll-i,:,:,iss)=f(ll+i,:,:,iss)+fac*  (log(f(ll+i,:,:,irho)/f(ll-i,:,:,irho)) + dx2_bound(-i)*inh)
endif
else
f(ll-i,:,:,iss)=f(ll+i,:,:,iss)+fac*  (f(ll+i,:,:,ilnrho)-f(ll-i,:,:,ilnrho) + dx2_bound(-i)*inh)
endif
enddo
!
...
select case (topbot)
case ('bot')
lfrozen_bb_bot(j-iax+1) = .true.
case ('top')
lfrozen_bb_top(j-iax+1) = .true.
case default
...
if (.not. ((lfirst_proc_z .and. (topbot == 'bot')) .or. (llast_proc_z .and. (topbot == 'top'))))  call fatal_error ('bc_aa_pot_field_extrapol', 'Only implemented for topmost or downmost z-layer.', lfirst_proc_xy)
!

if (mod (nx, nprocy) /= 0)  call fatal_error ('bc_aa_pot_field_extrapol', 'nx needs to be an integer multiple of nprocy.', lfirst_proc_xy)
!
...
exp_fact_bot = spread (exp (sqrt (spread (ky_fft(1:bnx), 2, bny) ** 2 +  spread (kx_fft(kx_start+1:kx_start+bny), 1, bnx) ** 2)), 3, nghost)
do pos_z = 1, nghost
delta_z = reduce_factor * (z(n1) - z(n1-nghost+pos_z-1))
exp_fact_bot(:,:,pos_z) = exp_fact_bot(:,:,pos_z) ** delta_z / (nxgrid*nygrid)
enddo
endif
call vect_pot_extrapol_z_parallel  (f(l1:l2,m1:m2,n1,iax:iaz), f(l1:l2,m1:m2,n1-nghost:n1-1,iax:iaz), exp_fact_bot)
case ('top')
...
kx_start = (ipx+ipy*nprocx)*bny
exp_fact_top = spread (exp (sqrt (spread (ky_fft(1:bnx), 2, bny) ** 2 +  spread (kx_fft(kx_start+1:kx_start+bny), 1, bnx) ** 2)), 3, nghost)
do pos_z = 1, nghost
delta_z = z(n2) - z(n2+pos_z)
exp_fact_top(:,:,pos_z) = exp_fact_top(:,:,pos_z) ** delta_z / (nxgrid*nygrid)
enddo
endif
call vect_pot_extrapol_z_parallel  (f(l1:l2,m1:m2,n2,iax:iaz), f(l1:l2,m1:m2,n2+1:n2+nghost,iax:iaz), exp_fact_top)
case default
...
if (mod(nxgrid,nygrid)/=0)  call fatal_error("bc_aa_pot", "pot-field doesn't work "// "with mod(nxgrid,nygrid)/=0", lfirst_proc_xy)
do j=0,1
...
real, dimension (l2-l1+1,m2-m1+1) :: fac,kk,kkkx,kkky,  f1r,f1i,g1r,g1i,f2r,f2i,f3r,f3i
...
if (.not.(lequidist(1) .and. lequidist(2)))  call fatal_error('bc_wind_z',  'non equidistant grid in x and y not implemented')
!
...
if (headtt) print*,'bc_ADI_flux_z: Fbot, hcondADI, dz=',  Fbot, hcondADI, dz
!
...
if (headtt) print*, 'bc_force_ux_time: ampl_forc, k_forc, '// 'w_forc, x_forc, dx_forc=', ampl_forc, k_forc, w_forc,  x_forc, dx_forc
l1st=.false.
endif
!
...
case ('top')
do m=m1,m2
if (      (y(m)>=xyz0(2) +   Lxyz(2)/4) .and. (y(m)<=xyz0(2) + 3*Lxyz(2)/4)) then
if (j==iux) then
...
case ('bot')
f(:,:,n1,j)=(-18.*f(:,:,n1+1,j)  +9.*f(:,:,n1+2,j)  -2.*f(:,:,n1+3,j))/11.
!
...
case ('top')
f(:,:,n2,j)=(+18.*f(:,:,n2-1,j)  -9.*f(:,:,n2-2,j)  +2.*f(:,:,n2-3,j))/11.
!
...
case ('bot')
do k=1,3
f(:,:,n1-k,j)=f(:,:,n1-k+1,j)+(cp-cv)* (lnrho_(:,:,n1-k+1)-lnrho_(:,:,n1-k))
enddo
!

case ('top')
do k=1,3
f(:,:,n2+k,j)=f(:,:,n2+k-1,j)+(cp-cv)* (lnrho_(:,:,n2+k-1)-lnrho_(:,:,n2+k))
enddo
!
...
if (ldensity_nolog) then
f(:,:,n1-k,j)=f(:,:,n1,j)+(cp-cv) *  (log(f(:,:,n1,j-1))-log(f(:,:,n1-k,j-1))) +  cv*log((z(n1)-z(n1-k))*density_scale+1.)
else
f(:,:,n1-k,j)=f(:,:,n1,j)+(cp-cv)* (f(:,:,n1,j-1)-f(:,:,n1-k,j-1))+ cv*log((z(n1)-z(n1-k))*density_scale+1.)
endif
else
...
if (ldensity_nolog) then
f(:,:,n2+k,j)=f(:,:,n2,j)+(cp-cv)* (log(f(:,:,n2,j-1))-log(f(:,:,n2+k,j-1)))+ cv*log((z(n2+k)-z(n2))*density_scale+1.)
else
f(:,:,n2+k,j)=f(:,:,n2,j)+(cp-cv)* (f(:,:,n2,j-1)-f(:,:,n2+k,j-1))+ cv*log((z(n2+k)-z(n2))*density_scale+1.)
endif
else
call fatal_error('bc_ism','only for irho, ilnrho, iuz or iss')
endif
...
character :: dir
integer :: k,p
real, dimension(0:3,3), parameter :: coefs=reshape(  (/ 4., -6., 4., -1.,  10.,-20.,15., -4.,  20.,-45.,36.,-10./),(/4,3/))
select case (dir)
case ('x')
select case (topbot)
case ('top')
