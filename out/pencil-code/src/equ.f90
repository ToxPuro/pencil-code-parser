...
call rhs_loop(f,df,p,mass_per_proc,early_finalize,df_iuu_pencil,nyz,1)
!$omp parallel
...
mn_loop: do imn=2,nyz-1
!

!$      omp_rank=OMP_get_thread_num()

call rhs_loop(f,df,p,mass_per_proc,early_finalize,df_iuu_pencil,nyz,imn)
enddo mn_loop
!
...
call rhs_loop(f,df,p,mass_per_proc,early_finalize,df_iuu_pencil,nyz,nyz)
...
subroutine rhs_loop(f,df,p,mass_per_proc,early_finalize,df_iuu_pencil,nyz,imn)
use Diagnostics
use Chiral
use Chemistry
use Cosmicray
