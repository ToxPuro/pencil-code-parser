...
integer :: idiag_fviscmz=0
integer :: idiag_fviscsmmz=0
integer :: idiag_epsKmz=0
integer :: idiag_viscforcezmz=0
integer :: idiag_viscforcezupmz=0
integer :: idiag_viscforcezdownmz=0
!
...
integer :: idiag_fviscmx=0
integer :: idiag_numx=0
!
...
integer :: idiag_fviscmxy=0
integer :: idiag_fviscsmmxy=0
integer :: idiag_fviscymxy=0
!
...
lvisc_nu_shock=.true.
if (.not. lshock)  call stop_it('initialize_viscosity: shock viscosity'//  ' but module setting SHOCK=noshock')
case ('nu-shock-profz')
...
if (lroot) print*, 'viscous force(i): '//  'nu_hyper/rho*(del2(del2(del2(u)))+del2(del2(grad(divu))))'
if (.not.lhyperviscosity_strict)  call stop_it('initialize_viscosity: This viscosity type'// ' cannot be used with HYPERVISC_STRICT=nohypervisc_strict')
...
if (lrun) then
if (lroot) then
if ((lvisc_simplified.or.lvisc_rho_nu_const.or.  lvisc_sqrtrho_nu_const.or.lvisc_nu_const.or.  lvisc_nu_tdep.or.lvisc_nu_cspeed.or. lvisc_mu_cspeed).and.nu==0.0)  call warning('initialize_viscosity',  'Viscosity coefficient nu is zero!')
if ((lvisc_rho_nu_const_bulk).and.zeta==0.0)  call warning('initialize_viscosity',  'Viscosity coefficient zeta is zero!')
endif
if (lvisc_hyper2_simplified.and.nu_hyper2==0.0)  call fatal_error('initialize_viscosity',  'Viscosity coefficient nu_hyper2 is zero!')
if ( (lvisc_hyper3_simplified.or.lvisc_hyper3_rho_nu_const.or.  lvisc_hyper3_rho_nu_const_bulk.or.lvisc_hyper3_nu_const.or.  lvisc_hyper3_rho_nu_const_symm.or.  lvisc_hyper3_polar.or. lvisc_hyper3_mu_const_strict .or.  lvisc_hyper3_nu_const_strict .or.  lvisc_hyper3_cmu_const_strt_otf  ).and.  nu_hyper3==0.0 )  call fatal_error('initialize_viscosity',  'Viscosity coefficient nu_hyper3 is zero!')
if (lvisc_hyper3_mesh.and.nu_hyper3_mesh==0.0)  call fatal_error('initialize_viscosity',  'Viscosity coefficient nu_hyper3_mesh is zero!')
if (lvisc_hyper3_mesh_residual.and.nu_hyper3_mesh==0.0)  call fatal_error('initialize_viscosity',  'Viscosity coefficient nu_hyper3_mesh_residual is zero!')
if (lvisc_hyper3_csmesh.and.nu_hyper3_mesh==0.0)  call fatal_error('initialize_viscosity',  'Viscosity coefficient nu_hyper3_mesh is zero!')
if (lvisc_spitzer.and.nu_spitzer==0.0)  call fatal_error('initialize_viscosity',  'Viscosity coefficient nu_spitzer is zero!')
if ( (lvisc_hyper3_rho_nu_const_aniso.or.lvisc_hyper3_nu_const_aniso).and. ((nu_aniso_hyper3(1)==0. .and. nxgrid/=1 ).or.  (nu_aniso_hyper3(2)==0. .and. nygrid/=1 ).or.  (nu_aniso_hyper3(3)==0. .and. nzgrid/=1 )) )  call fatal_error('initialize_viscosity',  'A viscosity coefficient of nu_aniso_hyper3 is zero!')
if ( (lvisc_smag.or.lvisc_smag_simplified.or.lvisc_smag_cross_simplified).and.  C_smag==0.0 )  call fatal_error('initialize_viscosity',  'Viscosity coefficient C_smag is zero!')
if (lvisc_nu_shock .and.nu_shock==0.0)  call fatal_error('initialize_viscosity',  'Viscosity coefficient nu_shock is zero!')
if (lvisc_nu_shock_profz .and.nu_shock==0.0)  call fatal_error('initialize_viscosity',  'Viscosity coefficient nu_shock is zero!')
if (lvisc_nu_shock_profr .and.nu_shock==0.0)  call fatal_error('initialize_viscosity',  'Viscosity coefficient nu_shock is zero!')
...
if (ldynamical_diffusion.and.  .not.(lvisc_hyper3_mesh.or.lvisc_hyper3_mesh_residual.or.lvisc_hyper3_csmesh)) then
call fatal_error("initialize_viscosity", "Dynamical diffusion requires mesh hyper-diffusion, switch ivisc='hyper3-mesh' "//  "'hyper3-mesh-residual', or 'hyper3-csmesh'")
endif
if (.not.ldensity) then
if (lvisc_smag.or.lvisc_smag_simplified.or.lvisc_smag_cross_simplified)  call warning('initialize_viscosity',  "ldensity better be .true. for ivisc='smagorinsky*'")
endif
endif
if (lyinyang) then
if (lvisc_nu_prof.or.lvisc_nu_shock_profz.or.lvisc_nut_from_magnetic)  call fatal_error("initialize_viscosity", "z dependent profiles not implemented for Yin-Yang grid")
endif
...
call write_yprof('visc_profy_bound', nu + (nu*(nu_jump-1.))*  (step(y(m1:m2),xyz1(2)-3*dynu, dynu) + step(y(m1:m2),xyz0(2)+3*dynu, -dynu)))
endif
...
if (lvisc_nu_shock_profz)  call write_zprof('visc_shock',  nu_shock+(nu_shock*(nu_jump_shock-1.))*step(z(n1:n2),znu_shock,-widthnu_shock))
!
...
if (lvisc_nu_shock_profr)  call write_zprof('visc_shock',  nu_shock+(nu_shock*(nu_jump_shock-1.))*step(x(l1:l2),xnu_shock,-widthnu_shock))
endif
!
...
call put_shared_variable('lvisc_hyper3_nu_const_strict',lvisc_hyper3_nu_const_strict,  caller='initialize_viscosity')
...
if (.not.lvisc_simplified) call fatal_error("initialize_viscosity",  "Boussinesq works only if ivisc='simplified' in run.in")
...
xmask_vis = xmask_vis * (xyz1(1)**3 - xyz0(1)**3)  / (vis_xaver_range(2)**3 - vis_xaver_range(1)**3)
elseif (lcylindrical_coords) then
xmask_vis = xmask_vis * (xyz1(1)**2 - xyz0(1)**2)  / (vis_xaver_range(2)**2 - vis_xaver_range(1)**2)
else
...
if ((Lambda_V0==0).and.(Lambda_V1==0).and.(Lambda_H1==0))  call warning('initialize_lambda',  'You have chose llambda_effect=T but, all Lambda coefficients to be zero!')
if ((Lambda_V0==0).and.((Lambda_V1/=0).or.(Lambda_H1==0)))  call warning('initialize_lambda',  'Lambda effect: V_zero=0 but V1 or H1 nonzero')
!
...
case ('top_hat')
if (lroot) print*,'lambda profile top_hat, rzero_lambda, rmax_lambda,roffset_lambda, wlambda:', rzero_lambda,rmax_lambda,roffset_lambda,wlambda
...
der_LV0_rprof=der_step(x,rzero_lambda+roffset_lambda,wlambda)  -der_step(x,rmax_lambda+roffset_lambda,wlambda)
...
case ('V0jump')
if (lroot) print*,'lambda_profile, radial_step, rzero_lambda, wlambda:', lambda_profile,rzero_lambda,wlambda
...
if ((lentropy.or.ltemperature) .and.  (lvisc_rho_nu_const .or. lvisc_rho_nu_const_bulk .or.  lvisc_sqrtrho_nu_const .or. lvisc_nu_cspeed .or. lvisc_nu_const .or. lvisc_nu_tdep .or. lvisc_nu_shock .or.  lvisc_nu_prof .or. lvisc_nu_profx .or. lvisc_spitzer .or.  lvisc_nu_profr .or. lvisc_nu_profr_powerlaw .or. lvisc_nu_profy_bound .or.  lvisc_nu_profr_twosteps .or. lvisc_nu_shock_profz .or.  lvisc_nut_from_magnetic .or. lvisc_nu_shock_profr .or. lvisc_mu_cspeed)) lpenc_requested(i_TT1)=.true.
if (lvisc_rho_nu_const .or. lvisc_rho_nu_const_bulk .or.  lvisc_sqrtrho_nu_const .or.  lvisc_nu_const .or. lvisc_nu_tdep .or. lvisc_nu_cspeed .or.  lvisc_nu_prof.or.lvisc_nu_profx.or.lvisc_spitzer .or.  lvisc_nu_profr.or.lvisc_nu_profr_powerlaw .or.  lvisc_nu_profr_twosteps .or. lvisc_nu_profy_bound .or.  lvisc_nut_from_magnetic.or.lvisc_mu_cspeed.or.  (lvisc_simplified.and.lboussinesq) ) then
if ((lenergy.and.lviscosity_heat) .or.  idiag_epsK/=0 .or. idiag_epsKint/=0 .or. idiag_epsK_LES/=0 .or.  idiag_epsKmz/=0 .or.  idiag_fviscmz/=0.or.idiag_fviscsmmz/=0.or.idiag_fviscmx/=0)  lpenc_requested(i_sij2)=.true.
lpenc_requested(i_graddivu)=.true.
endif
if (lthermal_energy.and.lviscosity_heat) lpenc_requested(i_rho)=.true.
if ((lentropy.or.ltemperature).and.  (lvisc_nu_cspeed.or.lvisc_mu_cspeed.or.lvisc_spitzer))  lpenc_requested(i_lnTT)=.true.
if (lvisc_smag .or.lvisc_smag_simplified .or. lvisc_smag_cross_simplified)  lpenc_requested(i_graddivu)=.true.
...
if (lvisc_rho_nu_const .or.  lvisc_sqrtrho_nu_const .or. lvisc_nu_const .or. lvisc_nu_tdep .or.  lvisc_smag .or. lvisc_smag_simplified .or. lvisc_smag_cross_simplified .or.  lvisc_nu_prof .or. lvisc_nu_profx .or. lvisc_spitzer .or.  lvisc_nu_profr_powerlaw .or. lvisc_nu_profr .or.  lvisc_nu_profr_twosteps .or. lvisc_nu_profy_bound .or.  lvisc_nut_from_magnetic .or. lvisc_nu_cspeed .or. lvisc_mu_cspeed)  lpenc_requested(i_del2u)=.true.
if (.not. limplicit_viscosity .and. lvisc_simplified) lpenc_requested(i_del2u)=.true.
if (lvisc_hyper3_simplified .or. lvisc_hyper3_simplified_tdep .or.  lvisc_hyper3_rho_nu_const .or.  lvisc_hyper3_nu_const .or. lvisc_hyper3_rho_nu_const_symm )  lpenc_requested(i_del6u)=.true.
...
if (lvisc_nu_const .or. lvisc_nu_tdep .or.  lvisc_nu_prof .or. lvisc_nu_profx .or. lvisc_nu_profy_bound .or.  lvisc_smag .or. lvisc_smag_simplified .or. lvisc_smag_cross_simplified .or.  lvisc_nu_profr_powerlaw .or. lvisc_nu_profr .or.  lvisc_nu_profr_twosteps .or. lvisc_sqrtrho_nu_const .or.  lvisc_nut_from_magnetic.or.lvisc_nu_cspeed)  lpenc_requested(i_sglnrho)=.true.
!

if (lvisc_smag_Ma) lpenc_requested(i_Ma2)=.true.
!

if (lvisc_spitzer .or. lvisc_mu_cspeed .or. lvisc_nu_cspeed)  lpenc_requested(i_sglnTT)=.true.
!
...
if (idiag_epsK/=0 .or. idiag_epsKint/=0 .or. idiag_epsK_LES/=0 .or.  idiag_epsKmz/=0 .or.  idiag_viscforcezmz/=0.or.idiag_viscforcezupmz/=0.or.  idiag_viscforcezdownmz/=0) then
...
if ((idiag_meshRemax/=0.or.idiag_dtnu/=0).and.(lvisc_nu_shock  .or.lvisc_nu_shock_profz.or.lvisc_nu_shock_profr)) then
lpenc_diagnos(i_shock)=.true.
endif
...
do jj=1,3
do kk=1,3; do ll=1,3
deljskl2 (:,jj) = deljskl2 (:,jj) +  p%uijk(:,kk,ll,jj)*p%sij(:,ll,kk) + p%uijk(:,ll,kk,jj)*p%sij(:,kk,ll)
enddo;enddo
enddo
do ii=1,3
...
if (ldensity) then
do i=1,3
p%fvisc(:,i) = p%fvisc(:,i) + 2*muTT*p%sglnrho(:,i) +muTT*(p%del2u(:,i) + 1./3.*p%graddivu(:,i) +2*nu_cspeed*p%sglnTT(:,i))
enddo
else
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity', 'viscous heating term '//  'is not implemented for lvisc_hyper2_simplified')
endif
endif
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity', 'viscous heating term '//  'is not implemented for lvisc_hyper3_simplified')
endif
endif
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity', 'viscous heating term '// 'is not implemented for lvisc_hyper3_polar')
endif
endif
if (lfirst .and. ldt)  p%diffus_total3=p%diffus_total3+nu_hyper3*pi4_1*dxmin_pencil**4
endif
!
...
enddo
if (lpencil(i_visc_heat)) then
if (headtt)  call warning('calc_pencils_viscosity', 'viscous heating term '// 'is not implemented for lvisc_hyper3_mesh')
endif
if (lfirst .and. ldt) then
if (ldynamical_diffusion) then
...
enddo
if (lpencil(i_visc_heat)) then
if (headtt)  call warning('calc_pencils_viscosity', 'viscous heating term '// 'is not implemented for lvisc_hyper3_csmesh')
endif
if (lfirst .and. ldt) then
if (ldynamical_diffusion) then
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity', 'viscous heating term '//  'is not implemented for lvisc_hyper3_rho_nu_const')
endif
endif
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity', 'viscous heating term '//  'is not implemented for lvisc_hyper3_mu_const_strict')
endif
endif
if (lfirst .and. ldt) p%diffus_total3=p%diffus_total3+nu_hyper3
endif
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity', 'viscous heating term '//  'is not implemented for lvisc_hyper3_mu_const_strict_otf')
endif
endif
if (lfirst .and. ldt) p%diffus_total3=p%diffus_total3+nu_hyper3
endif
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity', 'viscous heating term '//  'is not implemented for lvisc_hyper3_nu_const_strict')
endif
endif
if (lfirst .and. ldt) p%diffus_total3=p%diffus_total3+nu_hyper3
endif
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity', 'viscous heating term '//  'is not implemented for lvisc_hyper3_rho_nu_const_aniso')
endif
endif
!

if (lfirst .and. ldt) p%diffus_total3=p%diffus_total3+ (nu_aniso_hyper3(1)*dline_1(:,1)**6 +  nu_aniso_hyper3(2)*dline_1(:,2)**6 +  nu_aniso_hyper3(3)*dline_1(:,3)**6)/ dxyz_6
!
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity', 'viscous heating term '//  'is not implemented for lvisc_hyper3_nu_const_aniso')
endif
endif
!
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity', 'viscous heating term '//  'is not implemented for lvisc_hyper3_rho_nu_const_bulk')
endif
endif
if (lfirst .and. ldt) p%diffus_total3=p%diffus_total3+murho1
endif
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity', 'viscous heating term '//  'is not implemented for lvisc_hyper3_nu_const')
endif
endif
if (lfirst .and. ldt) p%diffus_total3=p%diffus_total3+nu_hyper3
endif
...
if (lpencil(i_visc_heat)) then
if (headtt) then
call warning('calc_pencils_viscosity','viscous heating term '// 'is not implemented for lvisc_smag_cross_simplified')
endif
endif
if (lfirst .and. ldt) p%diffus_total=p%diffus_total+p%nu_smag
endif
...
do i=1,3
call calc_slope_diff_flux(f,iux+(i-1),p,h_sld_visc,nlf_sld_visc,tmp2(:,i),div_sld_visc,  HEAT=tmp4,HEAT_TYPE='viscose',  FLUX1=d_sld_flux(:,1,i),FLUX2=d_sld_flux(:,2,i),FLUX3=d_sld_flux(:,3,i))
...
do i=1,3
call calc_slope_diff_flux(f,iux+(i-1),p,h_sld_visc,nlf_sld_visc,tmp3,div_sld_visc, HEAT=tmp4,HEAT_TYPE='viscose')
...
if (lcylindrical_coords .or. lspherical_coords) then
do i=1,3
call calc_slope_diff_flux(f,iux+(i-1),p,h_sld_visc,nlf_sld_visc,tmp2(:,i),div_sld_visc,  FLUX1=d_sld_flux(:,1,i),FLUX2=d_sld_flux(:,2,i),FLUX3=d_sld_flux(:,3,i))
enddo
if (lcylindrical_coords) then
p%fvisc(:,1)=p%fvisc(:,1)+tmp2(:,1)-(d_sld_flux(:,2,2))/x(l1:l2)
p%fvisc(:,2)=p%fvisc(:,2)+tmp2(:,2)+(d_sld_flux(:,2,1))/x(l1:l2)
...
use Sub, only: div, calc_all_diff_fluxes, grad, dot_mn, calc_sij2,  stagger_to_base_interp_1st, stagger_to_base_interp_3rd
...
case('carreau')
nu_effective= nu_infinity+  (nu0-nu_infinity)/(1+(non_newton_lambda**2)*gdotsqr)**((1.-carreau_exponent)/2.)
gradnu_effective= (non_newton_lambda**2)*0.5*(carreau_exponent-1.)*  (nu0-nu_infinity)/(1+(non_newton_lambda**2)*gdotsqr)**(1.5-carreau_exponent/2.)
case('step')
...
df(l1:l2,m,n,ilnTT) = df(l1:l2,m,n,ilnTT) +  p%cv1*p%TT1*p%visc_heat*cubic_step(z(n),no_visc_heat_z0,no_visc_heat_zwidth)
else
...
use Diagnostics, only: sum_mn_name, max_mn_name, xysum_mn_name_z,  yzsum_mn_name_x, zsum_mn_name_xy, max_mn_name, phisum_mn_name_rz,  integrate_mn_name
...
if (ldiagnos.and.idiag_dtnu/=0)  call max_mn_name(diffus_nu/cdtv,idiag_dtnu,l_dt=.true.)
if (ldiagnos.and.idiag_dtnu3/=0)  call max_mn_name(diffus_nu3/cdtv3,idiag_dtnu3,l_dt=.true.)
endif
!
...
if (ldiagnos) then
if (idiag_fviscm/=0 .or. idiag_fviscmax/=0 .or. idiag_fviscrmsx/=0)  call dot2(p%fvisc,fvisc2)
if (idiag_fviscm/=0) call sum_mn_name(fvisc2,idiag_fviscm,lsqrt=.true.)
if (idiag_ufviscm/=0)   call sum_mn_name(p%uu(:,1)*p%fvisc(:,1)+  p%uu(:,2)*p%fvisc(:,2)+  p%uu(:,3)*p%fvisc(:,3),idiag_ufviscm)
...
if (l1davgfirst) then
if (idiag_fviscmz/=0)  call xysum_mn_name_z(-2.*p%rho*nu*(  p%uu(:,1)*p%sij(:,1,3)+  p%uu(:,2)*p%sij(:,2,3)+  p%uu(:,3)*p%sij(:,3,3)),idiag_fviscmz)
if (idiag_fviscsmmz/=0)  call xysum_mn_name_z(-2.*p%rho*p%nu_smag*(  p%uu(:,1)*p%sij(:,1,3)+  p%uu(:,2)*p%sij(:,2,3)+  p%uu(:,3)*p%sij(:,3,3)),idiag_fviscsmmz)
if (idiag_epsKmz/=0)  call xysum_mn_name_z(p%visc_heat*p%rho,idiag_epsKmz)
if (idiag_fviscmx/=0)  call yzsum_mn_name_x(-2.*p%rho*nu*(  p%uu(:,1)*p%sij(:,1,1)+  p%uu(:,2)*p%sij(:,2,1)+  p%uu(:,3)*p%sij(:,3,1)),idiag_fviscmx)
if (idiag_numx/=0)  call yzsum_mn_name_x(p%nu,idiag_numx)
...
if (lvisc_sqrtrho_nu_const) then
call zsum_mn_name_xy(-2.*sqrt(p%rho)*nu*(  p%uu(:,1)*p%sij(:,1,1)+p%uu(:,2)*p%sij(:,2,1)+  p%uu(:,3)*p%sij(:,3,1)),idiag_fviscmxy)
elseif (lvisc_rho_nu_const) then
call zsum_mn_name_xy(-2.*nu*(  p%uu(:,1)*p%sij(:,1,1)+p%uu(:,2)*p%sij(:,2,1)+  p%uu(:,3)*p%sij(:,3,1)),idiag_fviscmxy)
elseif (lvisc_nu_profy_bound) then
call zsum_mn_name_xy(-2.*p%rho*pnu*(  p%uu(:,1)*p%sij(:,1,1)+p%uu(:,2)*p%sij(:,2,1)+  p%uu(:,3)*p%sij(:,3,1)),idiag_fviscmxy)
else
call zsum_mn_name_xy(-2.*p%rho*nu*(  p%uu(:,1)*p%sij(:,1,1)+p%uu(:,2)*p%sij(:,2,1)+  p%uu(:,3)*p%sij(:,3,1)),idiag_fviscmxy)
endif
endif
if (idiag_fviscsmmxy/=0) call zsum_mn_name_xy(-2.*p%rho*p%nu_smag*(  p%uu(:,1)*p%sij(:,1,1)+p%uu(:,2)*p%sij(:,2,1)+  p%uu(:,3)*p%sij(:,3,1)),idiag_fviscsmmxy)
...
call zsum_mn_name_xy(-2.*p%rho*nu*(  p%uu(:,1)*p%sij(:,1,2)+p%uu(:,2)*p%sij(:,2,2)+  p%uu(:,3)*p%sij(:,3,2)),idiag_fviscymxy)
endif
endif
if (idiag_fviscrsphmphi/=0) then
...
fluxv(:,2)=p%uu(:,1)*p%sij(:,1,2)+p%uu(:,2)*p%sij(:,2,2)+p%uu(:,3)*p%sij(:,3,2)
fluxv(:,3)=p%uu(:,1)*p%sij(:,1,3)+p%uu(:,2)*p%sij(:,2,3)+p%uu(:,3)*p%sij(:,3,3)
call phisum_mn_name_rz(-2.*p%rho*nu*(fluxv(:,1)*p%evr(:,1)+  fluxv(:,2)*p%evr(:,2)+fluxv(:,3)*p%evr(:,3)),idiag_fviscrsphmphi)
endif
endif
!
...
call fatal_error('getnu', 'p must be present in call to getnu when nu_pencil is present!')
endif
...
real,dimension(nx) :: div_lambda,lomega,dlomega_dr,dlomega_dtheta,  lver,lhor,dlver_dr,dlhor_dtheta
type (pencil_case) :: p
!
...
div_lambda = lver*(sinth(m)*lomega*p%glnrho(:,1)  +3.*sinth(m)*lomega/x(l1:l2) + sinth(m)*dlomega_dr)  +lomega*sinth(m)*dlver_dr + lhor*(costh(m)*lomega*p%glnrho(:,2)  -sinth(m)*lomega/x(l1:l2) + 2.*cotth(m)*costh(m)*lomega/x(l1:l2)  +costh(m)*dlomega_dtheta) + lomega*costh(m)*dlhor_dtheta
!
