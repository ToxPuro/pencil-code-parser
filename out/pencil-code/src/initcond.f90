...
j=i+1
f(:,:,:,j)=f(:,:,:,j)+ampl*( spread(spread(cos(kx1*x)*exp(-.5*(KKx1*x)**2),2,my),3,mz)* spread(spread(cos(ky1*y)*exp(-.5*(KKy1*y)**2),1,mx),3,mz)* spread(spread(cos(kz1*z)*exp(-.5*(KKz1*z)**2),1,mx),2,my))
endif
!
...
if (lroot) write(*,wave_fmt1) 'sinx_siny_sinz: ampl,kx,ky,kz=',  ampl,kx1,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*(spread(spread(sin(kx1*x),2,my),3,mz) *spread(spread(sin(ky1*y),1,mx),3,mz) *spread(spread(sin(kz1*z),1,mx),2,my))
!
...
if (lroot) write(*,wave_fmt1) 'sinx_siny_cosz: ampl,kx,ky,kz=',  ampl,kx1,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*(spread(spread(sin(kx1*x),2,my),3,mz) *spread(spread(sin(ky1*y),1,mx),3,mz) *spread(spread(cos(kz1*z),1,mx),2,my))
endif
!
...
if (lroot) write(*,wave_fmt1) 'x3_siny_cosz: ampl,ky,kz=',  ampl,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*(spread(spread((x-x1)*(x2-x),2,my),3,mz) *spread(spread(sin(ky1*y),1,mx),3,mz) *spread(spread(cos(kz1*z),1,mx),2,my))
endif
!
...
if (lroot) write(*,wave_fmt1) 'x_siny_cosz: ampl,kx,ky,kz=',  ampl,kx1,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*(spread(spread(   (    x),2,my),3,mz) *spread(spread(sin(ky1*y),1,mx),3,mz) *spread(spread(cos(kz1*z),1,mx),2,my))
endif
!
...
if (lroot) write(*,wave_fmt1) 'x1_siny_cosz: ampl,kx,ky,kz=',  ampl,kx1,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*(spread(spread(   ( 1./x),2,my),3,mz) *spread(spread(sin(ky1*y+phasey1),1,mx),3,mz) *spread(spread(cos(kz1*z),1,mx),2,my))
endif
!
...
if (lroot) write(*,wave_fmt1) 'x1_siny_cosz: ampl,kx,ky,kz=',  ampl,kx1,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*(spread(spread(   ( 1./x),2,my),3,mz) *spread(spread(cos(ky1*y),1,mx),3,mz) *spread(spread(cos(kz1*z),1,mx),2,my))
endif
!
...
if (lroot) write(*,wave_fmt1) 'lnx_cosy_cosz: ampl,kx,ky,kz=',  ampl,kx1,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*(spread(spread(log(    x),2,my),3,mz) *spread(spread(cos(ky1*y),1,mx),3,mz) *spread(spread(cos(kz1*z),1,mx),2,my))
endif
!
...
if (lroot) write(*,wave_fmt1) 'cosx_siny_cosz: ampl,kx,ky,kz=',  ampl,kx1,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*(spread(spread(cos(kx1*x),2,my),3,mz) *spread(spread(sin(ky1*y),1,mx),3,mz) *spread(spread(cos(kz1*z),1,mx),2,my))
endif
!
...
if (lroot) write(*,wave_fmt1) 'sin2x_sin2y_cosz: ampl,kx,ky,kz=', ampl,kx1,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*(spread(spread(sin(kx1*x)**2,2,my),3,mz) +spread(spread(sin(ky1*y)**2,1,mx),3,mz)) *spread(spread(cos(kz1*z),1,mx),2,my)
endif
!
...
if (lroot) write(*,wave_fmt1) 'cos(x-cosz): ampl,kx,kz=',  ampl,kx1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*cos(  spread(spread(kx1*x,2,my),3,mz)-  spread(spread(cos(kz1*z),1,mx),2,my))
endif
!
...
if (lroot) write(*,wave_fmt1) 'cos(y-sinz): ampl,ky,kz=',  ampl,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*cos(  spread(spread(ky1*y,1,mx),3,mz)-  spread(spread(cos(kz1*z),1,mx),2,my))
endif
!
...
if (lroot) write(*,wave_fmt1) 'cosx_cosy_cosz: ampl,kx,ky,kz=',  ampl,kx1,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*(spread(spread(cos(kx1*x),2,my),3,mz) *spread(spread(cos(ky1*y),1,mx),3,mz) *spread(spread(cos(kz1*z),1,mx),2,my))
endif
!
...
j=i; f(:,:,:,j)=f(:,:,:,j)+ampl*spread(spread(cos(kx1*x),2,my),3,mz) *spread(spread(cos(kz1*z),1,mx),2,my)
!
...
j=i; f(:,:,:,j)=f(:,:,:,j)+ampl*spread(spread(cos(ky1*y),1,mx),3,mz) *spread(spread(cos(kz1*z),1,mx),2,my)
!
...
j=i; f(:,:,:,j)=f(:,:,:,j)+ampl*spread(spread(cos(ky1*y),1,mx),3,mz) *spread(spread(sin(kz1*z),1,mx),2,my)
!
...
f(:,:,:,i)=f(:,:,:,i)+ampl*spread(spread(x*(1.-x)*(x-.2),2,my),3,mz) *spread(spread(cos(ky1*y),1,mx),3,mz) *spread(spread(cos(kz1*z),1,mx),2,my)
!
...
if (lroot) write(*,wave_fmt1) 'cosx_cosy_cosz: ampl,kx,ky,kz=',  ampl,kx1,ky1,kz1
f(:,:,:,i)=f(:,:,:,i)+ampl*(spread(spread(cos(kx1*x),2,my),3,mz)  *spread(spread(-cos(ky1*y)*(           ((1./9.)*sin(ky*y)**8)+((8./63.)*    sin(ky*y)**6)+((16./105.)*           sin(ky*y)**4)+((64./315.)*           sin(ky*y)**2)                        + (128./315.)                        ),1,mx),3,mz)                         *spread(spread(cos(kz1*z),1,mx),2,my))
endif
!
...
if (lroot) write(*,wave_fmt1) 'cos2x_cos2y_cos2z: ampl,kx,ky,kz=',  ampl,kx1,ky1,kz1
if (ampl>0.) then
f(:,:,:,i)=f(:,:,:,i)+ampl*tanh(10.*  spread(spread(cos(.5*kx1*x)**2,2,my),3,mz)  *spread(spread(cos(.5*ky1*y)**2,1,mx),3,mz)  *spread(spread(cos(.5*kz1*z)**2,1,mx),2,my))
else
f(:,:,:,i)=f(:,:,:,i)-ampl*(1.-tanh(10.*  spread(spread(cos(.5*kx1*x)**2,2,my),3,mz)  *spread(spread(cos(.5*ky1*y)**2,1,mx),3,mz)  *spread(spread(cos(.5*kz1*z)**2,1,mx),2,my)))
endif
endif
!
...
if (lroot) write(*,wave_fmt1) 'innerbox: ampl,ampl2,width=',  ampl,ampl2,width
f(:,:,:,i)=ampl
...
if (lroot) print*,'innerbox: ll1,ll2,mm1,mm2,nn1,nn2=',  ll1,ll2,mm1,mm2,nn1,nn2
...
om_all(nr+2)=omegao
om_diff=0.
om_diff(1:nr+1)=om_all(1:nr+1)-om_all(2:nr+2)
omx(l1:l2)=omegai
do k=1,nr+1
if (k==1) then
x0=rinner+gap/2.
...
f(:,:,:,i)=f(:,:,:,i)+spread(spread((omx*x),2,my),3,mz)
endif
!
...
f(l,m,n,i) = f(l,m,n,i) + ampl*(  (0.5+0.5*tanh(kx2*(width2-x(l)**2))) *  (0.5+0.5*tanh(ky2*(width2-y(m)**2))) *  (0.5+0.5*tanh(kz2*(width2-z(n)**2))) )
enddo; enddo; enddo
endif
!
...
f(l1:l2,m,n,i)=f(l1:l2,m,n,i)+ampl*phase_factor  *exp(-((x(l1:l2)-posx)**2+(y(m)-posy)**2+(z(n)-posz)**2)*radius21)
!        do l=l1,l2
...
do l=1,mx
do m=1,my
profxy(l,m)=fleft+(fright-fleft)*.25*  (1.+tanh(x(l)/width))*(1.+tanh(y(m)/width))
enddo
enddo
...
profz=.5*(fyright+fyleft)*x  +.5*(fyright-fyleft)*width*alog_cosh_xwidth
...
profx=.5*(fyright+fyleft)*z  +.5*(fyright-fyleft)*width*alog_cosh_zwidth
profy=.5*(fxright+fxleft)*z  +.5*(fxright-fxleft)*width*alog_cosh_zwidth
do il=1,mx
do im=1,my
...
integer :: ix,iy,iz
real, dimension (mx,my,mz,mfarray) :: f
real :: ampl,kz,nfactor,maxAx,maxAy,zmax,zmin,zzm
complex :: omega,Ax,Ay
!
...
k=abs(kx)
if (k==0) print*,'beltrami: k must not be zero!'
cfuncx=sign(sqrt(abs(ampl/k)),kx)* real(cos(k*x+ph)+omgsqr*cos(omg*k*x+ph)+omg*cos(omgsqr*k*x+ph))
sfuncx=sign(sqrt(abs(ampl/k)),kx)* real(sin(k*x+ph)+omgsqr*sin(omg*k*x+ph)+omg*sin(omgsqr*k*x+ph))
if (present(kx2)) sfuncx=sfuncx*sin(kx2*x+ph)
if (ampl==0) then
if (lroot) print*,'beltrami: ampl=0; kx=',k
elseif (ampl>0) then
...
k=abs(ky)
if (k==0) print*,'beltrami: k must not be zero!'
cfuncy=sign(sqrt(abs(ampl/k)),ky)* real(cos(k*y+ph)+omgsqr*cos(omg*k*y+ph)+omg*cos(omgsqr*k*y+ph))
sfuncy=sign(sqrt(abs(ampl/k)),ky)* real(sin(k*y+ph)+omgsqr*sin(omg*k*y+ph)+omg*sin(omgsqr*k*y+ph))
if (present(ky2)) sfuncy=sfuncy*sin(ky2*y+ph)
if (ampl==0) then
if (lroot) print*,'beltrami: ampl=0; ky=',k
elseif (ampl>0) then
...
k=abs(kz)
if (k==0) print*,'beltrami: k must not be zero!'
cfuncz=sign(sqrt(abs(ampl/k)),kz)* real(cos(k*z+ph)-omgsqr*cos(omg*k*z+ph)-omg*cos(omgsqr*k*z+ph))
sfuncz=sign(sqrt(abs(ampl/k)),kz)* real(sin(k*z+ph)-omgsqr*sin(omg*k*z+ph)-omg*sin(omgsqr*k*z+ph))
if (present(kz2)) sfuncz=sfuncz*sin(kz2*z+ph)
if (ampl==0) then
if (lroot) print*,'beltrami: ampl=0; kz=',k
elseif (ampl>0) then
...
j=i
f(:,:,:,j)=f(:,:,:,j)+ampl*spread(spread(sin(kx*x),2,my),3,mz) *spread(spread(cos(ky*y),1,mx),3,mz) *spread(spread(cos(kz*z),1,mx),2,my)
j=i+1
f(:,:,:,j)=f(:,:,:,j)+ampl*spread(spread(cos(kx*x),2,my),3,mz) *spread(spread(sin(ky*y),1,mx),3,mz) *spread(spread(cos(kz*z),1,mx),2,my)
j=i+2
f(:,:,:,j)=f(:,:,:,j)+ampl*spread(spread(cos(kx*x),2,my),3,mz) *spread(spread(cos(ky*y),1,mx),3,mz) *spread(spread(sin(kz*z),1,mx),2,my) *(-1.)*(dimensionality-1)
!
...
j=i
f(:,:,:,j)=f(:,:,:,j)-ampl*kz*spread(spread(sin(kx*x       ),2,my),3,mz) *spread(spread(cos(kz*(z-zbot)),1,mx),2,my)
j=i+2
f(:,:,:,j)=f(:,:,:,j)+ampl*kx*spread(spread(cos(kx*x       ),2,my),3,mz) *spread(spread(sin(kz*(z-zbot)),1,mx),2,my)
!
...
j=i+0; f(:,:,:,j)=f(:,:,:,j)-fac1*spread(spread(cos(k*x),2,my),3,mz) *spread(spread(sin(k*y),1,mx),3,mz)
!

j=i+1; f(:,:,:,j)=f(:,:,:,j)+fac1*spread(spread(sin(k*x),2,my),3,mz) *spread(spread(cos(k*y),1,mx),3,mz)
!

j=i+2; f(:,:,:,j)=f(:,:,:,j)+fac2*spread(spread(cos(k*x),2,my),3,mz) *spread(spread(cos(k*y),1,mx),3,mz)
!
...
j=i+0; f(:,:,:,j)=f(:,:,:,j)-ampl*spread(spread(sin(ky*y),1,mx),3,mz)  *spread(spread(exp(-abs(kz*z)),1,mx),2,my)
j=i+1; f(:,:,:,j)=f(:,:,:,j)+ampl*spread(spread(sin(kx*x),2,my),3,mz)  *spread(spread(exp(-abs(kz*z)),1,mx),2,my)
!
...
j=i+1; f(:,:,:,j)=f(:,:,:,j)+ampl  *spread(spread(exp(-(kx*x)**2),2,my),3,mz)  *spread(spread(exp(-(ky*y)**2),1,mx),3,mz)  *spread(spread(exp(-abs(kz*z)),1,mx),2,my)
j=i+2; f(:,:,:,j)=f(:,:,:,j)+ampl  *spread(spread(exp(-(kx*x)**2)*(-2*x),2,my),3,mz)*kx**2  *spread(spread(exp(-(ky*y)**2),1,mx),3,mz)  *spread(spread(exp(-abs(kz*z)),1,mx),2,my)
!
...
j=i+1; f(:,:,n1,j)=f(:,:,n1,j)+ampl  *spread(exp(-(kx*x)**2),2,my)  *spread(exp(-(ky*y)**2),1,mx)
j=i+2; f(:,:,n1,j)=f(:,:,n1,j)+ampl  *spread(exp(-(kx*x)**2)*(-2*x),2,my)*kx**2  *spread(exp(-(ky*y)**2),1,mx)
endif
!
...
if (lroot) print*, 'sinwave_phase: i, ampl, kx, ky, kz, phase=',  i, ampl, kx, ky, kz, phase
!

do m=m1,m2; do n=n1,n2
f(l1:l2,m,n,i) = f(l1:l2,m,n,i) +  ampl*sin(kx*x(l1:l2)+ky*y(m)+kz*z(n)+phase)
enddo; enddo
!
...
if (lroot) print*, 'coswave_phase: i, ampl, kx, ky, kz, phase=',  i, ampl, kx, ky, kz, phase
!

do m=m1,m2; do n=n1,n2
f(l1:l2,m,n,i) = f(l1:l2,m,n,i) +  ampl*cos(kx*x(l1:l2)+ky*y(m)+kz*z(n)+phase)
enddo; enddo
!
...
if (lroot) then
print '(A,I4,A,I4,A,I4,A)','ERROR: The stratification file '//  'for this run is allowed to contain either',nzgrid,  ' lines (without ghost zones) or more than',mzgrid,  ' lines (with ghost zones). It does contain',n-1,  ' lines though.'
endif
call fatal_error('','')
!
...
if (lroot) then
print '(A,I4,A,I4,A,I4,A)','ERROR: The stratification file '//  'for this run is allowed to contain either',nxgrid,  ' lines (without ghost zones) or more than',mxgrid,  ' lines (with ghost zones). It does contain',n-1,  ' lines though.'
endif
call fatal_error('','')
!
...
if (sigma2<0.) then
if (lroot) print*,  'planet_hc: sigma2<0 not allowed; choose another value of eps_planet'
else
sigma=sqrt(sigma2)
endif
!
...
a_ell = radius/eps
c_ell = radius*delta
if (lroot) print*,'planet_hc: Ellipse axes (b_ell,a_ell,c_ell)=',  b_ell,a_ell,c_ell
...
hz(n)=(hz(n+1)*(1.0-0.5*delS)+  Omega**2*0.5*(z(n)+z(n+1))*dz)/(1.0+0.5*delS)
enddo
!
...
if (lentropy) then
f(l,m,n1:n2,ilnrho)=  (log(gamma_m1*hz(n1:n2)/cs20)-gamma*f(l,m,n1:n2,iss))/gamma_m1
if (lroot)  print*,'planet_hc: planet solution with entropy for gamma=',gamma
else
if (gamma==1.) then
...
if (gamma_m1<0. .and. lroot)  print*,'planet_hc: must have gamma>1 for planet solution'
!
...
if (ip<14)  print*,'planet_hc: iproc,lnrhosum_thisbox=',iproc,lnrhosum_thisbox
!
...
if (lroot .and. ip<14)  print*,'planet_hc: lnrhosum_wholebox=',lnrhosum_wholebox
!
...
hh = 0.5*delta2*Omega**2*(radius2-x(l1:l2)**2-eps2*y(m)**2)  -0.5*Omega**2*z(n)**2 + 0.5*Omega**2*ztop**2 + hh0
!
...
f(l1:l2,m,n,iuy) = cs20/(2*Omega)*exp( gamma*f(l1:l2,m,n,iss) +  (gamma-1)*f(l1:l2,m,n,ilnrho) ) * dlnrhobdx/gamma
!
...
j=i; f(:,:,:,j)=f(:,:,:,j)+ampl* spread(spread(sin(2*x),2,my),3,mz)* spread(spread(sin(3*y),1,mx),3,mz)* spread(spread(cos(1*z),1,mx),2,my)
j=i+1; f(:,:,:,j)=f(:,:,:,j)+ampl* spread(spread(sin(5*x),2,my),3,mz)* spread(spread(sin(1*y),1,mx),3,mz)* spread(spread(cos(2*z),1,mx),2,my)
j=i+2; f(:,:,:,j)=f(:,:,:,j)+ampl* spread(spread(sin(3*x),2,my),3,mz)* spread(spread(sin(4*y),1,mx),3,mz)* spread(spread(cos(2*z),1,mx),2,my)
!
...
integer :: i1,i2
real, dimension (mx,my,mz,mfarray) :: f
real, dimension (nx) :: tmp,tube_radius_sqr
...
tmp = (radius**2/2-0.5*(radius+a)*(r_minus_a*erfunc(r_minus_a/width)+width*exp(-r_minus_a**2/width**2)/sqrt(pi)) +0.25*width**2*erfunc(r_minus_a/width)+0.5*a*(-a*erfunc(-a/width)+width*exp(-a**2/width**2)/sqrt(pi)) -0.25*width**2*erfunc(-a/width))/radius**2
else
tmp = 0
...
tmp=.5*ampl*modulate*radius**2* (1.-exp(-(rhorad/radius)**2))/rhorad
else
...
f(l1:l2,m,n,i+1)=f(l1:l2,m,n,i+1)  +ampl*cos(kx*x(l1:l2))*exp(-abs(kz*z(n)))
enddo; enddo
!
...
do n=1,mz; do m=1,my
f(1:mx,m,n,ivar) = f(1:mx,m,n,ivar) + ((1.5*zoverh**2-powerlr)*x(1:mx)  -zoverh/(hoverr)*z(n)  +(powerlr/2-3*zoverh**2)*x(1:mx)**2  -0.5/(hoverr**2)*z(n)**2  +3.*zoverh/hoverr*z(n)*x(1:mx))
!
...
call gaunoise_vect(ampl,f,i1,i2)
!
...
print*,'powern: with cutoff : k^n*exp(-k^4/k0^4) w/ n=',  initpower,', k0 =',cutoff,' : var  i=',i
endif
endif
enddo
endif
...
if (stat>0) call fatal_error('powern',  'Could not allocate memory for kx')
allocate(ky(nygrid),stat=stat)
if (stat>0) call fatal_error('powern',  'Could not allocate memory for ky')
allocate(kz(nzgrid),stat=stat)
if (stat>0) call fatal_error('powern',  'Could not allocate memory for kz')
!
...
if (kgaussian /= 0.) then
k2mkpeak(ikx,iky,ikz)=(kx(ikx+ipx*nx)-kpeak)**2+(ky(iky+ipy*ny)-kpeak)**2  +(kz(ikz+ipz*nz)-kpeak)**2
endif
enddo
enddo
enddo
...
subroutine power_randomphase_hel(ampl,initpower,initpower2,  cutoff,ncutoff,kpeak,f,i1,i2,relhel,kgaussian,  lskip_projection,lvectorpotential,lscale_tobox,  k1hel, k2hel,lremain_in_fourier,lpower_profile_file,qexp,  lno_noise,nfact0,lfactors0,compk0,llogbranch0,initpower_med0,  kpeak_log0,kbreak0,ldouble0,nfactd0,qirro,time,cs)
!
...
allocate(kx(nxgrid),stat=stat)
if (stat>0) call fatal_error('power_randomphase_hel',  'Could not allocate memory for kx')
allocate(ky(nygrid),stat=stat)
if (stat>0) call fatal_error('power_randomphase_hel',  'Could not allocate memory for ky')
allocate(kz(nzgrid),stat=stat)
if (stat>0) call fatal_error('power_randomphase_hel',  'Could not allocate memory for kz')
!
...
r(ikx,iky,ikz)=(1.-2.*qirro1)*  (kx(ikx+ipx*nx)*u_re(ikx,iky,ikz,1)  +ky(iky+ipy*ny)*u_re(ikx,iky,ikz,2))/k2(ikx,iky,ikz)
...
r(ikx,iky,ikz)=(1.-2.*qirro1)*  (kx(ikx+ipx*nx)*u_im(ikx,iky,ikz,1)  +ky(iky+ipy*ny)*u_im(ikx,iky,ikz,2))/k2(ikx,iky,ikz)
...
r(ikx,iky,ikz)=(1.-2.*qirro1)*  (kx(ikx+ipx*nx)*u_re(ikx,iky,ikz,1)  +ky(iky+ipy*ny)*u_re(ikx,iky,ikz,2)  +kz(ikz+ipz*nz)*u_re(ikx,iky,ikz,3))/k2(ikx,iky,ikz)
!
...
r(ikx,iky,ikz)=(1.-2.*qirro1)*  (kx(ikx+ipx*nx)*u_im(ikx,iky,ikz,1)  +ky(iky+ipy*ny)*u_im(ikx,iky,ikz,2)  +kz(ikz+ipz*nz)*u_im(ikx,iky,ikz,3))/k2(ikx,iky,ikz)
v_im(ikx,iky,ikz,1)=p*u_im(ikx,iky,ikz,1)-kx(ikx+ipx*nx)*r(ikx,iky,ikz)
v_im(ikx,iky,ikz,2)=p*u_im(ikx,iky,ikz,2)-ky(iky+ipy*ny)*r(ikx,iky,ikz)
...
u_re(ikx,iky,ikz,1)=v_re(ikx,iky,ikz,1)  -ky(iky+ipy*ny)*v_im(ikx,iky,ikz,3)*r(ikx,iky,ikz)
u_im(ikx,iky,ikz,1)=v_im(ikx,iky,ikz,1)  +ky(iky+ipy*ny)*v_re(ikx,iky,ikz,3)*r(ikx,iky,ikz)
!
...
u_re(ikx,iky,ikz,2)=v_re(ikx,iky,ikz,2)  +kx(ikx+ipx*nx)*v_im(ikx,iky,ikz,3)*r(ikx,iky,ikz)
u_im(ikx,iky,ikz,2)=v_im(ikx,iky,ikz,2)  -kx(ikx+ipx*nx)*v_re(ikx,iky,ikz,3)*r(ikx,iky,ikz)
!
...
u_re(ikx,iky,ikz,3)=v_re(ikx,iky,ikz,3)  +ky(iky+ipy*ny)*v_im(ikx,iky,ikz,1)*r(ikx,iky,ikz)  -kx(ikx+ipx*nx)*v_im(ikx,iky,ikz,2)*r(ikx,iky,ikz)
u_im(ikx,iky,ikz,3)=v_im(ikx,iky,ikz,3)  -ky(iky+ipy*ny)*v_re(ikx,iky,ikz,1)*r(ikx,iky,ikz)  +kx(ikx+ipx*nx)*v_re(ikx,iky,ikz,2)*r(ikx,iky,ikz)
enddo
enddo
!  In 3-D
...
u_re(ikx,iky,ikz,1)=v_re(ikx,iky,ikz,1)  +kz(ikz+ipz*nz)*v_im(ikx,iky,ikz,2)*r(ikx,iky,ikz)  -ky(iky+ipy*ny)*v_im(ikx,iky,ikz,3)*r(ikx,iky,ikz)
u_im(ikx,iky,ikz,1)=v_im(ikx,iky,ikz,1)  -kz(ikz+ipz*nz)*v_re(ikx,iky,ikz,2)*r(ikx,iky,ikz)  +ky(iky+ipy*ny)*v_re(ikx,iky,ikz,3)*r(ikx,iky,ikz)
!
...
u_re(ikx,iky,ikz,2)=v_re(ikx,iky,ikz,2)  +kx(ikx+ipx*nx)*v_im(ikx,iky,ikz,3)*r(ikx,iky,ikz)  -kz(ikz+ipz*nz)*v_im(ikx,iky,ikz,1)*r(ikx,iky,ikz)
u_im(ikx,iky,ikz,2)=v_im(ikx,iky,ikz,2)  -kx(ikx+ipx*nx)*v_re(ikx,iky,ikz,3)*r(ikx,iky,ikz)  +kz(ikz+ipz*nz)*v_re(ikx,iky,ikz,1)*r(ikx,iky,ikz)
!
...
print*,'power_randomphase_hel: with cutoff : k^n*exp(-k^4/k0^4) w/ n=',  initpower,', k0 =',cutoff,' : var  i=',i
endif
endif
!
...
real, dimension (3,1024) :: kk,RA,RB
...
do i=1,3
f(l1:l2,m,n,jf+i-1)=f(l1:l2,m,n,jf+i-1)  +prof*exk(i)*real(exp(ii*(kdotx+phik)))
enddo
enddo
enddo
enddo
...
f(:,:,j,ilnrho) = (prof_lnrho(i)*(prof_z(i+1) - z(j)) +    prof_lnrho(i+1)*(z(j)-prof_z(i)) ) / (prof_z(i+1)-prof_z(i))
!

tmp =  (prof_lnT(i)*(prof_z(i+1) - z(j)) +    prof_lnT(i+1)*(z(j)-prof_z(i)) ) / (prof_z(i+1)-prof_z(i))
!

if (ltemperature) then
f(:,:,j,ilnTT) = tmp
elseif (lentropy) then
f(:,:,j,iss) = (log(gamma_m1/cs20)+tmp-  gamma_m1*(f(l1,m1,j,ilnrho)-lnrho0))/gamma
endif
exit
endif
enddo
...
tmp =  (prof_lnT(i)*(prof_z(i+1) - ztop) +    prof_lnT(i+1)*(ztop-prof_z(i)) ) / (prof_z(i+1)-prof_z(i))
...
tmp =  (prof_lnT(i)*(prof_z(i+1) - zbot) +    prof_lnT(i+1)*(zbot-prof_z(i)) ) / (prof_z(i+1)-prof_z(i))
...
if (.not.lequidist(1).or..not.lequidist(2)) call fatal_error('mdi_init',  'not yet implemented for non-equidistant grids')
!
...
call stop_it_if_any((iostat>0),'mdi_init: '//  'Could not allocate memory for variables, please check')
!
...
kx=spread(  cshift((/(i-(nxinit+1)/2,i=0,nxinit-1)/),+(nxinit+1)/2)*pi/Lx  ,2,nyinit)
ky=spread(  cshift((/(i-(nyinit+1)/2,i=0,nyinit-1)/),+(nyinit+1)/2)*pi/Ly  ,1,nxinit)
endif
!
...
open (11,file=mag_field_dat,form='unformatted',status='unknown',  recl=lend*nxgrid*nygrid,access='direct')
...
integer, parameter :: bnx=nxgrid, bny=ny/nprocx
integer, parameter :: enx=nygrid, eny=nx/nprocy
...
if (.not. lperi(1) .or. .not. lperi(2)) call fatal_error ('mag_init',  'Currently only implemented for xy-periodic setups!')
if (.not. lequidist(1) .or. .not. lequidist(2)) call fatal_error ('mag_init',  'not yet implemented for non-equidistant grids')
if (mod (nygrid, nprocxy) /= 0) call fatal_error ('mag_init',  'nygrid needs to be an integer multiple of nprocx*nprocy')
if (mod (nxgrid, nprocxy) /= 0) call fatal_error ('mag_init',  'nxgrid needs to be an integer multiple of nprocx*nprocy')
!
...
if (alloc_err > 0) call fatal_error('mag_init',  'Could not allocate memory for Bz', .true.)
allocate (exp_fact(enx,eny,mz), stat=alloc_err)
if (alloc_err > 0) call fatal_error('mag_init',  'Could not allocate memory for exp_fact', .true.)
!

if (lroot) then
inquire (file=mag_field_dat, exist=exists)
call stop_it_if_any(.not. exists,  'mag_init: Magnetogram file not found: "'//trim(mag_field_dat)//'"')
...
if (alloc_err > 0) call fatal_error('mag_Az_init',  'Could not allocate memory for A_global', .true.)
endif
if (lfirst_proc_z) then
allocate (A_local(nx,ny), stat=alloc_err)
if (alloc_err > 0) call fatal_error('mag_Az_init',  'Could not allocate memory for A_local', .true.)
endif
!
...
call stop_it_if_any(.not. exists,  'mag_Az_init: file not found: "'//trim(Az_init_dat)//'"')
inquire (iolength=rec_len) 1.0d0
...
if (alloc_err > 0) call fatal_error('file_init',  'Could not allocate memory for A_global', .true.)
endif
allocate (A_local(nx,ny,nz), stat=alloc_err)
if (alloc_err > 0) call fatal_error('file_init',  'Could not allocate memory for A_local', .true.)
!

if (lroot) then
inquire (file=A_init_dat, exist=exists)
call stop_it_if_any(.not. exists,  'file_init: file not found: "'//trim(A_init_dat)//'"')
inquire (iolength=rec_len) 1.0d0
...
f(l1:l2,m,n,i  )=0.0
if (ldensity_nolog) then
f(l1:l2,m,n,i+1)=ampl*(sumtmp(iproc+1)+ sum(f(l1:l2,m,n1:n,irho)))*dx*dz
else
f(l1:l2,m,n,i+1)=ampl*(sumtmp(iproc+1)+ sum(exp(f(l1:l2,m,n1:n,ilnrho))))*dx*dz
endif
f(l1:l2,m,n,i+1)=-ampl*(sumtmp(iproc+1)+ sum(exp(f(l1:l2,m,n1:n,ilnrho))))*dx*dz
f(l1:l2,m,n,i+2)=0.0
enddo
...
do m=m1,m2
if (ldensity_nolog) then
f(l1:l2,m,n,i)=ampl*(sumtmp(iproc+1)+ sum(f(l1:l2,m,n1:n,irho)))*dx*dz
else
f(l1:l2,m,n,i)=ampl*(sumtmp(iproc+1)+ sum(exp(f(l1:l2,m,n1:n,ilnrho))))*dx*dz
endif
f(l1:l2,m,n,i+1)=0.0
f(l1:l2,m,n,i+2)=0.0
enddo
...
call fatal_error('dipole_tor',  'dipolar toroidal field not implemented for non-spherical coordinates')
endif
!
...
subroutine pre_stellar_cloud(f, datafile, mass_cloud,   cloud_mode, T_cloud_out_rel, dens_coeff,  temp_coeff, temp_trans, temp_coeff_out)
!
...
write (*,*) '(Modified BE-sphere) R = ', var1, 'cm =', bigr, 'pc_units'
do jj = 1, len_file
...
call eoscalc(ilnrho_lnTT, lnrho_r(test), lnTT_file(test), ss=tmp)
f(l,m,n,iss) = tmp
counter = counter+1
else
...
enddo
enddo
write (*,*) 'Covers:', counter, '/',  (abs(n1-n2)*abs(m1-m2)*abs(l1-l2)), 'cells'
!
...
write (*,*) '(Isothermal BE-sphere) R =', var1, 'cm =', bigr, 'pc_units, T =', var2, 'K'
do jj = 1, len_file
read(19,*) var1, var2
r_rho(jj) = var1/unit_length
