...
if (ncpus>1 .or. nprocx>1 .or. nprocy>1 .or. nprocz>1)  call stop_it('Inconsistency: MPICOMM=nompicomm, but ncpus>=2 or nproc[xyz]>=2')
!
...
if (lyinyang)  call stop_it('Inconsistency: Yin-Yang grid requires MPI and >= 6 processors')
!
...
f( 1:l1-1,m1:m2,:,ivar1:ivar2) =  c1*cshift(f(l2i:l2,m1:m2,:,ivar1:ivar2),-displs+2,2)  +c2*cshift(f(l2i:l2,m1:m2,:,ivar1:ivar2),-displs+1,2)  +c3*cshift(f(l2i:l2,m1:m2,:,ivar1:ivar2),-displs  ,2)  +c4*cshift(f(l2i:l2,m1:m2,:,ivar1:ivar2),-displs-1,2)  +c5*cshift(f(l2i:l2,m1:m2,:,ivar1:ivar2),-displs-2,2)  +c6*cshift(f(l2i:l2,m1:m2,:,ivar1:ivar2),-displs-3,2)
f(l2+1:mx,m1:m2,:,ivar1:ivar2) =  c1*cshift(f(l1:l1i,m1:m2,:,ivar1:ivar2), displs-2,2)  +c2*cshift(f(l1:l1i,m1:m2,:,ivar1:ivar2), displs-1,2)  +c3*cshift(f(l1:l1i,m1:m2,:,ivar1:ivar2), displs  ,2)  +c4*cshift(f(l1:l1i,m1:m2,:,ivar1:ivar2), displs+1,2)  +c5*cshift(f(l1:l1i,m1:m2,:,ivar1:ivar2), displs+2,2)  +c6*cshift(f(l1:l1i,m1:m2,:,ivar1:ivar2), displs+3,2)
endif
!
...
subroutine radboundary_yz_periodic_ray(Qrad_yz,tau_yz,  Qrad_yz_all,tau_yz_all)
!
...
subroutine radboundary_zx_periodic_ray(Qrad_zx,tau_zx,  Qrad_zx_all,tau_zx_all)
!
...
subroutine mpisendrecv_int_arr(send_array,sendcnt,proc_dest,sendtag,  recv_array,proc_src,recvtag,comm)
...
subroutine mpisendrecv_real_scl(send_array,proc_dest,sendtag,  recv_array,proc_src,recvtag)
...
subroutine mpisendrecv_real_arr(send_array,sendcnt,proc_dest,sendtag,  recv_array,proc_src,recvtag,idir)
integer :: sendcnt
...
subroutine mpisendrecv_real_arr2(send_array,sendcnt_arr,proc_dest,sendtag,  recv_array,proc_src,recvtag,idir)
...
subroutine mpisendrecv_real_arr3(send_array,sendcnt_arr,proc_dest,sendtag,  recv_array,proc_src,recvtag)
...
subroutine mpisendrecv_real_arr4(send_array,sendcnt_arr,proc_dest,sendtag,  recv_array,proc_src,recvtag)
integer, dimension(4) :: sendcnt_arr
real, dimension(sendcnt_arr(1),sendcnt_arr(2),sendcnt_arr(3),  sendcnt_arr(4)) :: send_array
real, dimension(sendcnt_arr(1),sendcnt_arr(2),sendcnt_arr(3),  sendcnt_arr(4)) :: recv_array
integer :: proc_src, proc_dest, sendtag, recvtag
call keep_compiler_quiet(sendtag, recvtag, proc_dest, proc_src)
call keep_compiler_quiet(send_array,recv_array)
...
if (lemergency_brake) call stop_it(  "Emergency brake activated. Check for error messages above.")
!
...
if (mod(nx,ny)/=0) then
if (lroot) print*, 'transp: works only if nx is an integer '// 'multiple of ny!'
call stop_it('transp')
endif
!
...
if (mod(nx_other,ny_other)/=0) then
call stop_it('transp_other: nxgrid must be an integer'// 'multiple of nygrid')
endif
!
...
if (lroot) print*,  'transp_other: works only for nx_grid=nz_grid!'
...
case default; call stop_it("communicate_vect_field_ghosts: "//topbot// " should be either `top' or `bot'")
end select
!
