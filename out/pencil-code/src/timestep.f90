...
alpha_ts=(/0.0,-567301805773./1357537059087., -2404267990393./2016746695238., -3550918686646./2091501179385., -1275806237668./842570457699./)
beta_ts=(/1432997174477./9575080441755., 5161836677717./13612068292357., 1720146321549./2090206949498., 3134564353537./4481467310338., 2277821191437./14882151754819./)
!

else
call fatal_error('initialize_timestep','Not implemented: itorder= '//  trim(itoa(itorder)))
endif
...
use Particles_main, only: particles_timestep_first,  particles_timestep_second
use PointMasses, only: pointmasses_timestep_first,  pointmasses_timestep_second
use Solid_Cells, only: solid_cells_timestep_first,  solid_cells_timestep_second
...
if (.not.lgpu) df=alpha_ts(itsub)*df
...
call copy_addr(alpha_ts,p_par(1))
call copy_addr(beta_ts ,p_par(2))
