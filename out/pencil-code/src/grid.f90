...
dxyz_step(1,2)=(xyz_step(1,2)-xyz_step(1,1))/  (xi_step(1,2)-xi_step(1,1))
dxyz_step(1,3)=(x00+Lx-xyz_step(1,2))/(nxgrid-1.0-xi_step(1,2))
!

call grid_profile(xi1,grid_func(1),g1,g1der1,g1der2,  dxyz=dxyz_step(1,:),xistep=xi_step(1,:),delta=xi_step_width(1,:))
call grid_profile(xi1lo,grid_func(1),g1lo,  dxyz=dxyz_step(1,:),xistep=xi_step(1,:),delta=xi_step_width(1,:))
...
if (coeff_grid(1) == 0.)  call fatal_error('construct_grid:', 'Cannot create '// 'a grid for a power-law of zero. Use "log" instead.')
c= coeff_grid(1)
...
case default
call fatal_error('construct_grid',  'No such x grid function - '//trim(grid_func(1)))
endselect
!
...
dxyz_step(2,2)=(xyz_step(2,2)-xyz_step(2,1))/  (xi_step(2,2)-xi_step(2,1))
dxyz_step(2,3)=(y00+Ly-xyz_step(2,2))/ (merge(nygrid+0.,nygrid-1.,lpole(2))-xi_step(2,2))
!

if (lpole(2)) then
do i=1,my
xi2(i)=0.5-nghost+0.5+(ipy*ny+i-1)* (nygrid+2.*(nghost-0.5)-1)/(nygrid+2.*nghost-1)
enddo
xi2proc = (/ (real(1 - nghost) + (real(nghost + i * ny) - 0.5) * real(mygrid - 2)  / real(mygrid - 1), i = 0, nprocy) /)
endif
!

call grid_profile(xi2,grid_func(2),g2,g2der1,g2der2,dxyz=  dxyz_step(2,:),xistep=xi_step(2,:),delta=xi_step_width(2,:))
call grid_profile(xi2lo,grid_func(2),g2lo,dxyz=  dxyz_step(2,:),xistep=xi_step(2,:),delta=xi_step_width(2,:))
call grid_profile(xi2proc, grid_func(2), g2proc, dxyz=dxyz_step(2,:), xistep=xi_step(2,:), delta=xi_step_width(2,:))
!

if (lpole(2)) then
call grid_profile(xi2up,grid_func(2),g2up,dxyz=  dxyz_step(2,:),xistep=xi_step(2,:),delta=xi_step_width(2,:))
...
case default
call fatal_error('construct_grid',  'No such y grid function - '//trim(grid_func(2)))
!
...
dxyz_step(3,2)=(xyz_step(3,2)-xyz_step(3,1))/  (xi_step(3,2)-xi_step(3,1))
dxyz_step(3,3)=(z00+Lz-xyz_step(3,2))/(nzgrid-1.0-xi_step(3,2))
!

call grid_profile(xi3,grid_func(3),g3,g3der1,g3der2,  dxyz=dxyz_step(3,:),xistep=xi_step(3,:),delta=xi_step_width(3,:))
call grid_profile(xi3lo,grid_func(3),g3lo,  dxyz=dxyz_step(3,:),xistep=xi_step(3,:),delta=xi_step_width(3,:))
...
case default
call fatal_error('construct_grid',  'No such z grid function - '//trim(grid_func(3)))
endselect
!
...
elseif (    coord_system=='spherical'  .or.coord_system=='spherical_coords') then
...
elseif (    coord_system=='cylindric'  .or.coord_system=='cylindrical_coords') then
lcylindrical_coords=.true.
else if (coord_system=='pipeflows') then
lpipe_coords=.true.
elseif (coord_system=='Lobachevskii') then
call fatal_error('set_coorsys_dimmask',  'Lobachevskii ccordinates not implemented')
endif
!
...
dxmin = minval( (/dxmin_x, dxmin_y, dxmin_z, huge(dx)/),  MASK=((/nxgrid, nygrid, nzgrid, 2/) > 1) )
...
dxmax = maxval( (/dxmax_x, dxmax_y, dxmax_z, epsilon(dx)/),  MASK=((/nxgrid, nygrid, nzgrid, 2/) > 1) )
...
fact=.5/CrossSec_w**2
glnCrossSec=glnCrossSec0*(-exp(-fact*(x(l1:l2)-CrossSec_x1)**2)  +exp(-fact*(x(l1:l2)-CrossSec_x2)**2))
!

case default
call fatal_error('initialize_grid',  'no such pipe function - '//trim(pipe_func))
endselect
!
...
call fatal_error('initialize_grid',  'no such coordinate system: '//coord_system)
endif
!
...
ncoarse=nz/nghost
call warning('initialize_grid','there are jumped-over processors due to grid coarsening'//  ' -> ncoarse reduced to floor(nz/nghost)='//trim(itoa(ncoarse)))
endif
!
...
allocate(nphis(mexts(1):mexts(2)), nphis1(mexts(1):mexts(2)),  nphis2(mexts(1):mexts(2)))
...
if (mod(nzgrid,nphi)/=0)  call warning('initialize_grid','coarsened grid non-periodic: nphi(m='//  trim(itoa(mm))//')='//trim(itoa(nphi)))
nphis(mm)=nphi
nphis1(mm)=1./nphi
...
where(ninds(-nghost:-1,mm,nn)<n1)  ninds(-nghost:-1,mm,nn)=(ninds(-nghost:-1,mm,nn)-n1+1)/nphi-1+n1
ninds(+1:+nghost,mm,nn)=nn+indgen(nghost)*nphi
where(ninds(+1:+nghost,mm,nn)>n2)  ninds(+1:+nghost,mm,nn)=(ninds(+1:+nghost,mm,nn)-n2-1)/nphi+1+n2
ne=nn
...
print*, 'On processors '//trim(itoa(iproc))//' ...(iprocy='//trim(itoa(ipy))//  ')... '//trim(itoa(find_proc(nprocx-1,ipy,nprocz-1)))//  ' the grid is coarsened for m = '//  trim(itoa(mexts(1)))//' ... '//trim(itoa(mexts(2)))
...
integer :: iv,izu
real, dimension(6), parameter :: coefs =  (/-0.00833333,0.0416667,-0.0833333,0.0833333,-0.0416667,0.00833333/)
izu=ipz*nz+nghost
do iv=1,mvar
...
real, dimension(my), save :: sinths,sin1ths,sin2ths,cosths,  cotths,cos1ths,tanths
...
if (nxgrid/=1)  box_volume = box_volume*1./3.*(xyz1(1)**3-xyz0(1)**3)
!
...
if (nxgrid/=1)  box_volume = box_volume*.5*(xyz1(1)**2-xyz0(1)**2)
!
...
if (  lpencil_in(i_pomx)  .or. lpencil_in(i_pomy)  .or. lpencil_in(i_phix)  .or. lpencil_in(i_phiy)) then
if (lcartesian_coords) then
...
if (lpenc_loc(i_rcyl_mn1)) p%rcyl_mn1=1./max(p%rcyl_mn,tini)
if (lpenc_loc(i_r_mn1))    p%r_mn1   =1./max(p%r_mn,tini)
if (lpenc_loc(i_pomx).or.lpenc_loc(i_pomy).or. lpenc_loc(i_phix).or.lpenc_loc(i_phiy))  call fatal_error('calc_pencils_grid',  'pomx, pomy, phix and phix not implemented for '//  'spherical polars')
endif
!
...
call fatal_error('calc_pencils_grid',  'position vector not implemented for '// 'non-cartesian coordinates')
endif
endif
!
...
call fatal_error('calc_pencils_grid',  'radial unit vector not implemented for '// 'non-cartesian coordinates')
endif
endif
!
...
case ('cos')
if (.not. present (param))  call fatal_error ('grid_profile', "'cos' needs its parameter.")
!
...
case ('arsinh')
if (.not. present (param))  call fatal_error ('grid_profile', "'arsinh' needs its parameter.")
!
...
case ('tanh')
if (.not. present (param))  call fatal_error ('grid_profile', "'tanh' needs its parameter.")
!
...
if (present(gder1)) then
gder1=  0.5*((1.-tanh(param*xi))+  (1+xi)*param*(tanh(param*xi)**2-1.0)+  exp(xi)*(1.+param+tanh(param*xi)-param*tanh(param*xi)**2))
endif
if (present(gder2)) then
gder2 = param*(tanh(param*xi)**2-1.) +  param**2*(1+xi)*(1.-tanh(param*xi)**2)*tanh(param*xi) +  0.5*exp(xi)*(1.+2*param+(1.-2*param**2)*tanh(param*xi)-  2*param*tanh(param*xi)**2+2*param**2*tanh(param*xi)**3)
endif
else
g=exp(xi)
if (present(gder1)) gder1=  g
...
case ('power-law')
if (.not. present (param))  call fatal_error ('grid_profile', "'power-law' needs its parameter.")
g=xi**(1./param)
...
case ('trough')
if (.not. present (param))  call fatal_error ('grid_profile', "'trough' needs its parameter.")
g=(-alog(exp(param*(xi-xistep(1)))+exp(-param*(xi-xistep(1))))/param+  alog(exp(param*(xi-xistep(2)))+exp(-param*(xi-xistep(2))))/param+    (2.+delta(1))*xi)/(2.+delta(1))
if (present(gder1)) then
gder1=(2.+delta(1)-tanh(param*(xi-xistep(1)))+  tanh(param*(xi-xistep(2))))/(2.+delta(1))
endif
if (present(gder2)) then
gder2=(-param*(1.-(tanh(param*(xi-xistep(1))))**2)+  param*(1.-(tanh(param*(xi-xistep(2))))**2))/  (2.+delta(1))
endif
!
...
case ('step-linear')
if (.not. (present(dxyz) .and. present(xistep) .and. present(delta)))  call fatal_error('grid_profile',"'step-linear' needs its parameters.")
if (xistep(1)/=0.0) then
g=                                                                     dxyz(1)*0.5*(xi-delta(1)*log(cosh(dble((xi-xistep(1))/delta(1))))) +  dxyz(2)*0.5*(   delta(1)*log(cosh(dble((xi-xistep(1))/delta(1))))  -  delta(2)*log(cosh(dble((xi-xistep(2))/delta(2)))) )+  dxyz(3)*0.5*(xi+delta(2)*log(cosh(dble((xi-xistep(2))/delta(2)))) )
!

if (present(gder1)) then
gder1=                                                       dxyz(1)*0.5*(1.0-tanh(dble((xi-xistep(1))/delta(1))) ) +   dxyz(2)*0.5*(    tanh(dble((xi-xistep(1))/delta(1)))   -   tanh(dble((xi-xistep(2))/delta(2))) ) +   dxyz(3)*0.5*(1.0+tanh(dble((xi-xistep(2))/delta(2))) )
!

endif
if (present(gder2)) then
gder2=   + 0.5/delta(1)*(dxyz(2)-dxyz(1))/cosh(dble((xi-xistep(1))/delta(1)))**2  + 0.5/delta(2)*(dxyz(3)-dxyz(2))/cosh(dble((xi-xistep(2))/delta(2)))**2
endif
else
g=                                                                     dxyz(2)*0.5*(xi-delta(2)*log(cosh(dble((xi-xistep(2))/delta(2)))) )+  dxyz(3)*0.5*(xi+delta(2)*log(cosh(dble((xi-xistep(2))/delta(2)))) )
!

if (present(gder1)) then
gder1=                                                       dxyz(2)*0.5*(1.0-tanh(dble((xi-xistep(2))/delta(2))) ) +   dxyz(3)*0.5*(1.0+tanh(dble((xi-xistep(2))/delta(2))) )
!

endif
if (present(gder2)) then
gder2=   + 0.5/delta(2)*(dxyz(3)-dxyz(2))/cosh(dble((xi-xistep(2))/delta(2)))**2
endif
endif
!
...
if (xi_lo>=xi_up)  call fatal_error('find_star','xi1 >= xi2 -- this should not happen')
!
...
if (any(lshift_origin) .or. any(lshift_origin_lower))  call fatal_error('inverse_grid', 'lshift_origin and lshift_origin_lower are not supported. ')
!
...
case default func
call fatal_error('inverse_grid in grid.f90',  'unknown grid function ' // trim(grid_func(dir)))
!
...
xi = xi - real(shift)
inear = nint(xi)
where ((x < xyz1(dir) .and. inear > inear_max) .or.  (x < xyz0(dir) .and. inear > nghost)) xi = nearest(xi, -1.0)
endif getloc
!
...
if (nxgrid>1) then
if (lfirst_proc_x)  dx2_bound(-1:-nghost:-1)= 2.*(x(l1+1:l1+nghost)-x(l1))
if (llast_proc_x)  dx2_bound(nghost:1:-1)  = 2.*(x(l2)-x(l2-nghost:l2-1))
!

call calc_bound_coeffs(x,coeffs_1_x)
endif
if (nygrid>1) then
if (lfirst_proc_y)  dy2_bound(-1:-nghost:-1)= 2.*(y(m1+1:m1+nghost)-y(m1))
if (llast_proc_y)  dy2_bound(nghost:1:-1)  = 2.*(y(m2)-y(m2-nghost:m2-1))
!

call calc_bound_coeffs(y,coeffs_1_y)
endif
if (nzgrid>1) then
if (lfirst_proc_z)  dz2_bound(-1:-nghost:-1)= 2.*(z(n1+1:n1+nghost)-z(n1))
if (llast_proc_z)  dz2_bound(nghost:1:-1)  = 2.*(z(n2)-z(n2-nghost:n2-1))
!
