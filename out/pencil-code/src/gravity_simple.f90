...
character (len=labellen) :: gravx_profile='zero',gravy_profile='zero',  gravz_profile='zero'
!
...
namelist /grav_init_pars/  gravx_profile, gravy_profile, gravz_profile, gravx, gravy, gravz,  xgrav, ygrav, zgrav, kx_gg, ky_gg, kz_gg, dgravx, pot_ratio, z1, z2,  nux_epicycle, nu_epicycle, xref, zref, g_ref, sphere_rad, lnrho_bot, lnrho_top,  ss_bot, ss_top, lgravx_gas, lgravx_dust, lgravy_gas, lgravy_dust,  lgravz_gas, lgravz_dust, xinfty, yinfty, zinfty, lxyzdependence,  lcalc_zinfty, kappa_x1, kappa_x2, kappa_z1, kappa_z2, reduced_top,  lboussinesq_grav, n_pot, cs0hs, H0hs, grav_tilt, grav_amp,  potx_const,poty_const,potz_const, zclip, n_adjust_sphersym, gravitational_const,  mass_cent_body, g_A_factor, g_C_factor, g_B_factor, g_D_factor
!

namelist /grav_run_pars/  gravx_profile, gravy_profile, gravz_profile, gravx, gravy, gravz,  xgrav, ygrav, zgrav, kx_gg, ky_gg, kz_gg, dgravx, pot_ratio,  nux_epicycle, nu_epicycle, xref, zref, g_ref, sphere_rad,  lgravx_gas, lgravx_dust, lgravy_gas, lgravy_dust,  lgravz_gas, lgravz_dust, xinfty, yinfty, zinfty, lxyzdependence,  lcalc_zinfty, kappa_x1, kappa_x2, kappa_z1, kappa_z2, reduced_top,  lboussinesq_grav, n_pot, grav_tilt, grav_amp,  potx_const,poty_const,potz_const, zclip, n_adjust_sphersym, gravitational_const,  mass_cent_body, g_A_factor, g_C_factor, g_B_factor, g_D_factor
!
...
integer :: idiag_epotmxy=0
integer :: idiag_epotuxmxy=0
!
...
if (lrun) then
if (gravx_profile == 'zero' .and.  gravy_profile == 'zero' .and.  gravz_profile == 'zero') then
...
if (lroot) print*,'initialize_gravity: linear x-grav with z dep, nu=',  nux_epicycle,kappa_z1,kappa_z2
...
potx_xpencil=(tanh((x+pi/3.)/0.1)+tanh(-(x-pi/3.)/0.1))/2.* gravx*(.5*cos(2*(x-pi/2.))-0.5) + potx_const
!
...
case default
if (lroot) print*,  'initialize_gravity: unknown gravx_profile ', gravx_profile
...
case default
if (lroot) print*,  'initialize_gravity: unknown gravy_profile ', gravy_profile
...
if (alloc_err > 0) call fatal_error ('initialize_gravity',  'Could not allocate memory for gravity and gravity potential profiles', .true.)
call read_grav_profile(gravity_z_dat, grav_init_z, real(unit_length/(unit_time*unit_time)), .false.)
gravz_zpencil = grav_init_z
potz_zpencil = -gravz_zpencil * (z - zinfty)
gravz = grav_init_z(n1)
!	
...
case ('linear_xdep')
nu_epicycle2=nu_epicycle**2
if (lroot) print*,'initialize_gravity: linear z-grav with x dep, nu=',  nu_epicycle,kappa_x1,kappa_x2
...
nu_epicycle2=nu_epicycle**2
if (lroot) print*,'initialize_gravity: linear z-grav, '//  'smoothed to zero at top/bottom, nu=', nu_epicycle
...
if (lroot.and.(cs0hs==0.or.H0hs==0))  call fatal_error('initialize-gravity',  'Set cs0hs and H0hs in grav_init_pars!')
...
case default
if (lroot) print*,  'initialize_gravity: unknown gravz_profile ', gravz_profile
...
if (notanumber(gravx_xpencil))  call fatal_error('initialize_gravity','found NaN or +/-Inf in gravx_xpencil')
if (notanumber(gravy_ypencil))  call fatal_error('initialize_gravity','found NaN or +/-Inf in gravy_ypencil')
if (notanumber(gravz_zpencil))  call fatal_error('initialize_gravity','found NaN or +/-Inf in gravz_zpencil')
!
...
if (gravitational_const<=0.)  call fatal_error('initialize_gravity', 'positive gravitational constant needed')
endif
...
if (.not. parallel_file_exists (filename))  call fatal_error ('read_grav_profile', "can't find "//filename)
if (lroot) then
...
if (alloc_err > 0) call fatal_error ('read_grav_profile',  'Could not allocate memory for data and its z coordinate', .true.)
!
...
call warning ("interpolate_grav_profile",  "extrapolated "//trim (itoa (num_below))//" grid points below bottom")
endif
if (lfirst_proc_xy .and. (num_over > 0)) then
call warning ("interpolate_grav_profile",  "extrapolated "//trim (itoa (num_over))//" grid points over top")
endif
!
...
if (idiag_epot/=0 .or. idiag_epot/=0 .or. idiag_epotmx/=0 .or.  idiag_epotmy/=0 .or. idiag_epotmz/=0 .or.  idiag_epottot/=0) lpenc_diagnos(i_epot)=.true.
...
call get_shared_variable('reference_state',reference_state,caller='duu_dt_grav')
...
if (idiag_ugm/=0)  call sum_mn_name(p%uu(:,1)*p%gg(:,1) +  p%uu(:,2)*p%gg(:,2) +  p%uu(:,3)*p%gg(:,3),idiag_ugm)
...
call fatal_error('potential_global','this subroutine has been '//  'deprecated for gravity_simple')
!
...
case default
call fatal_error('potential_point',  'gravx_profile='//gravx_profile//' not implemented')
endselect
endif
!
...
case default
call fatal_error('potential_point',  'gravy_profile='//gravy_profile//' not implemented')
endselect
endif
!
...
case default
call fatal_error('potential_point',  'gravz_profile='//gravz_profile//' not implemented')
endselect
endif
!
