...
fmt = '(A, A'  // ', T' // trim(adjustl(tmp2))  // ', " v. ", A, T' // trim(adjustl(tmp3))  // ', " (", A, T' // trim(adjustl(tmp4))  // ', ") ", A)'
...
call extract_substring(svnid, iy0, it1-1, date)
else
...
write(*,fmt) "SVN: ",  trim(filename),  revision(1:wv),  date(1:wd),  trim(author)
!

write(unit,fmt) "SVN: ",  trim(filename),  revision(1:wv),  date(1:wd),  trim(author)
else
wd1 = min(wd, len(svnid))
write(*,fmt) "SVN: ",  '-------',  '',  '',  svnid(1:wd1)
write(unit,fmt) "SVN: ",  '-------',  '',  '',  svnid(1:wd1)
endif
!
...
if (ltermcap_color) then
write(*,fmt='(A1,A1,I1,A1,I2,A1)',ADVANCE='no')  CHAR(27), '[', iterm_BRIGHT, ';', iterm_FG_MAGENTA, 'm'
endif
!
...
if (ltermcap_color) then
write(*,fmt='(A1,A1,I1,A1,I2,A1)',ADVANCE='no')  CHAR(27), '[', iterm_BRIGHT, ';', iterm_FG_RED, 'm'
endif
!
...
call warning(scaller,'End-Of-File'//trim (filename)//trim (message))
else
outlog = .true.
call fatal_error(scaller,'End-Of-File'//trim (filename)//trim (message))
endif
elseif (code > 0 .and. .not.(lread.and.lcontl)) then
outlog = .true.
call fatal_error(scaller,'I/O error (code '//trim (itoa (code))//')'//  trim (filename)//trim (message), force=.true.)
endif
return
endif
!
...
if (lwrite.and.code==0) curback = curback+1
!
...
if (.not.lopen.and.curdist/=0) then
!
...
if (lclose.and.code==0) open(curdist,file=curfile,position='append')
!

if ( backskip(curdist,curback) ) then
if (lroot) submsg = trim(submsg)//' not'
endif
if (lroot) submsg = trim(submsg)//' synchronized.'
!

close(curdist,IOSTAT=iostat)
...
else if ( curdist<0 .and. code/=0 ) then
!
...
close(abs(curdist),IOSTAT=iostat)
if (iostat/=0) call safe_character_append(submsg,'. File not closed!')
!
...
if (lopen)  call safe_character_append(errormsg,' opening ')
if (lread.or.lwrite)  call safe_character_append(errormsg,' for ')
call safe_character_append(errormsg,trim(modestr)//'ing ')
if ( .not.(lopen.or.lclose).and.(lread .or. lwrite) ) then
if (lread.or.lwrite)  call safe_character_append(errormsg,' '//trim(item))
if (lread) then
...
if ( .not.lopen.and..not.lread )  call system_cmd(  'echo '//trim(errormsg)//'|'//trim(mailcmd)//"-s 'PencilCode Message' "//trim(mailaddress)//' >& /dev/null')
endif
endif
!
...
outlog = .true.
if (lstop_on_ioerror.or.(.not.lopen.and.lread.and..not.lcontl))  call fatal_error(scaller, 'I/O error due to '//trim(errormsg)//' with '//trim(filename), force=.true.)
endif
!
