...
if (any(ndown==0))  call fatal_error('run','zero points in processor '  //trim(itoa(iproc))//' for downsampling')
...
if (mglobal/=0) call input_globals('global.dat',  f(:,:,:,mvar+maux+1:mvar+maux+mglobal),mglobal)
!
...
if ( ((lpencil_check .and. .not. suppress_pencil_check) .or.  (.not.lpencil_check.and.lpencil_check_small)) .and. nt>0 )  call pencil_consistency_check(f,df,p)
!
...
time_per_step = (time_this_diagnostic - time_last_diagnostic)  /(  it_this_diagnostic -   it_last_diagnostic)
...
if (ip<=12.and.lroot) print*,'wsnap: written snapshot var.dat in ', mpiwtime()-tvar1,' seconds'
...
wall_clock_time=time2-time1
print*
write(*,'(A,1pG10.3,A,1pG11.4,A)')  ' Wall clock time [hours] = ', wall_clock_time/3600.0,  ' (+/- ', real(mpiwtick())/3600.0, ')'
if (it>1) then
if (lparticles) then
write(*,'(A,1pG10.3)')  ' Wall clock time/timestep/(meshpoint+particle) [microsec] =',  wall_clock_time/icount/(nw+npar/ncpus)/ncpus/1.0e-6
else
write(*,'(A,1pG14.7)')  ' Wall clock time/timestep/meshpoint [microsec] =',  wall_clock_time/icount/nw/ncpus/1.0e-6
endif
endif
endif
memuse=memusage()
