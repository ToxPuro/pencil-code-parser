...
public :: read_range, merge_ranges, add_merge_range, get_range_no, write_by_ranges,  write_by_ranges_1d_real, write_by_ranges_1d_cmplx,  write_by_ranges_2d_real, write_by_ranges_2d_cmplx
...
public :: qualify_position_bilin, qualify_position_bicub,  qualify_position_biquin
...
integer, parameter :: LGAP=-3, RGAP=3, NOGAP=0, LNEIGH=-4, RNEIGH=4, LMARG=-2, RMARG=2,  LMARG2=-1, RMARG2=1, LNEIGH2=-5, RNEIGH2=5
!
...
use Cdata, only: mm,nn,imn_array,necessary,necessary_imn,lroot,ip,  lyinyang,lcutoff_corners,nycut,nzcut,  lfirst_proc_y, lfirst_proc_z, llast_proc_y, llast_proc_z
!
...
if (lfirst_proc_z) call reset_triangle_inds(1,my-nycut+nghost, 1,mz-nzcut+nghost,imn_array)
if (llast_proc_z ) call reset_triangle_inds(1,my-nycut+nghost,mz,nzcut+1-nghost ,imn_array)
endif
if (llast_proc_y) then
if (lfirst_proc_z) call reset_triangle_inds(my,nycut+1-nghost,  1,mz-nzcut+nghost,imn_array)
if (llast_proc_z ) call reset_triangle_inds(my,nycut+1-nghost, mz,nzcut+1-nghost ,imn_array)
endif
endif
!
...
lloop=q>0.27597
if (lloop)  lloop=(q>0.27846).or.(v**2>-4.0*log(u)*u**2)
enddo
!
...
iy=ia*(iy-k*iq)-ir*k
if (iy < 0) iy=iy+im
nr_ran=am*ior(iand(im,ieor(ix,iy)),1)
...
print*, "safe_character_assign: ",  "RUNTIME ERROR: FORCED STRING TRUNCATION WHEN ASSIGNING '"  //src//"' to ",destLen," characters"
dest=src(1:destLen)
else
dest=src
endif
...
w3(1)=-1./(z(3)-z(2))**2
w2(1)=w1(1)+w3(1)
d(1)=2.*((f(2)-f(1))/(z(2)-z(1))**3  -(f(3)-f(2))/(z(3)-z(2))**3)
!
...
d(2:mz-1)=3.*(w3(2:mz-1)**2*(f(3:mz)-f(2:mz-1))  +w1(2:mz-1)**2*(f(2:mz-1)-f(1:mz-2)))
!
...
w2(mz)=w1(mz)+w3(mz)
d(mz)=2.*((f(mz-1)-f(mz-2))/(z(mz-1)-z(mz-2))**3  -(f(mz)-f(mz-1))/(z(mz)-z(mz-1))**3)
!
...
q(2:mz)=.5*dz(2:mz)*(f(1:mz-1)+f(2:mz))  +(1./12.)*dz(2:mz)**2*(df(1:mz-1)-df(2:mz))
!
...
S(j) = (fac*h1(i)) * (sol(i+1)*(x2(j)-arrx(i))**3 + sol(i)*(arrx(i+1) - x2(j))**3)  +  (x2(j) - arrx(i))*(arry(i+1)*h1(i) - h(i)*sol(i+1)*fac)                          +  (arrx(i+1) - x2(j))*(arry(i)*h1(i) - h(i)*sol(i)*fac)
endif
!
...
downgrade: if (ltvd .and. (y1(n) - y2(i)) * (y2(i) - a) < 0.0 .or.  lposdef .and. y2(i) < 0.0) then
...
z=abs(x)
t=1/(1.0+0.5*z)
erfcc=t*exp(-z*z-1.26551223+t*(1.00002368+t*(.37409196+t*  (.09678418+t*(-.18628806+t*(.27886807+t*(-1.13520398+t*   (1.48851587+t*(-.82215223+t*.17087277)))))))))
...
REAL :: Y,P1,P2,P3,P4,P5,R1,R2,R3,R4,R5,R6   ,Q1,Q2,Q3,Q4,Q5,S1,S2,S3,S4,S5,S6
DATA P1,P2,P3,P4,P5 /1.D0,-.1098628627D-2,.2734510407D-4,  -.2073370639D-5,.2093887211D-6 /
DATA Q1,Q2,Q3,Q4,Q5 /-.1562499995D-1,.1430488765D-3,  -.6911147651D-5,.7621095161D-6,-.9349451520D-7 /
DATA R1,R2,R3,R4,R5,R6 /57568490574.,-13362590354.,  651619640.7,-11214424.18,77392.33017,-184.9052456 /
DATA S1,S2,S3,S4,S5,S6 /57568490411.,1029532985.,  9494680.718,59272.64853,267.8532712,1. /
IF(X==0.) GO TO 1
AX = ABS (X)
IF (AX<8.) THEN
Y = X*X
...
REAL :: Y,P1,P2,P3,P4,P5,P6,R1,R2,R3,R4,R5,R6   ,Q1,Q2,Q3,Q4,Q5,S1,S2,S3,S4,S5,S6
DATA P1,P2,P3,P4,P5 /1.D0,.183105D-2,-.3516396496D-4,   .2457520174D-5,-.240337019D-6 /,P6 /.636619772D0 /
DATA Q1,Q2,Q3,Q4,Q5 /.04687499995D0,-.2002690873D-3,    .8449199096D-5,-.88228987D-6,.105787412D-6 /
DATA R1,R2,R3,R4,R5,R6 /72362614232.,-7895059235.,  242396853.1,-2972611.439,15704.48260,-30.16036606 /
DATA S1,S2,S3,S4,S5,S6 /144725228442.,2300535178.,  18583304.74,99447.43394,376.9991397,1. /
!
...
if (lcheck) then
print*, 'linear_interpolate_1d: Interpolation point does not ' //  'lie within the calculated grid point interval.'
...
if ( xx(ix0)-eps<=xxp(1) .and. xx(ix0+1)+eps>=xxp(1) .and.  yy(iy0)-eps<=xxp(2) .and. yy(iy0+1)+eps>=xxp(2) )  then
else
if (lcheck) then
print*, 'linear_interpolate_2d: Interpolation point does not ' //  'lie within the calculated grid point interval.'
...
if ((x(ix0)<=xxp(1) .and. x(ix0+1)>=xxp(1) .or. nxgrid==1) .and.  (y(iy0)<=xxp(2) .and. y(iy0+1)>=xxp(2) .or. nygrid==1) .and.  (z(iz0)<=xxp(3) .and. z(iz0+1)>=xxp(3) .or. nzgrid==1)) then
else
print*, 'linear_interpolate: Interpolation point does not ' //  'lie within the calculated grid point interval.'
...
gp = g1 + xp0*dx1*(-g1+g2) + yp0*dy1*(-g1+g3) + zp0*dz1*(-g1+g5) +  xp0*yp0*dxdy1*(g1-g2-g3+g4) + xp0*zp0*dxdz1*(g1-g2-g5+g6) +  yp0*zp0*dydz1*(g1-g3-g5+g7) +  xp0*yp0*zp0*dxdydz1*(-g1+g2+g3-g4+g5-g6-g7+g8)
!
...
print*, 'linear_interpolate: x0, y0, z0=',  x(ix0), y(iy0), z(iz0)
print*, 'linear_interpolate: i, gp(i)=', i, gp(i)
print*, 'linear_interpolate: g1...g8=',  g1(i), g2(i), g3(i), g4(i), g5(i), g6(i), g7(i), g8(i)
print*, '------------------'
endif
...
store(:,istore) = (/testrange(2)+1,range(2)/)
endif
elseif (range(1)<testrange(1)) then
if (range(2)>=testrange(1)) then
store(:,istore) = (/range(1),testrange(1)-1/)
elseif (range(2)<testrange(1)) then
continue
else
store(:,istore) = (/range(1),testrange(1)-1/)
iestore=iestore+1
store(:,iestore) = (/testrange(2)+1,range(2)/)
istore=istore+1
endif
else
continue
...
if ( iar <= iei+stepi .and.  ier >= iai-stepi .and.  mod(iar-iai,stepi) == 0 ) then
!
...
if ( ldefr .and. ios == 0 )  range(1) = min(max(defrange(1),range(1)),defrange(2))
endif
!
...
if ( ldefr .and. ios1 == 0 )  range(2) = min(max(defrange(1),range(2)),defrange(2))
endif
!
...
if ( ios/=0 .or. ios1/=0 .or. ios2/=0 )  print*, 'read_range_r - Warning: invalid data in range!'
!
...
if ( ios/=0 .or. ios1/=0 .or. ios2/=0 )  print*, 'read_range_i - Warning: invalid data in range!'
!
...
get_range_no = get_range_no +  ceiling( (ranges(2,i)-ranges(1,i)+1.)/ranges(3,i))
else
exit
endif
enddo
...
if (len(date) < 20)  print*, 'date_time_string: WARNING -- string arg "date" too short'
!

call date_and_time(VALUES=values)
write(datestr,'(I2.2,"-",A3,"-",I4.2," ",I2.2,":",I2.2,":",I2.2)')  values(3), month(values(2)), values(1),  values(5), values(6), values(7)
date=trim(datestr)
!
...
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i8_1*(maxval(abs(f(ll,mm,nn,      k:k+2)))  + maxval(abs(f(ll,mm,nn+1,    k:k+2)))  + maxval(abs(f(ll,mm+1,nn,    k:k+2)))  + maxval(abs(f(ll,mm+1,nn+1,  k:k+2)))  + maxval(abs(f(ll+1,mm,nn,    k:k+2)))  + maxval(abs(f(ll+1,mm,nn+1,  k:k+2)))  + maxval(abs(f(ll+1,mm+1,nn,  k:k+2)))  + maxval(abs(f(ll+1,mm+1,nn+1,k:k+2))))
enddo; enddo; enddo
!
...
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i2_1*(maxval(abs(f(ll  ,mm,nn,k:k+2)))  + maxval(abs(f(ll+1,mm,nn,k:k+2))))
enddo; enddo; enddo
elseif (nygrid/=1) then
do ll=l1,l2; do mm=2,my-2; do nn=n1,n2
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i2_1*(maxval(abs(f(ll,mm  ,nn,k:k+2)))  + maxval(abs(f(ll,mm+1,nn,k:k+2))))
enddo; enddo; enddo
else
do ll=l1,l2; do mm=m1,m2; do nn=2,mz-2
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i2_1*(maxval(abs(f(ll,mm,nn,  k:k+2)))  + maxval(abs(f(ll,mm,nn+1,k:k+2))))
enddo; enddo; enddo
endif
!
...
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i4_1*(maxval(abs(f(ll  ,mm  ,nn,k:k+2)))  + maxval(abs(f(ll  ,mm+1,nn,k:k+2)))  + maxval(abs(f(ll+1,mm  ,nn,k:k+2)))  + maxval(abs(f(ll+1,mm+1,nn,k:k+2))))
enddo; enddo; enddo
elseif (nygrid==1) then
do ll=2,mx-2; do mm=m1,m2; do nn=2,mz-2
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i4_1*(maxval(abs(f(ll  ,mm,nn,  k:k+2)))  + maxval(abs(f(ll  ,mm,nn+1,k:k+2)))  + maxval(abs(f(ll+1,mm,nn,  k:k+2)))  + maxval(abs(f(ll+1,mm,nn+1,k:k+2))))
enddo; enddo; enddo
else
do ll=l1,l2; do mm=2,my-2; do nn=2,mz-2
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i4_1*(maxval(abs(f(ll,mm  ,nn,  k:k+2)))  + maxval(abs(f(ll,mm  ,nn+1,k:k+2)))  + maxval(abs(f(ll,mm+1,nn,  k:k+2)))  + maxval(abs(f(ll,mm+1,nn+1,k:k+2))))
enddo; enddo; enddo
endif
...
do ll=2,mx-2; do mm=2,my-2; do nn=2,mz-2
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i8_1*(abs(f(ll,mm,nn,      k))  + abs(f(ll,mm,nn+1,    k))  + abs(f(ll,mm+1,nn,    k))  + abs(f(ll,mm+1,nn+1,  k))  + abs(f(ll+1,mm,nn,    k))  + abs(f(ll+1,mm,nn+1,  k))  + abs(f(ll+1,mm+1,nn,  k))  + abs(f(ll+1,mm+1,nn+1,k)))
enddo; enddo; enddo
!
...
if (nxgrid/=1) then
do ll=2,mx-2; do mm=m1,m2; do nn=n1,n2
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i2_1*(abs(f(ll  ,mm,nn,k))  + abs(f(ll+1,mm,nn,k)))
enddo; enddo; enddo
elseif (nygrid/=1) then
do ll=l1,l2; do mm=2,my-2; do nn=n1,n2
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i2_1*(abs(f(ll,mm  ,nn,k))  + abs(f(ll,mm+1,nn,k)))
enddo; enddo; enddo
else
do ll=l1,l2; do mm=m1,m2; do nn=2,mz-2
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i2_1*(abs(f(ll,mm,nn,  k))  + abs(f(ll,mm,nn+1,k)))
enddo; enddo; enddo
endif
!
...
elseif (nzgrid==1) then
do ll=2,mx-2; do mm=2,my-2; do nn=n1,n2
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i4_1*(abs(f(ll  ,mm  ,nn,k))  + abs(f(ll  ,mm+1,nn,k))  + abs(f(ll+1,mm  ,nn,k))  + abs(f(ll+1,mm+1,nn,k)))
enddo; enddo; enddo
elseif (nygrid==1) then
do ll=2,mx-2; do mm=m1,m2; do nn=2,mz-2
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i4_1*(abs(f(ll  ,mm,nn,  k))  + abs(f(ll  ,mm,nn+1,k))  + abs(f(ll+1,mm,nn,  k))  + abs(f(ll+1,mm,nn+1,k)))
enddo; enddo; enddo
else
do ll=l1,l2; do mm=2,my-2; do nn=2,mz-2
f(ll,mm,nn,jmean) = f(ll,mm,nn,jmean)  +weight*i4_1*(abs(f(ll,mm  ,nn,  k))  + abs(f(ll,mm  ,nn+1,k))  + abs(f(ll,mm+1,nn,  k))  + abs(f(ll,mm+1,nn+1,k)))
enddo; enddo; enddo
endif
...
do ll=2,mx-2; do mm=2,my-2; do nn=2,mz-2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(maxval(abs(f(ll,mm,nn,      k:k+2)))  ,maxval(abs(f(ll,mm,nn+1,    k:k+2)))  ,maxval(abs(f(ll,mm+1,nn,    k:k+2)))  ,maxval(abs(f(ll,mm+1,nn+1,  k:k+2)))  ,maxval(abs(f(ll+1,mm,nn,    k:k+2)))  ,maxval(abs(f(ll+1,mm,nn+1,  k:k+2)))  ,maxval(abs(f(ll+1,mm+1,nn,  k:k+2)))  ,maxval(abs(f(ll+1,mm+1,nn+1,k:k+2)))))
enddo; enddo; enddo
elseif (dimensionality==1) then
if (nxgrid/=1) then
do ll=2,mx-2; do mm=m1,m2; do nn=n1,n2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(maxval(abs(f(ll,mm,nn,  k:k+2)))  ,maxval(abs(f(ll+1,mm,nn,k:k+2)))))
enddo; enddo; enddo
elseif (nygrid/=1) then
do ll=l1,l2; do mm=2,my-2; do nn=n1,n2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(maxval(abs(f(ll,mm,nn,  k:k+2)))  ,maxval(abs(f(ll,mm+1,nn,k:k+2)))))
enddo; enddo; enddo
else
do ll=l1,l2; do mm=m1,m2; do nn=2,mz-2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(maxval(abs(f(ll,mm,nn,  k:k+2)))  ,maxval(abs(f(ll,mm,nn+1,k:k+2)))))
enddo; enddo; enddo
endif
elseif (nzgrid==1) then
do ll=2,mx-2; do mm=2,my-2; do nn=n1,n2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(maxval(abs(f(ll,mm,nn,    k:k+2)))  ,maxval(abs(f(ll,mm+1,nn,  k:k+2)))  ,maxval(abs(f(ll+1,mm,nn,  k:k+2)))  ,maxval(abs(f(ll+1,mm+1,nn,k:k+2)))))
enddo; enddo; enddo
elseif (nygrid==1) then
do ll=2,mx-2; do mm=m1,m2; do nn=2,mz-2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(maxval(abs(f(ll,mm,nn,    k:k+2)))  ,maxval(abs(f(ll,mm,nn+1,  k:k+2)))  ,maxval(abs(f(ll+1,mm,nn,  k:k+2)))  ,maxval(abs(f(ll+1,mm,nn+1,k:k+2)))))
enddo; enddo; enddo
else
do ll=l1,l2; do mm=2,my-2; do nn=2,mz-2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(maxval(abs(f(ll,mm,nn,    k:k+2)))  ,maxval(abs(f(ll,mm,nn+1,  k:k+2)))  ,maxval(abs(f(ll,mm+1,nn,  k:k+2)))  ,maxval(abs(f(ll,mm+1,nn+1,k:k+2)))))
enddo; enddo; enddo
endif
...
do ll=2,mx-2; do mm=2,my-2; do nn=2,mz-2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(abs(f(ll,mm,nn,      k))  ,abs(f(ll,mm,nn+1,    k))  ,abs(f(ll,mm+1,nn,    k))  ,abs(f(ll,mm+1,nn+1,  k))  ,abs(f(ll+1,mm,nn,    k))  ,abs(f(ll+1,mm,nn+1,  k))  ,abs(f(ll+1,mm+1,nn,  k))  ,abs(f(ll+1,mm+1,nn+1,k))))
enddo; enddo; enddo
elseif (dimensionality==1) then
if (nxgrid/=1) then
do ll=2,mx-2; do mm=m1,m2; do nn=n1,n2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(abs(f(ll,mm,nn,  k))  ,abs(f(ll+1,mm,nn,k))))
enddo; enddo; enddo
elseif (nygrid/=1) then
do ll=l1,l2; do mm=2,my-2; do nn=n1,n2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(abs(f(ll,mm,nn,  k))  ,abs(f(ll,mm+1,nn,k))))
enddo; enddo; enddo
else
do ll=l1,l2; do mm=m1,m2; do nn=2,mz-2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(abs(f(ll,mm,nn,  k))  ,abs(f(ll,mm,nn+1,k))))
enddo; enddo; enddo
endif
elseif (nzgrid==1) then
do ll=2,mx-2; do mm=2,my-2; do nn=n1,n2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(abs(f(ll,mm,nn,    k))  ,abs(f(ll,mm+1,nn,  k))  ,abs(f(ll+1,mm,nn,  k))  ,abs(f(ll+1,mm+1,nn,k))))
enddo; enddo; enddo
elseif (nygrid==1) then
do ll=2,mx-2; do mm=m1,m2; do nn=2,mz-2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(abs(f(ll,mm,nn,    k))  ,abs(f(ll,mm,nn+1,  k))  ,abs(f(ll+1,mm,nn,  k))  ,abs(f(ll+1,mm,nn+1,k))))
enddo; enddo; enddo
else
do ll=l1,l2; do mm=2,my-2; do nn=2,mz-2
f(ll,mm,nn,jmax) = max(f(ll,mm,nn,jmax),  weight*max(abs(f(ll,mm,nn,    k))  ,abs(f(ll,mm,nn+1,  k))  ,abs(f(ll,mm+1,nn,  k))  ,abs(f(ll,mm+1,nn+1,k))))
enddo; enddo; enddo
endif
...
use Cdata, only: iproc_world, directory, workdir, datadir, directory_dist,  datadir_snap, directory_snap, directory_collect,  datadir_prestart, directory_prestart
...
call safe_character_assign(directory_dist,  trim(workdir)//trim(datadir_snap)//'/proc'//chproc)
call safe_character_assign(directory_prestart,  trim(workdir)//trim(datadir_prestart)//'/proc'//chproc)
!

if (loptest(lproc)) then
call safe_character_assign(directory_snap,  trim(workdir)//trim(datadir_snap)//'/proc'//chproc)
else
call safe_character_assign(directory_snap,  trim(workdir)//trim(datadir_snap)//'/allprocs')
endif
call safe_character_assign(directory_collect,  trim(workdir)//trim (datadir_snap)//'/allprocs')
!
...
integer :: j
var_is_vec = iuu>0.and.j>=iux.and.j<=iuz .or.   iaa>0.and.j>=iax.and.j<=iaz .or.   iaatest>0.and.j>=iaatest.and.j<=iaatest+ntestfield-1 .or.  iuutest>0.and.j>=iuutest.and.j<=iuutest+ntestflow-1
...
iph_shift=ioptest(iph_shift_)
ith_shift=ioptest(ith_shift_)
thphprime(1,:,:)=0.
!
...
call copy_with_shift(istart,iend,jstart,jend,source(:,my-len_cut+1:,:,j),dest(:,:,:,j),0,shift,ladd)
jstart=jstart_; jend=jstart+nghost-1
call copy_with_shift(istart,iend,jstart,jend,source(:,:,:,j),dest(:,:,:,j),0,shift,ladd)
if (ladd) then
dest(:,my-len_cut+1:my,jstart:jend,j) = dest  (:,my-len_cut+1:my,jstart:jend,j)  +source(:,2*(my-len_cut)+1:my,1:nghost,j)
else
...
if (ladd) then
dest(:,:len_cut,jstart:jend,j) = dest  (:,:len_cut,jstart:jend,j)  +source(:,:len_cut,1:nghost,j)
else
...
call copy_with_shift(istart,iend,jstart,jend,source(:,:,mz-len_cut+1:,j),dest(:,:,:,j),shift,0,ladd)
istart=istart_; iend=istart+nghost-1
call copy_with_shift(istart,iend,jstart,jend,source(:,:,:,j),dest(:,:,:,j),shift,0,ladd)
if (ladd) then
dest(:,istart:iend,mz-len_cut+1:mz,j) = dest  (:,istart:iend,mz-len_cut+1:mz,j)  +source(:,1:nghost,2*(mz-len_cut)+1:mz,j)
else
...
if (ladd) then
dest(:,istart:iend,1:len_cut,j) = dest  (:,istart:iend,1:len_cut,j)  +source(:,1:nghost,1:len_cut,j)
else
...
if (powers(1)/=0)  transformed(:,1) = vec(:,1)
if (powers(2)/=0)  transformed(:,2) = b*vec(:,2) + a*vec(:,3)
if (powers(3)/=0)  transformed(:,3) =-a*vec(:,2) + b*vec(:,3)
...
nok=0
do i=1,nyz
if (yzyang(1,i) < minval(y)-dy .or. yzyang(1,i) > maxval(y)+dy .or.  yzyang(2,i) < minval(z)-dz .or. yzyang(2,i) > maxval(z)+dz) then
nok=nok+1
inds(nok)=i
endif
enddo
...
integer, intent(in) :: n
real, intent(out), optional :: step_size
...
if (ios/=0) then
if (lroot)  print*, trim(caller)//': Warning - unreadable species number in slice name "'//  trim(name)//'"'
return
endif
endif
if (ispec>maxnr) then
if (lroot)  print*, trim(caller)//': Warning - species number in slice name "'//  trim(name)//'" bigger than maximum '//trim(itoa(maxnr))
ispec=0
endif
...
cmd="grep -i '"//trim(name)//" *=' "//trim(coptest(nml,'data/param2.nml'))//" | sed -e's/^.*"//trim(name)//" *= *//'"  //" -e's/^.*"//upper_case(trim(name))//" *= *//'"
...
if (ind==nl-2) then
qual=LNEIGH2
elseif (ind==nl-1) then
qual=LNEIGH
elseif (ind==nl) then
qual=LGAP
elseif (ind==nl+1.and..not.lrestr_par_low.and.lrestr_perp) then
qual=LMARG
elseif (ind==nl+2.and..not.lrestr_par_low.and.lrestr_perp) then
qual=LMARG2
elseif (ind==nu-1.and..not.lrestr_par_up.and.lrestr_perp) then
qual=RMARG2
