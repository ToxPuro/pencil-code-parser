...
interface grad
module procedure grad_main
module procedure grad_other
endinterface
!
...
if (lcylindrical_coords)  call fatal_error('rms_mn','not implemented for cylindrical')
!
...
if (lcylindrical_coords)  call fatal_error('rms2_mn','not implemented for cylindrical')
!
...
if (lcylindrical_coords)  call fatal_error('sum_mn','not implemented for cylindrical')
!
...
QQ =   A(1,1)*A(2,2)   + A(2,2)*A(3,3)   + A(1,1)*A(3,2)   - A(1,2)*A(2,1)   - A(2,3)*A(3,2)   - A(1,3)*A(3,1)
...
g=g+tmp
if (lspherical_coords)  g=g+r1_mn*(2.*f(l1:l2,m,n,k1+1)+cotth(m)*f(l1:l2,m,n,k1+2))
...
if (j==1) then
if (nxgrid/=1) then
df=( -    (f(l1+1:l2+1,m,n,k)-f(l1-2:l2-2,m,n,k))      +27.*(f(l1  :l2  ,m,n,k)-f(l1-1:l2-1,m,n,k)) )/(24.*dx)
else
df=0.
...
elseif (j==2) then
if (nygrid/=1) then
df=( -    (f(l1:l2,m+1,n,k)-f(l1:l2,m-2,n,k))     +27.*(f(l1:l2,m  ,n,k)-f(l1:l2,m-1,n,k)) )/(24.*dy)
...
elseif (j==3) then
if (nzgrid/=1) then
df=( -    (f(l1:l2,m,n+1,k)-f(l1:l2,m,n-2,k))     +27.*(f(l1:l2,m,n  ,k)-f(l1:l2,m,n-1,k)) )/(24.*dz)
...
a1 = 1
a2 = size(a,1)
if (((a2 /= nx) .and. (a2 /= mx)) .or. (size(a,2) /= 3))  call fatal_error('curl_mn','array "a" has wrong size, must be (nx,3) or (mx,3)')
if (a2 == mx) then
a1 = l1
a2 = l2
endif
...
if (rcyl_mn(1)==0.) b(i1,3)=(360.*b(i2,3)-450.*b(i3,3)+400.*b(i4,3)  -225.*b(i5,3)+72.*b(i6,3)-10.*b(i7,3))/147.
endif
!
...
call fatal_error('del2v',  'Cannot do a spherical del2v without aij and aa')
endif
!

! for r component (factors of line elements are taken care of inside p%uij

del2f(:,1)= del2f(:,1)+ r1_mn*(2.*(fij(:,1,1)-fij(:,2,2)-fij(:,3,3)  -r1_mn*pff(:,1)-cotth(m)*r1_mn*pff(:,2) )  +cotth(m)*fij(:,1,2) )
! for theta component

del2f(:,2)=del2f(:,2)+ r1_mn*(2.*(fij(:,2,1)-cotth(m)*fij(:,3,3) +fij(:,1,2) ) +cotth(m)*fij(:,2,2)-r1_mn*sin1th(m)*sin1th(m)*pff(:,2) )
! for phi component

del2f(:,3)=del2f(:,3)+ r1_mn*(2.*(fij(:,3,1)+fij(:,1,3) +cotth(m)*fij(:,2,3) )  +cotth(m)*fij(:,3,2)-sin1th(m)*pff(:,3) )
endif
!
...
if (lcylindrical_coords) then
call fatal_error('del2m',  'del2m is not implemented in cylindrical coordiates')
endif
!

if (lspherical_coords) then
call fatal_error('del2m',  'del2m is not implemented in spherical coordiates')
endif
!
...
if (.not. lcartesian_coords)  call fatal_error('sub','del2fi_dxjk implemented only in cartesian')
!
...
if (.not. lcartesian_coords)  call fatal_error('sub','d2fidxj implemented only in cartesian')
!
...
if (.not. lcartesian_coords)  call fatal_error('sub','d2f_dxj implemented only in cartesian')
!
...
do i=1,3
do j=1,i
if (i/=j)  matrix_ut3x3(:,i,j) = matrix_ut3x3(:,j,i)
enddo
enddo
!
...
do i=1,3
do j=1,3
if (present(del2) .or. present(curlcurl) .or. present(gradcurl) .or.  present(graddiv)) then
call der2 (f,k1+i,tmp,  j); fijj(:,i,j)=tmp
endif
if (present(graddiv) .or. present(curlcurl).or. present(gradcurl)) then
call derij(f,k1+j,tmp,j,i); fjji(:,i,j)=tmp
endif
enddo
enddo
!
...
fjik(:,3)=tmp
if (lcylindrical_coords.or.lspherical_coords)  call fatal_error('del2v_etc','gradcurl at del2v_etc not '//  'implemented for non-cartesian coordinates')
endif
!
...
if (lspherical_coords) call fatal_error('del2v_etc',  'del2 at del2v_etc not implemented for spherical '//  'coordinates - use gij_etc')
endif
!
...
if (lspherical_coords) call fatal_error('del2v_etc',  'graddiv is implemented in gij_etc for spherical '//  'coords - use gij_etc')
endif
!
...
if (lcylindrical_coords) call fatal_error('del2v_etc',  'curlcurl not implemented for non-cartesian coordinates')
if (lspherical_coords) call fatal_error('del2v_etc',  'curlcurl not implemented for non-cartesian coordinates - '//  'use gij_etc')
endif
!
...
if (lcylindrical_coords.or.lspherical_coords)  call fatal_error('del2v_etc','gradcurl not implemented '//  'for non-cartesian coordinates')
endif
!
...
if (lcylindrical_coords.or.lspherical_coords)  call fatal_error('del2vi_etc',  'not implemented for non-cartesian coordinates')
!
...
if (lcylindrical_coords.or.lspherical_coords)  call fatal_error('del4v',  'not implemented for non-cartesian coordinates')
!
...
call der2(f,iay,tmp,1)
call derij(f,iax,d2adrdt,1,2)
call der(f,iax,tmp1,2)
...
call der2(f,iax,tmp,2)
call derij(f,iay,d2adrdt,1,2)
call der(f,iay,tmp1,2)
...
if (lspherical_coords) then
if (.not.present(aij))  call fatal_error('gij_etc', 'aij needed for spherical coordinates')
do i=1,3
...
if (lcylindrical_coords) then
if (.not.present(aij))  call fatal_error('gij_etc', 'aij needed for cylindrical coordinates')
do i=1,3
...
if (lspherical_coords) then
if (.not.present(aa))  call fatal_error('gij_etc', 'aa needed for spherical coordinates')
...
bij(:,2,3)=bij(:,2,3)+(aij(:,1,2)*r1_mn-aij(:,2,1)*r1_mn-aa(:,2)*r2_mn) *cotth(m)
...
if (lcylindrical_coords) then
if (.not.present(aa))  call fatal_error('gij_etc', 'aa needed for cylindrical coordinates')
...
if (lspherical_coords) then
if (.not.present(aa))  call fatal_error('gij_etc', 'aa needed for spherical coordinates')
del2(:,1)= del2(:,1)+ r1_mn*(2.*(aij(:,1,1)-aij(:,2,2)-aij(:,3,3) -r1_mn*aa(:,1)-cotth(m)*r1_mn*aa(:,2) )  +cotth(m)*aij(:,1,2) )
del2(:,2)=del2(:,2)+ r1_mn*(2.*(aij(:,2,1)-cotth(m)*aij(:,3,3) +aij(:,1,2) ) +cotth(m)*aij(:,2,2)-r1_mn*sin2th(m)*aa(:,2) )
del2(:,3)=del2(:,3)+ r1_mn*(2.*(aij(:,3,1)+aij(:,1,3) +cotth(m)*aij(:,2,3) )  +cotth(m)*aij(:,3,2)-r1_mn*sin2th(m)*aa(:,3) )
endif
if (lcylindrical_coords) then
if (.not.present(aa))  call fatal_error('gij_etc', 'aa needed for cylindrical coordinates')
del2(:,1)= del2(:,1)+ rcyl_mn1*(aij(:,1,1)-2*aij(:,2,2))-rcyl_mn2*aa(:,1)
del2(:,2)=del2(:,2)+ rcyl_mn1*(aij(:,2,1)-2*aij(:,1,2))-rcyl_mn2*aa(:,2)
endif
endif
!
...
if (lcylindrical_coords.or.lspherical_coords)  call fatal_error('del4',  'not implemented for non-cartesian coordinates')
endif
!
...
if (ikk .ne. iuu) call fatal_error("del4graddiv", "del4graddiv only coded for velocity")
!
...
if (.not.lcartesian_coords)  call fatal_error('del6_other',  'not implemented for non-cartesian coordinates')
!
...
if (lcylindrical_coords.or.lspherical_coords)  call fatal_error('del6fj',  'not implemented for non-cartesian coordinates')
!
...
if (lcylindrical_coords.or.lspherical_coords)  call fatal_error('del2fjv',  'not implemented for non-cartesian coordinates')
!
...
case default
if (lroot) print*, 'sub:u_dot_grad: iadvec must be 0,1 or 2, iadvec=',  iadvec
...
udelf = (aphalf-vel)*(delfjp1+delfj)/2. +  (amhalf-vel)*(delfjm1+delfj)/2.
!
...
if (lcylindrical_coords)  ugradf=ugradf-rcyl_mn1*abs(uu(:,2))*del6u(:,2)
if (lspherical_coords)  ugradf=ugradf-r1_mn*abs(uu(:,2))*del6u(:,2)
endif
!
...
if (lspherical_coords)  ugradf=ugradf-r1_mn*sin1th(m)*abs(uu(:,3))*del6u(:,3)
endif
!
...
subroutine rdim(file,mx_in,my_in,mz_in,mvar_in,maux_in,mglobal_in, prec_in,nghost_in,ipx_in, ipy_in, ipz_in)
!
...
if (t0 == 0.0) then
if (file==trim(trim(datadir)//'/t2davg.dat') .or.   file==trim(trim(datadir)//'/t1davg.dat')) tout = 0.0
endif
else settout
...
write(lun) 0,0,0,t_sp,dummy,dummy
nvec=0
endif
!
...
real, parameter :: t4 = 0.
real, parameter, dimension (-1:1,-1:1,-1:1) :: interp3D = reshape( (/ t3, t2, t3,  t2, t1, t2,  t3, t2, t3,  t2, t1, t2,  t1, t4, t1,  t2, t1, t2,  t3, t2, t3,  t2, t1, t2,  t3, t2, t3 /), (/ 3,3,3 /))
integer :: ii,jj,kk
integer :: j
!
...
do jj=-1,1
do ii=-1,1
if (ii/=0.or.jj/=0.or.kk/=0)  meanf(3:mx-2)=meanf(3:mx-2)+interp3D(ii,jj,kk)*(f(3+ii:mx-2+ii,m+jj,n+kk,j)-tmp_penc(3:mx-2))
enddo
enddo
enddo
else
call fatal_error('shock_max3_pencil_interp',  'Tony got lazy and only implemented the 3D case')
endif
!
...
write(*,'(A,A10,",  x: <",A8,">, y: <",A8,">,  z: <",A8,">")')  'Bcs for ', varname_input,  trim(bcx(idx)), trim(bcy(idx)), trim(bcz(idx))
!
...
number2=max(1,number-length-number1)
!
...
call error("noform",  "Increase max_col_width or sanitize print.in{,.double}")
endif
!
...
levi_civita=1.
elseif (  (i==3 .and. j==2 .and. k==1) .or.  (i==1 .and. j==3 .and. k==2) .or.  (i==2 .and. j==1 .and. k==3) ) then
levi_civita=-1.
else
levi_civita=0.
endif
...
select case(aemm)
case (0)
sph_har=sqrt(21./(4*pi))  *(46189.*cost**10 - 109395.*cost**8 + 90090.*cost**6 - 30030.*cost**4 + 3465.*cost**2 - 63.)/256.
case default
...
if (emm<ell)  der = der - sqrt((2.*ell+1.)*(ell-aemm)*(ell+aemm)/(2.*ell-1.))*  ylm_core(cost,sint,cosp,sinp,ell-1,emm)/sint
endif
!
...
arg = min(arg,8.)
sechx=1./cosh(arg)
der6_step = (1./(2*width**6))*( -272.0*(sechx**6)*tanhx+416.0*(sechx**4)*(tanhx**3)  -32.0*(sechx**2)*(tanhx**5) )
!
...
xi = max(xi,-1.0)
xi = min(xi, 1.0)
quintic_der_step_pt = (0.9375 + xi**2*(-1.875 + xi**2*0.9375))  * width1
!
...
xi = max(xi,-1.0)
xi = min(xi, 1.0)
quintic_der_step_mn = (0.9375 + xi**2*(-1.875 + xi**2*0.9375))  * width1
!
...
print*,'nan_inform: NaN with message "', msg,  '" encountered in the variable ', varname(d)
...
do j=1,mcom
if (bc(j) == '') then
if (lroot) print*, 'Empty boundary condition No. ',  j, 'in (x, y, or z)'
endif
isep = index(bc(j),':')
if (isep > 0) then
...
character (len=255) :: envname, chunk
character (len=1) :: chr
...
if (.not. control_file_exists)  control_file_exists = parallel_file_exists(trim("runtime/"//fname), loptest(delete))
!
...
if (nseed > mseed) then
if (lroot) print*, 'This machine requires mseed >= ', nseed,  ', but you have only ', mseed
...
write(1,'(A,A,6(", ",1PG12.4))') 'positions = ',  'regular, regular, regular',  x00, dx, y00, dy, z00, dz
write(1,'(A)') ''
write(1,'(A)') 'end'
!
...
f(:,:,:,i)=f(:,:,:,i)+exp(ampl*(  spread(spread(exp(-fact_x*(x-x01)**2),2,my),3,mz)  *spread(spread(exp(-fact  *(y-y01)**2),1,mx),3,mz)  *spread(spread(exp(-fact  *(z-z01)**2),1,mx),2,my)))
else
!

f(:,:,:,i)=f(:,:,:,i)+ampl*(  spread(spread(exp(-fact_x*(x-x01)**2),2,my),3,mz)  *spread(spread(exp(-fact  *(y-y01)**2),1,mx),3,mz)  *spread(spread(exp(-fact  *(z-z01)**2),1,mx),2,my))
endif
endif
!
...
hyp2F1=(gamma_function(cc)*gamma_function(cc-aa-bb))/  (gamma_function(cc-aa)*gamma_function(cc-bb))*  hypergeometric2F1(aa,bb,aa+bb-cc+1,1-z,tol)  +(1-z)**(cc-aa-bb)*  (gamma_function(cc)*gamma_function(aa+bb-cc))/  (gamma_function(aa)*gamma_function(bb))*  hypergeometric2F1(cc-aa,cc-bb,cc-aa-bb+1,1-z,tol)
!
...
coeff1=(/0.66761295020790986D00,  0.36946093910826145D00,  0.18669829780572704D00,  4.8801451277274492D-2,  1.36528684153155468D-2,  1.7488042503123817D-3,  3.6032044608268575D-4/)
!

coeff2=(/0.66761295020791116D00,  0.754817592058897962D00,  -3.7915754844972276D-2,  -0.11379619871302534D00,  1.5035521280605477D-2,  3.1375176929984225D-3,  -5.5599617153443518D-4/)
!
...
dumerfc =  t * exp(-z * z - 1.26551223 + t *         ( 1.00002368 + t * ( 0.37409196 + t *            ( 0.09678418 + t * (-0.18628806 + t *            ( 0.27886807 + t * (-1.13520398 + t *            ( 1.48851587 + t * (-0.82215223 + t * 0.17087277 )))))))))
!
...
print*,'get_radial_distance: '// 'the array has dimension=',size(rrmn),' is that correct?'
call fatal_error('get_radial_distance','')
endif
!
...
rrmn  =sqrt(rcylmn**2+(z(n)-e3)**2)
elseif (coord_system=='spherical') then
rcylmn=sqrt((xc*sinth(m))**2 + (e1*sin(e2))**2 -  2*xc*e1*costh(m)*cos(e2))+tini
rrmn  =sqrt(xc**2 + e1**2 - 2*xc*e1* (costh(m)*cos(e2)+sinth(m)*sin(e2)*cos(z(n)-e3)))+tini
endif
endif
!
...
a(j+1:n,j+1:n)=a(j+1:n,j+1:n)-spread(a(j+1:n,j),dim=2,ncopies=(n-j)) *  spread(a(j,j+1:n),dim=1,ncopies=(n-j))
enddo
!
...
if (lcylindrical_coords) call fatal_error('gij_etc',  'use del2=graddiv-curlcurl for cylindrical coords')
endif
!
...
case (1); call xlocation(pos(i),lpos,linx); lpos=min(max(l1,lpos),l2)
...
if (ipy==0) amps(:,1:idim)=buffer
endif
case (3)
if (nprocz>1) then
call mpireduce_sum(amps,buffer,(/2,idim/),idir=3)
if (ipz==0) amps(:,1:idim)=buffer
endif
end select
deallocate(buffer)
endif
...
if (maxval(a2) > 1.+1e-6)  call fatal_error('unit_vector:','has not the length 1')
!
...
umax1 = sqrt(maxval(  f(l1:l2,m1:m2,n1:n2,iv  )**2  + f(l1:l2,m1:m2,n1:n2,iv+1)**2  + f(l1:l2,m1:m2,n1:n2,iv+2)**2  ))
...
if (lyinyang) then
if (idir/=3)  call fatal_error('finalize_aver_3D',  'Not implemented for other than phi direction on Yin-Yang grid')
else
...
use Mpicomm, only: mpiallreduce_sum, mpireduce_sum
!
...
subroutine calc_slope_diff_flux(f,j,p,h_slope_limited,nlf,div_flux,div_type,  heat,heat_type,flux1,flux2,flux3)
!
...
if ((k == 1 .and. nxgrid /= 1) .or.  (k == 2 .and. nygrid /= 1) .or.  (k == 3 .and. nzgrid /= 1)) then
!

if (ldiv_4th) then
call slope_lim_lin_interpol(f,j,k,fim12_l,fim12_r,fim1, fip12_l,fip12_r,fip1,ldiv_4th, fimm12_l,fimm12_r,fimm1, fipp12_l,fipp12_r,fipp1)
call characteristic_speed(f,k,cmax_im12,cmax_ip12,ldiv_4th,cmax_imm12,cmax_ipp12)
else
call slope_lim_lin_interpol(f,j,k,fim12_l,fim12_r,fim1, fip12_l,fip12_r,fip1,ldiv_4th)
call characteristic_speed(f,k,cmax_im12,cmax_ip12,ldiv_4th)
endif
endif
!
...
heat=heat  +0.5*flux_im12(:,k)*(dens*f(l1:l2,m,n,j)-dens_m1*fim1)  /(x(l1:l2)-x(l1-1:l2-1))  +0.5*flux_ip12(:,k)*(dens_p1*fip1-dens*f(l1:l2,m,n,j))  /(x(l1+1:l2+1)-x(l1:l2))
endif
!
...
heat=heat  +0.5*flux_im12(:,k)*(dens*f(l1:l2,m,n,j)-dens_m1*fim1)  /(x(l1:l2)*(y(m)-y(m-1)))  +0.5*flux_ip12(:,k)*(dens_p1*fip1-dens*f(l1:l2,m,n,j))  /(x(l1:l2)*(y(m+1)-y(m)))
else
heat=heat  +0.5*flux_im12(:,k)*(dens*f(l1:l2,m,n,j)-dens_m1*fim1)  /(y(m)-y(m-1))  +0.5*flux_ip12(:,k)*(dens_p1*fip1-dens*f(l1:l2,m,n,j))  /(y(m+1)-y(m))
endif
endif
!
...
if (lspherical_coords) then
heat=heat  +0.5*flux_im12(:,k)*(dens*f(l1:l2,m,n,j)-dens_m1*fim1)  /(x(l1:l2)*sin(y(m))*(z(n)-z(n-1)))  +0.5*flux_ip12(:,k)*(dens_p1*fip1-dens*f(l1:l2,m,n,j))  /(x(l1:l2)*sin(y(m))*(z(n+1)-z(n)))
else
heat=heat  +0.5*flux_im12(:,k)*(dens*f(l1:l2,m,n,j)-dens_m1*fim1)  /(z(n)-z(n-1))  +0.5*flux_ip12(:,k)*(dens_p1*fip1-dens*f(l1:l2,m,n,j))  /(z(n+1)-z(n))
endif
endif
!
...
if (nxgrid /= 1) then
if(ldiv_4th) then
div_flux=div_flux  +(-flux_ipp12(:,1)+27.*flux_ip12(:,1)-27.*flux_im12(:,1)+flux_imm12(:,1))  /(x12(l1:l2)-x12(l1-1:l2-1))/24.
else
if (lspherical_coords) then
div_flux=div_flux  +(x12(l1:l2)**2*flux_ip12(:,1)-x12(l1-1:l2-1)**2*flux_im12(:,1)) /(x(l1:l2)**2*(x12(l1:l2)-x12(l1-1:l2-1)))
elseif (lcylindrical_coords) then
div_flux=div_flux  +(x12(l1:l2)*flux_ip12(:,1)-x12(l1-1:l2-1)*flux_im12(:,1)) /(x(l1:l2)*(x12(l1:l2)-x12(l1-1:l2-1)))
else
div_flux=div_flux+(flux_ip12(:,1)-flux_im12(:,1)) /(x12(l1:l2)-x12(l1-1:l2-1))
endif
endif
endif
!
...
if (nygrid /= 1) then
if(ldiv_4th) then
div_flux=div_flux  +(-flux_ipp12(:,2)+27.*flux_ip12(:,2)-27.*flux_im12(:,2)+flux_imm12(:,2)) /(y12(m)-y12(m-1))/24.
else
if (lspherical_coords) then
div_flux=div_flux  +(sin(y12(m))*flux_ip12(:,2)-sin(y12(m-1))*flux_im12(:,2)) /(x(l1:l2)*sin(y(m))*(y12(m)-y12(m-1)))
elseif (lcylindrical_coords) then
div_flux=div_flux  +(flux_ip12(:,2)-flux_im12(:,2)) /(x(l1:l2)*(y12(m)-y12(m-1)))
else
div_flux=div_flux  +(flux_ip12(:,2)-flux_im12(:,2)) /(y12(m)-y12(m-1))
endif
endif
endif
if (nzgrid /= 1) then
if(ldiv_4th) then
div_flux=div_flux  +(-flux_ipp12(:,3)+27.*flux_ip12(:,3)-27.*flux_im12(:,3)+flux_imm12(:,3)) /(z12(n)-z12(n-1))/24.
else
if (lspherical_coords) then
div_flux=div_flux+(flux_ip12(:,3)-flux_im12(:,3)) /(x(l1:l2)*sin(y(m))*(z12(n)-z12(n-1)))
else
div_flux=div_flux+(flux_ip12(:,3)-flux_im12(:,3)) /(z12(n)-z12(n-1))
endif
endif
endif
!
...
subroutine slope_lim_lin_interpol(f,j,k,fim12_l,fim12_r,fim1, fip12_l,fip12_r,fip1,ldiv_4th, fimm12_l,fimm12_r,fimm1, fipp12_l,fipp12_r,fipp1)
!
...
do i=l1,l2
ix=i-nghost
tmp1=f(i,m-1,n,j)-f(i,m-2,n,j)
tmp2=f(i,m,n,j)  -f(i,m-1,n,j)
tmp3=f(i,m+1,n,j)-f(i,m,n,j)
tmp4=f(i,m+2,n,j)-f(i,m+1,n,j)
delfym1(ix) = minmod_alt(tmp1,tmp2)
delfy(ix) = minmod_alt(tmp2,tmp3)
delfyp1(ix) = minmod_alt(tmp3,tmp4)
if(ldiv_4th) then
...
fipp12_r(ix) = f(i,m+2,n,j)-delfypp1(ix)
fimm1(ix) = f(i,m-2,n,j)
fipp1(ix) = f(i,m+2,n,j)
endif
enddo
endif
!
...
right_left = diffs(2:len-1) - 0.5*(slope(2:) + slope(1:len-2))
...
limited = sign(min( 2.*abs(diff_left), 2.*abs(diff_right),  0.5*abs(diff_right+diff_left) ), diff_left )
elsewhere
limited = 0.
endwhere
case ('minmod')
...
if (diff_left(ii)>0) limited(ii)=min(2.*diff_left(ii),2.*diff_right(ii),  0.5*(diff_right(ii)+diff_left(ii)))
if (diff_left(ii)<0) limited(ii)=max(2.*diff_left(ii),2.*diff_right(ii),  0.5*(diff_right(ii)+diff_left(ii)))
else
limited(ii)=0
endif
enddo
...
call fatal_error('traceless_strain',  'Deformation matrix for curvilinear co-ordinates'//  'requires providing of the velocity itself')
return
endif
endif
!
...
real, dimension(nx),       intent(OUT):: fint
!
...
facc=facq*fac
fint = ( f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3,-1))+f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3, 0))  +f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3, 0))+f(ijk(1,-1):i2(-1),ijk(2, 0),ijk(3,-1))  +f(ijk(1, 0):i2( 0),ijk(2,-1),ijk(3,-1))+f(ijk(1,-1):i2(-1),ijk(2, 0),ijk(3, 0))  +f(ijk(1, 0):i2( 0),ijk(2,-1),ijk(3, 0))+f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3,-1)) )*facc
elseif (lactive_dimension(3)) then
fint = ( f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3,-1))+f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3,-1))  +f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3, 0))+f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3, 0)))*facq
else
!print*, ijk(1, 0),i2( 0),ijk(2,-1),n,f(ijk(1, 0):i2( 0),ijk(2,-1),n)

fint = ( f(ijk(1,-1):i2(-1),ijk(2,-1),n)+f(ijk(1, 0):i2( 0),ijk(2, 0),n)  +f(ijk(1,-1):i2(-1),ijk(2, 0),n)+f(ijk(1, 0):i2( 0),ijk(2,-1),n))*facq
endif
endif
...
i2 = ijk(1,:)+nx-1
if (dimensionality==1) then
fint = (f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3,-1))+f(ijk(1,0):i2(0),ijk(2,0),ijk(3,0)))*facp  -(f(ijk(1,-2):i2(-2),ijk(2,-2),ijk(3,-2))+f(ijk(1,1):i2(1),ijk(2,1),ijk(3,1)))*facm
else
facmq=facm*facm; facpq=facp*facp; facpm=facp*facm
if (dimensionality==2) then
if (lactive_dimension(3)) then
fint =  ( f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3,-1))+f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3, 0))          +f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3,-1))+f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3, 0)) )*facpq  -( f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3,+1))+f(ijk(1,+1):i2(+1),ijk(2,+1),ijk(3,-1))          +f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3,-2))+f(ijk(1,-2):i2(-2),ijk(2,-2),ijk(3,-1))          +f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3,-2))+f(ijk(1,-2):i2(-2),ijk(2,-2),ijk(3, 0))          +f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3,+1))+f(ijk(1,+1):i2(+1),ijk(2,+1),ijk(3, 0)) )*facpm  +( f(ijk(1,-2):i2(-2),ijk(2,-2),ijk(3,-2))+f(ijk(1,+1):i2(+1),ijk(2,+1),ijk(3,+1))          +f(ijk(1,-2):i2(-2),ijk(2,-2),ijk(3,+1))+f(ijk(1,+1):i2(+1),ijk(2,+1),ijk(3,-2)) )*facmq
else
fint =  ( f(ijk(1,-1):i2(-1),ijk(2,-1),n)+f(ijk(1, 0):i2( 0),ijk(2, 0),n)          +f(ijk(1,-1):i2(-1),ijk(2, 0),n)+f(ijk(1, 0):i2( 0),ijk(2,-1),n) )*facpq  -( f(ijk(1,-1):i2(-1),ijk(2,+1),n)+f(ijk(1,+1):i2(+1),ijk(2,-1),n)          +f(ijk(1,-1):i2(-1),ijk(2,-2),n)+f(ijk(1,-2):i2(-2),ijk(2,-1),n)          +f(ijk(1, 0):i2( 0),ijk(2,-2),n)+f(ijk(1,-2):i2(-2),ijk(2, 0),n)          +f(ijk(1, 0):i2( 0),ijk(2,+1),n)+f(ijk(1,+1):i2(+1),ijk(2, 0),n) )*facpm  +( f(ijk(1,-2):i2(-2),ijk(2,-2),n)+f(ijk(1,+1):i2(+1),ijk(2,+1),n)          +f(ijk(1,-2):i2(-2),ijk(2,+1),n)+f(ijk(1,+1):i2(+1),ijk(2,-2),n) )*facmq
endif
else
facpc =facpq*facp
facpqm=facpq*facm
facpmq=facp*facmq
facmc =facmq*facm
fint = ( f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3,-1))+f(ijk(1, 0):i2( 0),ijk(2,-1),ijk(3,-1))  +f(ijk(1,-1):i2(-1),ijk(2, 0),ijk(3,-1))+f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3, 0))  +f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3,-1))+f(ijk(1, 0):i2( 0),ijk(2,-1),ijk(3, 0))  +f(ijk(1,-1):i2(-1),ijk(2, 0),ijk(3, 0))+f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3, 0)) )*facpc  -( f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3,-2))+f(ijk(1, 0):i2( 0),ijk(2,-1),ijk(3,-2))      +f(ijk(1,-1):i2(-1),ijk(2, 0),ijk(3,-2))+f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3,-2))      +f(ijk(1,-1):i2(-1),ijk(2,-1),ijk(3,+1))+f(ijk(1, 0):i2( 0),ijk(2,-1),ijk(3,+1))      +f(ijk(1,-1):i2(-1),ijk(2, 0),ijk(3,+1))+f(ijk(1, 0):i2( 0),ijk(2, 0),ijk(3,+1))      +f(ijk(1,-1):i2(-1),ijk(2,-2),ijk(3,-1))+f(ijk(1, 0):i2( 0),ijk(2,-2),ijk(3,-1))      +f(ijk(1,-1):i2(-1),ijk(2,-2),ijk(3, 0))+f(ijk(1, 0):i2( 0),ijk(2,-2),ijk(3, 0))      +f(ijk(1,-1):i2(-1),ijk(2,+1),ijk(3,-1))+f(ijk(1, 0):i2( 0),ijk(2,+1),ijk(3,-1))      +f(ijk(1,-1):i2(-1),ijk(2,+1),ijk(3, 0))+f(ijk(1, 0):i2( 0),ijk(2,+1),ijk(3, 0))      +f(ijk(1,-2):i2(-2),ijk(2,-1),ijk(3,-1))+f(ijk(1,-2):i2(-2),ijk(2, 0),ijk(3,-1))      +f(ijk(1,-2):i2(-2),ijk(2,-1),ijk(3, 0))+f(ijk(1,-2):i2(-2),ijk(2, 0),ijk(3, 0))      +f(ijk(1,+1):i2(+1),ijk(2,-1),ijk(3,-1))+f(ijk(1,+1):i2(+1),ijk(2, 0),ijk(3,-1))      +f(ijk(1,+1):i2(+1),ijk(2,-1),ijk(3, 0))+f(ijk(1,+1):i2(+1),ijk(2, 0),ijk(3, 0)) )*facpqm  +( f(ijk(1,-2):i2(-2),ijk(2,-2),ijk(3,-1))+f(ijk(1,+1):i2(+1),ijk(2,-2),ijk(3,-1))      +f(ijk(1,-2):i2(-2),ijk(2,+1),ijk(3,-1))+f(ijk(1,+1):i2(+1),ijk(2,+1),ijk(3,-1))      +f(ijk(1,-2):i2(-2),ijk(2,-2),ijk(3, 0))+f(ijk(1,+1):i2(+1),ijk(2,-2),ijk(3, 0))      +f(ijk(1,-2):i2(-2),ijk(2,+1),ijk(3, 0))+f(ijk(1,+1):i2(+1),ijk(2,+1),ijk(3, 0))      +f(ijk(1,-2):i2(-2),ijk(2,-1),ijk(3,-2))+f(ijk(1,+1):i2(+1),ijk(2,-1),ijk(3,-2))      +f(ijk(1,-2):i2(-2),ijk(2,-1),ijk(3,+1))+f(ijk(1,+1):i2(+1),ijk(2,-1),ijk(3,+1))      +f(ijk(1,-2):i2(-2),ijk(2, 0),ijk(3,-2))+f(ijk(1,+1):i2(+1),ijk(2, 0),ijk(3,-2))      +f(ijk(1,-2):i2(-2),ijk(2, 0),ijk(3,+1))+f(ijk(1,+1):i2(+1),ijk(2, 0),ijk(3,+1))      +f(ijk(1,-1):i2(-1),ijk(2,-2),ijk(3,-2))+f(ijk(1,-1):i2(-1),ijk(2,+1),ijk(3,-2))      +f(ijk(1,-1):i2(-1),ijk(2,-2),ijk(3,+1))+f(ijk(1,-1):i2(-1),ijk(2,+1),ijk(3,+1))      +f(ijk(1, 0):i2( 0),ijk(2,-2),ijk(3,-2))+f(ijk(1, 0):i2( 0),ijk(2,+1),ijk(3,-2))      +f(ijk(1, 0):i2( 0),ijk(2,-2),ijk(3,+1))+f(ijk(1, 0):i2( 0),ijk(2,+1),ijk(3,+1)) )*facpmq  -( f(ijk(1,-2):i2(-2),ijk(2,-2),ijk(3,-2))+f(ijk(1,+1):i2(+1),ijk(2,-2),ijk(3,-2))      +f(ijk(1,-2):i2(-2),ijk(2,+1),ijk(3,-2))+f(ijk(1,-2):i2(-2),ijk(2,-2),ijk(3,+1))      +f(ijk(1,+1):i2(+1),ijk(2,+1),ijk(3,-2))+f(ijk(1,+1):i2(+1),ijk(2,-2),ijk(3,+1))      +f(ijk(1,-2):i2(-2),ijk(2,+1),ijk(3,+1))+f(ijk(1,+1):i2(+1),ijk(2,+1),ijk(3,+1)) )*facmc
endif
endif
...
vel=0.
where (pom>=torus%r_in .and. pom<=torus%r_in+torus%thick .and.  points(:,3)>=-torus%height/2. .and. points(:,3)<=torus%height/2. )
...
if (size(kernel,1)/2==nghost) then
width = nghost
