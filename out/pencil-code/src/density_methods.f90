...
if (lreference_state)  call get_shared_variable('reference_state',reference_state,  caller='initialize_density_methods')
...
if (ldensity_nolog) then
if (lreference_state) then
lnrho(l1:l2,:)= log(f(l1:l2,:)  +spread(reference_state(:,iref_rho),2,size(f,2)))
else
lnrho=log(f)
endif
else
...
if (ldensity_nolog) then
if (lreference_state) then
rho(l1:l2,:)= f(l1:l2,:)  +spread(reference_state(:,iref_rho),2,size(f,2))
else
rho=f
endif
else
...
if (ldensity_nolog) then
if (lreference_state) then
rho(l1:l2,:,:) = f(l1:l2,:,:)  +spread(spread(reference_state(:,iref_rho),2,size(f,2)),3,size(f,3))
else
rho=f
endif
else
...
if (ldensity_nolog) then
f(l1:l2)=rho
if (lreference_state)  f(l1:l2)=f(l1:l2)-reference_state(:,iref_rho)
else
f(l1:l2)=log(rho)
endif
...
if (ldensity_nolog) then
f(l1:l2,:)=exp(lnrho)
if (lreference_state)  f(l1:l2,:)=f(l1:l2,:)-spread(reference_state(:,iref_rho),2,my)
else
f(l1:l2,:)=lnrho
endif
...
n=iz
do m=1,size(f,2)
call der(f(:,:,:,ilnrho),derlnrho(:,m),3)
enddo
n=n_save; m=m_save
if (ldensity_nolog) then
if (lreference_state) then
derlnrho(l1:l2,:) = derlnrho(l1:l2,:)/(f(l1:l2,:,iz,ilnrho)  +spread(reference_state(:,iref_rho),2,my))
else
...
if (ldensity_nolog) then
if (lreference_state) then
dlnrho(l1:l2,:) = dlnrho(l1:l2,:)/(f(l1:l2,rm,:)  +spread(reference_state(:,iref_rho),2,mz))
else
dlnrho = dlnrho/f(:,rm,:)
endif
endif
...
dlnrho = f(:,:,rn+in)-f(:,:,rn-in)
if (ldensity_nolog) then
if (lreference_state) then
dlnrho(l1:l2,:) = dlnrho(l1:l2,:)/(f(l1:l2,:,rn)  +spread(reference_state(:,iref_rho),2,my))
else
dlnrho = dlnrho/f(:,:,rn)
endif
endif
