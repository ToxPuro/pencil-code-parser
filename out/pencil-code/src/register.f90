...
if (ierr /= 0) call stop_it("Cannot open "//trim(datadir)//  "/def_var.pro for writing -- is "//trim(datadir)//" visible from root node?")
if (ldebug) print *, 'Creating ' // trim(datadir) // '/variables.pro'
open(4,FILE=trim(datadir)//'/variables.pro',IOSTAT=ierr)
if (ierr /= 0) call stop_it("Cannot open "//trim(datadir)//  "/variables.pro for writing -- is "//trim(datadir)//" visible from root node?")
write(4,*) 'close,1'
...
call stop_it('register_modules: nvar /= mvar. '//  'Check your MVAR and/or MAUX CONTRIBUTION in cparam.local')
endif
!
...
use Param_IO
use Mpicomm,          only: mpireduce_sum,mpibcast_real, mpisend_real,mpirecv_real
...
if (lroot.and.leos_ionization.and.ip<14) print*,'initialize_modules: '  //'unit_velocity, unit_density, etc, are in cgs'
...
use Cdata, only: unit_system,G_Newton,c_light,hbar,lroot,  unit_length,unit_velocity,unit_density,unit_magnetic
...
if (G_Newton == impossible .or.  c_light == impossible .or.  hbar == impossible) then
...
if (lroot.and.ip<14) call information('choose_pencils','finding out which pencils '//  'are needed for the pencil case')
!
...
print*, 'choose_pencils: Swapped requested pencil number ',  ipencil_swap, ' to ', lpenc_requested(ipencil_swap)
endif
if (lpencil_diagnos_swap) then
lpenc_diagnos(ipencil_swap) = (.not. lpenc_diagnos(ipencil_swap))
print*, 'choose_pencils: Swapped diagnostic pencil number ',  ipencil_swap, ' to ', lpenc_diagnos(ipencil_swap)
endif
endif
!
...
if (parallel_file_exists(trim(print_in_file)//'.double'))  print_in_file = trim(print_in_file)//'.double'
endif
!
...
elseif ( nname==0 ) then
call fatal_error('rprint_list','You must have a "'//  trim(print_in_file)//  '" file in the run directory with valid print requests!')
endif
!
...
do i=1,nnamerz
cname_tmp=ctmp(i)
if ( cname_tmp=='uumphi' .or. cname_tmp=='uusphmphi' .or.   cname_tmp=='bbmphi' .or. cname_tmp=='bbsphmphi' .or.   cname_tmp=='uxbmphi'.or. cname_tmp=='jxbmphi'       )  iadd=iadd+2
enddo
...
lwrite_1daverages=  lwrite_xyaverages.or.lwrite_xzaverages.or.lwrite_yzaverages.or.lwrite_phizaverages
!
...
if (lpoint<1.or.lpoint>mx .or. lpoint2<1.or.lpoint2>mx .or.  mpoint<1.or.mpoint>mx .or. mpoint2<1.or.mpoint2>mx .or.  npoint<1.or.npoint>mx .or. npoint2<1.or.npoint2>mx) then
