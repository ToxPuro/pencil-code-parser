...
elseif (lpole(i)) then
if (lperi(i)) call fatal_error('start', 'lperi and lpole cannot be used together in same component')
if (.not.lspherical_coords) call fatal_error('start', 'lpole only implemented for spherical coordinates')
if (i==2) then
xyz0(i) = 0.
Lxyz(i) = pi
xyz0(3) = 0.
...
if (xyz1(i)/=impossible)  call fatal_error('start','Cannot set Lxyz and xyz1 at the same time')
endif
!
...
if (lroot) call write_dx_general(trim(datadir)//'/var.general',  x0-nghost*dx,y0-nghost*dy,z0-nghost*dz)
!
...
if (lyinyang.and.lroot)  call warning('start','Most initial conditions depending on y or z will be set correctly only on Yin grid.')
!
...
do i=1,init_loops
if (lroot .and. init_loops/=1)  print '(A33,i3,A25)', 'start: -- performing loop number', i,  ' of initial conditions --'
call init_gg(f)
!
