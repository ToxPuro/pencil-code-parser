...
if (lroot) then
if (.not. file_exists(file)) call fatal_error(  'parallel_read', 'file "'//trim(file)//'" not found', force=.true.)
bytes=file_size(file)
if (bytes < 0) call fatal_error(  'parallel_read', 'could not determine size of file "'//trim(file)//'"', force=.true.)
if (bytes == 0) call fatal_error(  'parallel_read', 'file "'//trim(file)//'" is empty', force=.true.)
endif
!
