...
if (headt .and. (imn==1)) write(*,'(A)')  'output_pencil: Writing to ' // trim(filename) //  ' for debugging -- this may slow things down'
!

call output_penciled_vect_c(filename, a, ndim,  imn, mm(imn), nn(imn), t_sp,  nx, ny, nz, nghost, len(filename))
!
...
t_sp = t
filename = trim (datadir_snap)//'/'//trim (file)
if ((ip<=8) .and. lroot .and. imn==1)  print*,'output_pencil_scal('//trim(filename)//')'
!

if (ndim /= 1)  call stop_it("output_pencil_scal: called with scalar field, but ndim/=1")
!

if (headt .and. (imn==1)) print*,  'output_pencil_scal: Writing to ', trim(filename),  ' for debugging -- this may slow things down'
!

call output_penciled_scal_c(filename, a, ndim,  imn, mm(imn), nn(imn), t_sp,  nx, ny, nz, nghost, len(filename))
!
