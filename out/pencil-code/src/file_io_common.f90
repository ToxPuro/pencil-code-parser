...
len = rec_len
num = num_rec
if (len /= rec_len) call fatal_error ('fseek_pos on unit '//trim (itoa (unit)),  "rec_len is not representable as integer(kind=4).", .true.)
if (num /= num_rec) call fatal_error ('fseek_pos on unit '//trim (itoa (unit)),  "num_rec is not representable as integer(kind=4).", .true.)
!
...
backspace(lun)
if (kind(x)==rkind8) then
read(lun,iostat=ios) x
backspace(lun)
endif
!
...
call warning ('read_namelist', 'namelist "'//trim(name)//trim(type)//trim(suffix)//  '" has an error ('//trim(itoa(ierr))//')!')
endif
endif
endif
!
...
open (unit, FILE=trim(source)//trim(directory_snap)//'/zaverages.dat',  FORM='unformatted', status='old')
buffer=0.
else
open (unit, FILE=trim(source)//trim(directory_snap)//'/zaverages.dat',  FORM='unformatted', status='old', position='append')
backspace(unit)
backspace(unit)
endif
...
call warning('read_zaver','file "'//  trim(source)//trim(directory_snap)//'/zaverages.dat" does not exist')
endif
