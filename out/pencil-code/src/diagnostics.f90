...
public :: zsum_mn_name_xy_mpar_scal, zsum_mn_name_xy_mpar,  zsum_mn_name_xy_arr, zsum_mn_name_xy_arr2
! GPU-END
...
where( fname_keep(1:nname) /= impossible .and.  itype_name(1:nname)<ilabel_complex )  buffer(1:nname) = buffer(1:nname)+fname_keep(1:nname)
!
...
do iname=nname,1,-1
if (itype_name(iname)>=ilabel_complex .and. cform(iname)/='')  call insert(buffer,(/fname_keep(iname)/),iname+1,nnamel)
enddo
!
...
fform = tform//','
if ( ncoords_sound>1 )  call safe_character_append(fform, trim(itoa(ncoords_sound))//'(')
!
...
if (itype==ilabel_sum_weighted .or.  itype==ilabel_sum_weighted_sqrt .or.  itype==ilabel_sum_par .or.  itype==ilabel_sum_sqrt_par .or.  itype==ilabel_sum_log10_par) then
...
call mpireduce_sum(fsum_tmp,fsum,nsum_count,comm=MPI_COMM_WORLD)
...
if (itype==ilabel_max)             vname(iname)=fmax(imax_count)
!

if (itype==ilabel_max_sqrt)        vname(iname)=sqrt(fmax(imax_count))
!

if (itype==ilabel_max_dt)          vname(iname)=fmax(imax_count)
!

if (itype==ilabel_max_neg)         vname(iname)=-fmax(imax_count)
!

if (itype==ilabel_max_reciprocal)  vname(iname)=1./fmax(imax_count)
!
...
if (itype==ilabel_save)             vname(iname)=fsum(isum_count)
!

if (itype==ilabel_sum)             vname(iname)=fsum(isum_count)*dVol_rel1
!

if (itype==ilabel_sum_sqrt)        vname(iname)=sqrt(fsum(isum_count)*dVol_rel1)
!

if (itype==ilabel_sum_log10)        vname(iname)=log10(fsum(isum_count)*dVol_rel1)
!

if (itype==ilabel_sum_par)         vname(iname)=fsum(isum_count)/fweight(isum_count)
!

if (itype==ilabel_sum_sqrt_par)         vname(iname)=sqrt(fsum(isum_count))/fweight(isum_count)
!

if (itype==ilabel_sum_log10_par)         vname(iname)=log10(fsum(isum_count)/fweight(isum_count))
!

if (itype==ilabel_integrate.or.itype==ilabel_sum_plain)       vname(iname)=fsum(isum_count)
!

if (itype==ilabel_integrate_sqrt)       vname(iname)=sqrt(fsum(isum_count))
!

if (itype==ilabel_integrate_log10)       vname(iname)=log10(fsum(isum_count))
!

if (itype==ilabel_surf)           vname(iname)=fsum(isum_count)
!
...
lvn = len(trim(vname))
if (vlabel(1:lvn)/=trim(vname))  call fatal_error('expand_cname_short','vlabel does not start with vname')
!
...
call expand_cname_full(ccname,nname,ic,  trim(vname)//'r'//trim(tail),  trim(vname)//'t'//trim(tail),  trim(vname)//'p'//trim(tail),form)
elseif ( lcylindrical_coords ) then
call expand_cname_full(ccname,nname,ic,  trim(vname)//'r'//trim(tail),  trim(vname)//'p'//trim(tail),  trim(vname)//'z'//trim(tail),form)
else
call expand_cname_full(ccname,nname,ic,  trim(vname)//'x'//trim(tail),  trim(vname)//'y'//trim(tail),  trim(vname)//'z'//trim(tail),form)
endif
endif
!
...
if (.not.lextend_vector(ccname,nname+itot-1))  call fatal_error('expand_cname_full','Not enough memory')
!
...
call warning("sum_lim_mn_name","no reason to call it if you are "// "not using a cylinder or a sphere embedded in a "// "Cartesian grid")
endif
!
...
if (nnamer==mnamer) call fatal_error('phizsum_mn_name_r',  'no slot for phi-normalization. decrease nnamer')
!
...
if (.not.(lcylinder_in_a_box.or.lsphere_in_a_box))  call warning('calc_phiavg_profile',  'no reason to call it if you are '// 'not using a cylinder or a sphere embedded in a '// 'Cartesian grid')
!
...
if (ierr>0) call fatal_error('allocate_sound',  'Could not allocate memory for sound_inds')
!
...
if ((sound_inds(il,1) == lsound) .and.  (sound_inds(il,2) == msound) .and.  (sound_inds(il,3) == nsound) ) then
lval = .false.
exit
endif
enddo
...
if (ierr>0) call fatal_error('allocate_sound',  'Could not allocate memory for cname_sound')
!
...
if (ierr>0) call fatal_error('allocate_sound',  'Could not allocate memory for sound_coords_list')
sound_coords_list = sound_inds(1:ncoords_sound,:)
!

allocate(fname_sound(ncoords_sound,nnamel),stat=ierr)
if (ierr>0) call fatal_error('allocate_sound',  'Could not allocate memory for fname_sound')
if (ldebug) print*, 'allocate_sound: allocated memory for '//  'fname_sound  with nname_sound  =', nnamel
!
...
if (ierr>0) call fatal_error('allocate_sound',  'Could not allocate memory for cform_sound')
cform_sound = ' '
!
...
if (stat>0) call fatal_error('allocate_fnames',  'Could not allocate memory for cname')
if (ldebug) print*, 'allocate_fnames    : allocated memory for '//  'cname   with nname   =', nnamel
cname=''
!

allocate(fname(nnamel),stat=stat)
if (stat>0) call fatal_error('allocate_fnames',  'Could not allocate memory for fname')
...
if (stat>0) call fatal_error('allocate_fnames',  'Could not allocate memory for cform')
if (ldebug) print*, 'allocate_fnames    : allocated memory for '//  'cform   with nname   =', nnamel
cform=''
!
...
if (stat>0) call fatal_error('allocate_vnames',  'Could not allocate memory for cnamev')
if (ldebug) print*, 'allocate_vnames    : allocated memory for '//  'cnamev  with nnamev  =', nnamel
cnamev=''
!

if (allocated(cformv)) deallocate(cformv)
allocate(cformv(nnamel),stat=stat)
if (stat>0) call fatal_error('allocate_vnames',  'Could not allocate memory for cformv')
if (ldebug) print*, 'allocate_vnames    : allocated memory for '//  'cformv   with nname   =', nnamel
cformv=''
!
...
if (stat>0) call fatal_error('allocate_xyaverages',  'Could not allocate memory for cnamez')
if (ldebug) print*, 'allocate_xyaverages: allocated memory for '//  'cnamez  with nnamez  =', nnamel
cnamez=''
!
...
if (stat>0) call fatal_error('allocate_xyaverages',  'Could not allocate memory for cformz')
if (ldebug) print*, 'allocate_xyaverages: allocated memory for '//  'cformz  with nnamez  =', nnamel
cformz=''
!
...
if (stat>0) call fatal_error('allocate_xzaverages',  'Could not allocate memory for cnamey')
if (ldebug) print*, 'allocate_xzaverages: allocated memory for '//  'cnamey  with nnamey  =', nnamel
cnamey=''
!
...
if (stat>0) call fatal_error('allocate_xzaverages',  'Could not allocate memory for cformy', .true.)
if (ldebug) print*, 'allocate_xzaverages: allocated memory for '//  'cformy  with nnamey  =', nnamel
cformy=''
!
...
if (stat>0) call fatal_error('allocate_yzaverages',  'Could not allocate memory for cnamex')
if (ldebug) print*, 'allocate_yzaverages: allocated memory for '//  'cnamex  with nnamex  =', nnamel
cnamex=''
!
...
if (stat>0) call fatal_error('allocate_yzaverages',  'Could not allocate memory for cformx')
if (ldebug) print*, 'allocate_yzaverages: allocated memory for '//  'cformx  with nnamex  =', nnamel
cformx=''
!
...
if (stat>0) call fatal_error('allocate_phizaverages',  'Could not allocate memory for cnamer')
if (ldebug) print*, 'allocate_phizaverages: allocated memory for '//  'cnamer  with nnamel =', nnamel
cnamer=''
!
...
if (stat>0) call fatal_error('allocate_phizaverages',  'Could not allocate memory for cformr')
if (ldebug) print*, 'allocate_phizaverages: allocated memory for '//  'cformr  with nnamel =', nnamel
cformr=''
!
...
if (stat>0) call fatal_error('allocate_yaverages',  'Could not allocate memory for cnamexz')
if (ldebug) print*, 'allocate_yaverages : allocated memory for '//  'cnamexz with nnamexz =', nnamel
cnamexz=''
!
...
if (stat>0) call fatal_error('allocate_yaverages',  'Could not allocate memory for cformxz')
if (ldebug) print*, 'allocate_yaverages : allocated memory for '//  'cformxz with nnamexz =', nnamel
cformxz=''
!
...
if (stat>0) call fatal_error('allocate_zaverages',  'Could not allocate memory for cnamexy')
if (ldebug) print*, 'allocate_zaverages : allocated memory for '//  'cnamexy with nnamexy =', nnamel
cnamexy=''
!

allocate(cformxy(nnamel),stat=stat)
if (stat>0)  call fatal_error('allocate_zaverages',  'Could not allocate memory for cformxy')
if (ldebug) print*, 'allocate_zaverages : allocated memory for '//  'cformxy with nnamel =', nnamel
cformxy=''
...
if (ldebug) print*, 'allocate_zaverages_data: allocated memory for '//  'fnamexy_cap with nycap, nnamexy =', nycap, nnamel
endif
!
...
if (stat>0) call fatal_error('allocate_phiaverages',  'Could not allocate memory for cnamerz')
if (ldebug) print*, 'allocate_phiaverages : allocated memory for '//  'cnamerz with nnamerz =', nnamel
cnamerz=''
!
...
if (stat>0) call fatal_error('allocate_phiaverages',  'Could not allocate memory for cformrz')
if (ldebug) print*, 'allocate_phiaverages : allocated memory for '//  'cformrz with nnamerz =', nnamel
cformrz=''
!
