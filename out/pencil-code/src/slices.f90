...
if (lwrite_slice_r) slices%r => slice_r
slices%ready=.false.
!
...
if (ldensity .or. lanelastic)  call get_slices_density     (f,slices)
...
if (lhydro.or.lhydro_kinematic)  call get_slices_hydro       (f,slices)
...
if (nprocx>1) call warning('setup_slice',  'slice_position=w may be wrong for nprocx>1')
iy = nygrid/2+nghost
...
if (nprocx>1) call warning('setup_slice',  'slice_position=s may be wrong for nprocx>1')
...
if (nprocx>1) call warning('setup_slice',  'slice_position=e may be wrong for nprocx>1')
lwrite_slice_xy=lfirst_proc_z; if (lwrite_slice_xy) iz_loc=n1
...
if (lroot)  call fatal_error('setup_slices',  'No such value for slice_position: '//slice_position)
endif
!
...
lwrite_slice_r=.false.
elseif (r_rslice>min(xyz1(1),-xyz0(1)) .or. r_rslice>min(xyz1(2),-xyz0(2))  .or. r_rslice>min(xyz1(3),-xyz0(3)) ) then
