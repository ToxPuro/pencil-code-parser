...
call stop_it_if_any(.true.,'pencil_consistency_check: '//  'failed to allocate required memory')
endif
endif
...
if (lroot) then
print*, 'pencil_consistency_check: the presence of NaNs '//  'in "df_ref" makes this test impossible'
call stop_it_if_any(.true.,'pencil_consistency_check: '//  'generation of reference failed, quitting pencil check')
endif
endif
call stop_it_if_any(.false.,'')
!
...
do penc=0,nite
if (lroot.and.penc==0)  print*, 'pencil_consistency_check: performing small pencil check'
if (lroot.and.penc==1)  print*, 'pencil_consistency_check: performing full pencil check'//  ' (takes a while)'
df=0.0
if (lpencil_check_no_zeros) then
call random_seed_wrapper(PUT=iseed_org)
do i=1,mvar+maux
...
do iv=1,mvar
if (lfound_nan(iv)) then
print*, 'pencil_consistency_check: NaNs in "df" at variable',  iv, ' for pencil ', trim(pencil_names(penc))
...
print '(a,i4,a)', ' pencil_consistency_check: '//  'possible overcalculation... pencil '// trim(pencil_names(penc))//' (',penc,')'//  ' is requested, but does not appear to be required!'
elseif ( (.not. lconsistent_allproc) .and.  (.not. lpenc_requested(penc)) ) then
print '(a,i4,a)',' pencil_consistency_check: '//  'MISSING PENCIL... pencil '//  trim(pencil_names(penc))//' (',penc,')'//  ' is not requested, but calculating it changes the results!'
ldie=.true.
endif
else
if (.not.lconsistent_allproc) then
print*, 'pencil_consistency_check: '//  'the small pencil check has found one or more'
print*, '                          MISSING pencils. '//  'This could be a serious problem.'
if (lpencil_check) then
print*, 'pencil_consistency_check: '//  'the full pencil check beginning will now find'
print*, '                          out which pencils are missing'
else
print*, 'pencil_consistency_check: '//  'you need to run the full pencil check to find'
print*, '                          out which ones ('//  'set lpencil_check=T in &run_pars'
print*, '                          and run again)'
ldie=.true.
endif
else
print*, 'pencil_consistency_check: '//  'the small pencil check reported no problems'
endif
endif
endif
enddo
...
if (lroot) print*, 'pencil_consistency_check: '//  'checking dependence on pencil initialization'
df=0.0
if (lpencil_check_no_zeros) then
call random_seed_wrapper(PUT=iseed_org)
do i=1,mvar+maux
...
if (lroot) then
if (.not. lconsistent_allproc) then
print*, 'pencil_consistency_check: '//  'results depend on pencil initialization'
print*, '                          This is a serious problem '//  'that may show the use of'
print*, '                          uninitialized pencils. Check '//  'carefully that all'
print*, '                          pencils are calculated and '//  'that conjugate pencils are'
print*, '                          calculated *after* the '//  'individual components'
ldie=.true.
else
print*, 'pencil_consistency_check: '//  'results are independent of pencil initialization'
endif
endif
!
...
if (lroot)  print*, 'pencil_consistency_check: checking diagnostics pencils'
!
...
if (lroot) then
if (penc>0) then
if (lpencil_check_diagnos_opti .and.  lconsistent_allproc .and. lpenc_diagnos(penc) ) then
print '(a,i4,a)','pencil_consistency_check: '//  'OPTIMISATION POTENTIAL... pencil '//  trim(pencil_names(penc))//' (',penc,')'//  ' is requested for diagnostics, '//  'but does not appear to be required!'
elseif ( (.not. lconsistent_allproc) .and.  (.not. lpenc_diagnos(penc)) .and.  (.not. lpenc_requested(penc)) ) then
print '(a,i4,a)','pencil_consistency_check: '//  'MISSING PENCIL... pencil '//  trim(pencil_names(penc))//' (',penc,')'//  ' is not requested for diagnostics, '//  'but calculating it changes the diagnostics!'
ldie=.true.
endif
else
if (.not.lconsistent_allproc) then
print*, 'pencil_consistency_check: '//  'the small pencil check has found one or more'
print*, '                          MISSING diagnostics pencils. '//  'This could be a serious problem.'
if (lpencil_check) then
print*, 'pencil_consistency_check: '//  'the full pencil check beginning now will find'
print*, '                          out which pencils are missing'
else
print*, 'pencil_consistency_check: '//  'you need to run the full pencil check to find'
print*, '                          out which ones ('//  'set lpencil_check=T in &run_pars'
...
print*, 'pencil_consistency_check: '//  'set lpencil_check_diagnos_opti=T in run.in to report'
print*, 'pencil_consistency_check: '//  'optimisation possibilities for diagnostics pencils'
endif
!
...
if (lroot) then
if (.not. lconsistent_allproc) then
print*, 'pencil_consistency_check: '//  'diagnostics depend on pencil initialization'
print*, '                          This is a serious problem '//  'that may show the use of'
print*, '                          uninitialized pencils. Check '//  'carefully that all'
print*, '                          pencils are calculated and '//  'that conjugate pencils are'
print*, '                          calculated *after* the '//  'individual components'
else
print*, 'pencil_consistency_check: '//  'diagnostics are independent of pencil initialization'
endif
endif
!
