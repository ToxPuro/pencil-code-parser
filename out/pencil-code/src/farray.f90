...
call fatal_error("farray_register_variable",  "Registering "//trim(varname)//" as scratch variable fails. Use the"//  "farray_acquire_scratch_area routine.")
endif
!
...
call fatal_error("farray_register_variable",  "Registering "//trim(varname)//" fails: Name already exists but with a different "// "number of components!")
endif
...
call fatal_error("farray_register_variable",  "Registering "//trim(varname)//" fails: Name already exists but with a different "// "array size!")
endif
...
call fatal_error("farray_register_variable",  "Registering "//trim(varname)//" fails: Name already exists but with a different variable type!")
endif
...
call fatal_error("farray_register_variable",  "Registering "//trim(varname)//" fails: Insufficient mvar variables allocated.  This probably means "// "the MVAR CONTRIBUTION header is incorrect in one of the physics "//  "modules or in cparam.local.")
endif
...
call fatal_error("farray_register_variable",  "Registering "//trim(varname)//" fails: Insufficient maux variables allocated.  This either means "// "the MAUX CONTRIBUTION header is incorrect in one of the physics "//  "modules. Or that there you are using some code that can, depending "//  "on runtime parameters, require extra auxiliary variables.  For the "//  "latter try adding an MAUX CONTRIBUTION header to cparam.local.")
endif
...
call fatal_error("farray_register_variable",  "Registering "//trim(varname)//" fails: Insufficient maux_com variables allocated.  This either means "// "the COMMUNICATED AUXILIARIES header is incorrect in one of the physics "//  "modules. Or that there you are using some code that can, depending "//  "on runtime parameters, require extra auxiliary variables.  For the "//  "latter try adding an MAUX CONTRIBUTION and COMMUNICATED AUXILIARIES "//  "headers to cparam.local.")
endif
...
call fatal_error("farray_register_variable",  "Registering "//trim(varname)//" fails: Insufficient mglobal variables allocated.  This either means "// "the MGLOBAL CONTRIBUTION header is incorrect in one of the physics "//  "modules. Or that there you are using some code that can, depending "//  "on runtime parameters, require extra global variables.  For the "//  "latter try adding an MGLOBAL CONTRIBUTION header to cparam.local.")
endif
case default
call fatal_error("farray_register_variable",  "Registering "//trim(varname)//" fails: Invalid vartype set")
endselect
!
...
if (.not. lwrite_aux .and. (vartype==iFARRAY_TYPE_COMM_AUXILIARY .or.  vartype==iFARRAY_TYPE_AUXILIARY )) return
!
...
if ((num_vector /= 0) .and. (num_vector /= 3) .and. (num_vector /= 6) .and. (num_vector /= 9)  .and. lfatal_num_vector_369)  call fatal_error ("farray_index_append", "vector (or tensor) '"//trim(varname)//"' must have 3, 6, or 9 components!")
...
call fatal_error("farray_acquire_scratch_area",  "f array variable name already exists and with a different "// "number of components!")
endif
if (present(ierr)) then
ierr=iFARRAY_ERR_DUPLICATE
ivar=0
...
print*,"Acquiring f-array scratch area: ",varname
call fatal_error("farray_acquire_scratch_area",  "There are insufficient mscratch slots allocated. This means that"//  "the total number of 3D scratch arrays used at any one time is more"//  "than the number currently allocated.  Try adding "//  "an MSCRATCH CONTRIBUTION header to cparam.local or increasing the value.")
endif
!
...
print*,"Acquiring f-array scratch area: ",varname
call fatal_error("farray_acquire_scratch_area",  "There are insufficient consecutive mscratch slots available. "//  "The scratch area has become fragmented!")
endif
!
...
call fatal_error("farray_release_scratch_area",  "scratch area not found")
endif
!
...
print*,"Releasing f-array scratch area: ",varname
call fatal_error("farray_release_scratch_area",  "scratch area found but not"//  "with the index specified")
endif
!
...
character(len=*), parameter :: MESSAGE =  NEWLINE // "The count of MAUXs is inconsistent with the actual number of" //  NEWLINE // "auxiliaries required.  Please check your src/cparam.local.  It is" //  NEWLINE // "also possible that someone unintentionally added *optional*" //  NEWLINE // "auxiliaries to MAUX CONTRIBUTION to one of the physics modules." //  NEWLINE // "Please urge them to move it to src/cparam.local under their" //  NEWLINE // "specific application. "
!
