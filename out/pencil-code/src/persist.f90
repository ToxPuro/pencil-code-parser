...
if (lshear) then
if (read_persist ('SHEAR_DELTA_Y', deltay))  call warning('input_persist_general_by_label','no persistent value of deltay found')
endif
!
...
if (write_persist ('RANDOM_SEEDS', id_record_RANDOM_SEEDS, seed(1:nseed)))  output_persistent_general = .true.
endif
!

call random_seed_wrapper (GET=seed2,CHANNEL=2)
if (any (seed2(1:nseed) /= seed0)) then
if (write_persist ('RANDOM_SEEDS2', id_record_RANDOM_SEEDS2, seed2(1:nseed)))  output_persistent_general = .true.
endif
!

if (lshear) then
if (write_persist ('SHEAR_DELTA_Y', id_record_SHEAR_DELTA_Y, deltay))  output_persistent_general = .true.
endif
!
